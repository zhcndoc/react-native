...
- 如果禁用 Hermes，React Native 将使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)，也就是为 Safari 提供支持的 JavaScript 引擎。请注意，在 iOS 上，由于 iOS 应用中缺少可写的可执行内存，JavaScriptCore 不会使用 JIT。
- 在使用 Chrome 调试时，所有 JavaScript 代码都在 Chrome 本身中运行，并通过 WebSockets 与原生代码通信。Chrome 使用 [V8](https://v8.dev/) 作为其 JavaScript 引擎。

尽管这些环境非常相似，你仍可能会遇到一些不一致之处。最好避免依赖任何运行时的具体实现细节。

## JavaScript 语法转换器

语法转换器通过让你可以使用新的 JavaScript 语法，而无需等待所有解释器都支持它，从而让编写代码更愉快。

React Native 附带 [Babel JavaScript 编译器](https://babeljs.io)。请查看 [Babel 文档](https://babeljs.io/docs/plugins/#transform-plugins) 中关于其支持的转换的更多细节。
...
<tbody>
  ...
  <TableRow name="async 函数" code="async function doStuffAsync() {const foo = await doOtherStuffAsync();};" url="https://github.com/tc39/ecmascript-asyncawait" />
...
许多标准函数在所有受支持的 JavaScript 运行时中也可用。
