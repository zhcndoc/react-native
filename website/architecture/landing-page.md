...
function ViewWithTooltip() {
  ...
  // 我们获取布局信息并传递给 Tooltip 以定位自身
  const onLayout = React.useCallback(event => {
    targetRef.current?.measureInWindow((x, y, width, height) => {
      // 此状态更新不保证在同一次提交中运行
      // 这会导致 Tooltip 重新定位时出现视觉“跳动”
...
function ViewWithTooltip() {
  ...
  useLayoutEffect(() => {
    // `targetRect` 的测量和状态更新发生在单次提交中
    // 允许 Tooltip 定位自身而无需中间绘制
    targetRef.current?.measureInWindow((x, y, width, height) => {
      setTargetRect({x, y, width, height});
    });
  }, [setTargetRect]);
...
<div className="TwoColumns TwoFigures">
 <figure>
  <img src="/img/new-architecture/async-on-layout.gif" alt="一个视图正在移动到视口的角落和中心，工具提示渲染在其上方或下方。工具提示在视图移动后短暂延迟后渲染。" />
  <figcaption>Tooltip 的异步测量和渲染。[查看代码](https://gist.github.com/lunaleaps/eabd653d9864082ac1d3772dac217ab9)。</figcaption>
</figure>
<figure>
  <img src="/img/new-architecture/sync-use-layout-effect.gif" alt="一个视图正在移动到视口的角落和中心，工具提示渲染在其上方或下方。视图和工具提示同步移动。" />
  <figcaption>Tooltip 的同步测量和渲染。[查看代码](https://gist.github.com/lunaleaps/148756563999c83220887757f2e549a3)。</figcaption>
</figure>
</div>
