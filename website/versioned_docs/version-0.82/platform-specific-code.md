...
React Native 提供了一个模块来检测应用运行的平台。你可以使用检测逻辑来实现平台特定的代码。当组件中只有一小部分是平台特定的时，使用此选项。
...
还有一个 `Platform.select` 方法可用，给定一个对象，其键可以是 `'ios' | 'android' | 'native' | 'default'` 之一，返回最适合当前运行平台的值。也就是说，如果你在手机上运行，`ios` 和 `android` 键将优先。如果未指定这些，将使用 `native` 键，然后是 `default` 键。
...
## 特定于 Native 的扩展名（即与 NodeJS 和 Web 共享代码）
...
```shell
Container.js # 被 webpack、Rollup 或任何其他 Web 打包工具选取
Container.native.js # 被 React Native 打包工具选取，适用于 Android 和 iOS (Metro)
```
...
**专业提示：** 配置你的 Web 打包工具以忽略 `.native.js` 扩展名，以避免在生产 bundle 中包含未使用的代码，从而减小最终 bundle 的大小。
