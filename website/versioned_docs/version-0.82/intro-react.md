...
React Native 运行在 [React](https://react.dev/) 之上，这是一个流行的用于使用 JavaScript 构建用户界面的开源库。为了充分利用 React Native，理解 React 本身会有所帮助。本节可以帮助你入门，也可以作为复习课程。
...
:::tip
这是导出组件的多种方式之一。这种导出方式适用于 Snack Player。但是，根据你的应用的文件结构，你可能需要使用不同的约定。这份 [JavaScript 导入和导出的便捷速查表](https://medium.com/dailyjs/javascript-module-cheatsheet-7bd474f1d829) 会有所帮助。
:::
...
React 和 React Native 使用 **JSX**，这是一种允许你在 JavaScript 内部编写元素的语法，如下所示：`<Text>Hello, I am your cat!</Text>`。React 文档有一份 [JSX 综合指南](https://react.dev/learn/writing-markup-with-jsx) 供你参考以了解更多。因为 JSX 是 JavaScript，你可以在其中使用变量。这里你为猫声明了一个名字 `name`，并使用花括号将其嵌入到 `<Text>` 中。
...
:::tip
因为 JSX 包含在 React 库中，如果你没有在文件顶部编写 `import React from 'react'`，它将无法工作！
:::
...
:::note
注意 `style` 的宽度和高度周围的双花括号 `{{ }}`。在 JSX 中，JavaScript 值使用 `{}` 引用。如果你传递的不是字符串作为 props，比如数组或数字，这很方便：`<Cat food={["fish", "kibble"]} age={2} />`。但是，JS 对象 **_也_** 用花括号表示：`{width: 200, height: 200}`。因此，要在 JSX 中传递 JS 对象，你必须将对象包裹在 **另一对** 花括号中：`{{width: 200, height: 200}}`
:::

你可以用 props 和核心组件 [`Text`](text)、[`Image`](image) 和 [`View`](view) 构建许多东西！但要构建交互式的東西，你需要 state。
...
你可以通过调用 [React 的 `useState` Hook](https://react.dev/learn/state-a-components-memory) 向组件添加 state。Hook 是一种让你能够“钩入”React 功能的函数。例如，`useState` 是一个允许你向函数组件添加 state 的 Hook。你可以在 [React 文档中了解更多关于其他类型的 Hook。](https://react.dev/reference/react)
...
:::info
你可能已经注意到，虽然 `isHungry` 是一个 [const](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/const)，但它似乎可以重新赋值！发生的情况是，当像 `setIsHungry` 这样的 state 设置函数被调用时，其组件将重新渲染。在此情况下，`Cat` 函数将再次运行——这次，`useState` 将给我们 `isHungry` 的下一个值。
:::
...
:::info
看到上面的 `<>` 和 `</>` 了吗？这些 JSX 片段是 [片段](https://react.dev/reference/react/Fragment)。相邻的 JSX 元素必须包裹在一个封闭标签中。片段让你这样做而无需嵌套额外的、不必要的包装元素，如 `View`。
:::

---

现在你已经涵盖了 React 和 React Native 的核心组件，让我们通过查看 [处理 `<TextInput>`](handling-text-input) 来更深入地探讨其中一些核心组件。
