...
尚不支持其他包含的响应者（contained responders）阻止此 scroll view 成为响应者。

`<ScrollView>` 与 [`<FlatList>`](flatlist.md) - 该用哪一个？

`ScrollView` 一次性渲染所有其 React 子组件，但这有性能缺点。

想象你有一个很长的列表想要显示，可能有几屏的内容。一次性创建所有 JS 组件和原生视图，其中许多甚至可能不会显示，这将导致渲染缓慢和内存使用增加。

这就是 `FlatList` 发挥作用的地方。`FlatList` 懒加载渲染项，当它们即将出现时渲染，并移除滚动出屏幕很远的项以节省内存和处理时间。

如果你想在项之间渲染分隔符、多列、无限滚动加载或它支持的任何其他功能，`FlatList` 也很方便。
...
当为 true 时，即使内容小于 scroll view 本身，滚动视图在到达尽头时也会在水平方向弹跳。

| 类型 | 默认值                                                |
| ---- | ----------------------------------------------------- |
| bool | `true` 当 `horizontal={true}`<hr/>`false` 其他情况 |

---

### `alwaysBounceVertical` <div className="label ios">iOS</div>

当为 true 时，即使内容小于 scroll view 本身，滚动视图在到达尽头时也会在垂直方向弹跳。

| 类型 | 默认值                                                |
| ---- | ----------------------------------------------------- |
| bool | `false` 当 `horizontal={true}`<hr/>`true` 其他情况 |
...
当为 `true` 时，手势可以驱动缩放超过最小/最大值，并且缩放将在手势结束时动画化到最小/最大值，否则缩放将不会超过限制。
...
当为 `false` 时，一旦跟踪开始，如果触摸移动，将不会尝试拖拽。
...
这些样式将应用于滚动视图内容容器，该容器包裹所有子视图。示例：
...
此属性指定如何使用安全区域 insets 来修改滚动视图的内容区域。适用于 iOS 11 及更高版本。
...
当为 true 时，ScrollView 将尝试在拖拽时仅锁定垂直或水平滚动。
...
当为 true 时，ScrollView 上的默认 JS pan 响应者被禁用，ScrollView 内部触摸的完全控制权留给其子组件。如果启用了 `snapToInterval`，这特别有用，因为它不遵循典型的触摸模式。不要在没有 `snapToInterval` 的常规 ScrollView 用例中使用此属性，因为它可能导致滚动时发生意外的触摸。
...
有时 scrollview 占据的空间比其内容填充的空间大。在这种情况下，此属性将用颜色填充 scrollview 的其余部分，以避免设置背景并创建不必要的过度绘制。这是一个高级优化，在一般情况下不需要。
...
如果值大于 `0`，淡出边缘将根据当前的滚动方向和位置进行设置，指示是否有更多内容要显示。

| 类型                                               | 默认值 |
| -------------------------------------------------- | ------ |
| number<hr />对象：`{start: number, end: number}` | `0`     |
...
- `'interactive'`，键盘随拖拽交互消失并与触摸同步移动，向上拖拽取消消失。在 Android 上不支持此功能，它将具有与 `'none'` 相同的行为。
...
- `'never'` 当键盘弹出时，点击聚焦的文本输入框外部会消失键盘。当这种情况发生时，子项不会接收点击。
- `'always'`，键盘不会自动消失，滚动视图不会捕获点击，但滚动视图的子项可以捕获点击。
- `'handled'`，当点击被滚动视图的子项处理（或被祖先捕获）时，键盘不会自动消失。
- `false`，**_已废弃_**，改用 `'never'`
- `true`，**_已废弃_**，改用 `'always'`
...
设置后，滚动视图将调整滚动位置，以便当前可见且位于或超过 `minIndexForVisible` 的第一个子项不会改变位置。这对于双向加载内容的列表很有用，例如聊天线程，其中新消息进入否则可能导致滚动位置跳跃。值 0 很常见，但其他值（如 1）可用于跳过加载旋转器或其他不应保持位置的内容。

可选的 `autoscrollToTopThreshold` 可用于在调整后使内容自动滚动到顶部，如果用户在调整前位于顶部阈值内。这也适用于类似聊天的应用程序，你希望看到新消息滚动到位，但如果用户已经向上滚动了一段距离，滚动一大段内容会造成干扰。

注意事项 1：在此启用的情况下重新排序 scrollview 中的元素可能会导致跳跃和卡顿。它可以修复，但目前没有计划这样做。现在，不要重新排序使用此功能的任何 ScrollViews 或 Lists 的内容。
...
当为 `true` 时，当子视图离屏时，它们会从其原生备份父视图中移除。这可以提高大列表的滚动性能。在 Android 上，默认值为 `true`。
...
用于记录此滚动视图滚动性能的标签。将强制开启动量事件（参见 sendMomentumEvents）。这在开箱即用情况下不起任何作用，你需要实现一个自定义原生 FpsListener 才能使其有用。
...
设置后，导致滚动视图停在定义的偏移量处。这可用于分页浏览各种长度小于滚动视图的子项。通常与 `decelerationRate="fast"` 结合使用。覆盖配置较少的 `pagingEnabled` 和 `snapToInterval` 属性。
...
当设置为 `true` 时，向下滚动列表时粘性头部将被隐藏，向上滚动时它将停靠在列表顶部。
...
一个子项索引数组，确定哪些子项在滚动时停靠在屏幕顶部。例如，传递 `stickyHeaderIndices={[0]}` 将导致第一个子项固定在滚动视图的顶部。你也可以使用像 [x,y,z] 这样的方式使多个项在顶部时粘性。此属性不支持与 `horizontal={true}` 结合使用。
