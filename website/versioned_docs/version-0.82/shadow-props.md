...
- `boxShadow`：View 的样式属性，也是与 [同名 Web 样式属性](https://developer.mozilla.org/en-US/docs/Web/CSS/box-shadow) 规范一致的实现。
...
- `dropShadow` 是一个 alpha 蒙版，因此只有正 alpha 值的像素才会“投射”阴影。`boxShadow` 会围绕元素的边框盒投射阴影，无论其内容是什么（除非它是内阴影）。
...
此属性仅在 Android API 28 及以上版本有效。若要在更低版本的 Android API 上实现类似功能，请使用 [`elevation` 属性](view-style-props#elevation-android)。
