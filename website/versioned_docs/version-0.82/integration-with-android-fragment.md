...
遵循 [与现有应用集成](/docs/integration-with-existing-apps) 指南直到结束，以确保你可以安全地在全屏 Activity 中运行你的 React Native 应用。

### 2. 为 React Native Fragment 添加一个 FrameLayout

在这个示例中，我们将使用 `FrameLayout` 将一个 React Native Fragment 添加到一个 Activity 中。这种方法足够灵活，可以适应在其他布局中使用 React Native，例如 Bottom Sheets 或 Tab Layouts。
...
确保运行 `yarn start` 来启动 bundler，然后在 Android Studio 中运行你的 android 应用。应用应该从开发服务器加载 JavaScript/TypeScript 代码，并将其显示在 Activity 中的 React Native Fragment 中。
