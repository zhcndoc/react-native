...
您也可以通过 iOS 模拟器中的 Debug > Open System Log… 访问这些日志，或者当 Android 应用运行在设备或模拟器上时，在终端中运行 `adb logcat "*:S" ReactNative:V ReactNativeJS:V`。
...
另一个选项是使用 React Native CLI 运行您的应用，并将原生 IDE（Android Studio 或 Xcode）的原生调试器附加到该进程。
...
在 Xcode 中，点击顶部菜单栏上的 "Debug"，选择 "Attach to process" 选项，并在 "Likely Targets" 列表中选择应用。
