...
:::caution
Flexbox 在 React Native 中的工作方式与其在 Web 的 CSS 中相同，但有少数例外。
...
[`flex`](layout-props#flex) 会定义你的元素如何沿主轴利用可用空间进行 **“填充”**。空间会根据每个元素的 flex 属性进行分配。

在下面的示例中，红色、橙色和绿色视图都是容器视图中的子元素，该容器设置了 `flex: 1`。红色视图使用 `flex: 1`，橙色视图使用 `flex: 2`，绿色视图使用 `flex: 3`。**1+2+3 = 6**，这意味着红色视图将获得空间的 `1/6`，橙色视图获得 `2/6`，绿色视图获得 `3/6`。
...
[`flexDirection`](layout-props#flexdirection) 控制节点子元素的布局方向。这也被称为主轴。交叉轴是与主轴垂直的轴，或者是换行行所在的轴。

- `column`（**默认值**）将子元素从上到下排列。如果启用换行，则下一行将从容器顶部第一个元素的右侧开始。

- `row` 将子元素从左到右排列。如果启用换行，则下一行将从容器左侧第一个元素的下方开始。

- `column-reverse` 将子元素从下到上排列。如果启用换行，则下一行将从容器底部第一个元素的右侧开始。

- `row-reverse` 将子元素从右到左排列。如果启用换行，则下一行将从容器右侧第一个元素的下方开始。
...
布局 [`direction`](layout-props#direction) 指定层级中子元素和文本应如何布局。布局方向也会影响 `start` 和 `end` 分别指向哪一侧。默认情况下，React Native 采用 LTR 布局方向。在这种模式下，`start` 指向左侧，`end` 指向右侧。

- `LTR`（**默认值**）文本和子元素从左到右排列。应用到元素起始位置的 margin 和 padding 会作用在左侧。

- `RTL` 文本和子元素从右到左排列。应用到元素起始位置的 margin 和 padding 会作用在右侧。
...
[`justifyContent`](layout-props#justifycontent) 描述了如何在容器的主轴上对齐子元素。例如，你可以使用这个属性在 `flexDirection` 设置为 `row` 的容器中水平居中子元素，或者在 `flexDirection` 设置为 `column` 的容器中垂直居中子元素。
...
- `space-around` 在容器主轴上均匀分布子元素，并在子元素周围分配剩余空间。与 `space-between` 相比，使用 `space-around` 会让第一个子元素的开头和最后一个子元素的末尾也分配到空间。

- `space-evenly` 在对齐容器的主轴上均匀分布子元素。每一对相邻项目之间的间距、主轴起始边缘与第一个项目之间的间距，以及主轴结束边缘与最后一个项目之间的间距，都完全相同。

你可以在[这里](https://www.yogalayout.dev/docs/styling/justify-content)了解更多信息。
...
[`alignItems`](layout-props#alignitems) 描述了如何沿着容器的交叉轴对齐子元素。它与 `justifyContent` 非常相似，但 `justifyContent` 作用于主轴，而 `alignItems` 作用于交叉轴。
...
- `baseline` 将容器的子元素沿着共同的基线对齐。单个子元素可以被设置为其父级的参考基线。

:::info
要让 `stretch` 生效，子元素在次轴上不能有固定尺寸。在下面的示例中，只有移除子元素上的 `width: 50` 之后，设置 `alignItems: stretch` 才会生效。
:::

你可以在[这里](https://www.yogalayout.dev/docs/styling/align-items-self)了解更多信息。
...
[`alignSelf`](layout-props#alignself) 具有与 `alignItems` 相同的选项和效果，但不同于影响容器内的子项，你可以将此属性应用于单个子项，以改变其在父容器中的对齐方式。`alignSelf` 会覆盖父级通过 `alignItems` 设置的任何选项。
...
[alignContent](layout-props#aligncontent) 定义了沿交叉轴的行分布方式。只有当项目使用 `flexWrap` 换行到多行时才会生效。

- `flex-start` (**默认值**) 将换行后的各行对齐到容器交叉轴起始处。

- `flex-end` 将换行后的各行对齐到容器交叉轴末端。

- `stretch` (_在 web 上使用 Yoga 时的默认值_) 拉伸换行后的各行，使其与容器交叉轴的高度一致。

- `center` 将换行后的各行居中对齐在容器交叉轴上。
...
[`flexWrap`](layout-props#flexwrap) 属性设置在容器上，用于控制当子元素沿主轴方向超出容器尺寸时会发生什么。默认情况下，子元素会被强制放在单行中（这可能会让元素缩小）。如果允许换行，项目会在需要时沿主轴方向换成多行。

当发生换行时，可以使用 `alignContent` 来指定这些行在容器中的排列方式。更多信息请[点击这里](https://www.yogalayout.dev/docs/styling/flex-wrap)。
...
- [`flexBasis`](layout-props#flexbasis) 是一种与轴无关的方式，用于提供项目在主轴方向上的默认尺寸。如果其父元素是 `flexDirection: row` 的容器，那么为子元素设置 `flexBasis` 类似于设置该子元素的 `width`；如果其父元素是 `flexDirection: column` 的容器，那么则类似于设置子元素的 `height`。项目的 `flexBasis` 是该项目的默认尺寸，也就是在执行任何 `flexGrow` 和 `flexShrink` 计算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述了容器中应如何在其子元素之间沿主轴分配空间。布局完成后，容器会根据子元素指定的 flex grow 值分配任何剩余空间。

  `flexGrow` 接受任何大于等于 0 的浮点数，默认值为 0。容器会按照子元素的 `flexGrow` 值加权，在子元素之间分配剩余空间。

- [`flexShrink`](layout-props#flexshrink) 描述了在子元素总尺寸沿主轴方向超出容器尺寸时，如何沿主轴方向缩小子元素。`flexShrink` 与 `flexGrow` 非常相似，如果将任何溢出尺寸视为负的剩余空间，也可以用同样的方式来理解它。这两个属性也能很好地配合使用，使子元素可以按需放大和缩小。

  `flexShrink` 接受任何大于等于 0 的浮点数，默认值为 0（在 web 上默认值为 1）。容器会按照子元素的 `flexShrink` 值加权来缩小子元素。
...
- `relative`（**默认值**）默认情况下，元素采用相对定位。这意味着元素先按照布局的正常流进行定位，然后再根据 `top`、`right`、`bottom` 和 `left` 的值，相对于该位置进行偏移。该偏移不会影响任何兄弟元素或父元素的位置。

- `absolute` 绝对定位时，元素不参与正常的布局流，而是独立于其兄弟元素进行布局。其位置由 `top`、`right`、`bottom` 和 `left` 的值决定。这些值会将元素相对于其包含块进行定位。

- `static` 静态定位时，元素按照布局的正常流进行定位，并会忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 还会导致该元素不会为绝对定位的后代形成包含块，除非存在其他优先级更高的样式属性（例如 `transform`）。这使得 `absolute` 元素可以定位到不是其父元素的对象上。请注意，**`static` 仅在新架构中可用**。
...
应用于绝对定位元素的百分比长度（例如：`width: '50%'` 或 `padding: '10%'`）将相对于其包含块的尺寸进行计算。例如，如果包含块宽度为 100 点，那么绝对定位元素上的 `width: 50%` 会使其宽度变为 50 点。

下面的列表将帮助你确定任意给定元素的包含块：

- 如果该元素的 `position` 类型为 `relative` 或 `static`，那么它的包含块就是其父元素。
- 如果该元素的 `position` 类型为 `absolute`，那么它的包含块就是最近的祖先元素，并且满足以下条件之一：
  - 它的 `position` 类型不是 `static`
  - 它具有 `transform`

## 深入了解

查看交互式的 [yoga playground](https://www.yogalayout.dev/playground)，你可以借此更好地理解 flexbox。

我们已经介绍了基础内容，但布局可能还需要许多其他样式。控制布局的全部 prop 列表已在 [这里](./layout-props.md) 记录。
