...
一个用于让视图正确响应触摸的包装器。在按下时，被包装视图的不透明度会降低，从而呈现变暗效果。

不透明度通过将子组件包裹在添加到视图层级中的 `Animated.View` 中来控制。请注意，这可能会影响布局。
...
_（仅 Apple TV）_ TV 首选焦点（参见 View 组件的文档）。
