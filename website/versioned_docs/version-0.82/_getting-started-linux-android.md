...
遵循 [你的 Linux 发行版的安装说明](https://nodejs.org/en/download/package-manager/) 来安装 Node 20.19.4 或更高版本。
...
将以下行添加到你的 `$HOME/.bash_profile` 或 `$HOME/.bashrc` 配置文件（如果你使用的是 `zsh`，则是 `~/.zprofile` 或 `~/.zshrc`）：
...
:::info
[Watchman](https://facebook.github.io/watchman/docs/install) 是 Facebook 提供的一个用于监视文件系统变化的工具。强烈建议你安装它以获得更好的性能并在某些边缘情况下增加兼容性（翻译：你可能可以不安装这个就能凑合用，但效果因人而异；现在安装这个可能会让你以后省去麻烦）。
:::

<h2>准备 Android 设备</h2>

你需要一个 Android 设备来运行你的 React Native Android 应用。这可以是一个物理 Android 设备，或者更常见的是，你可以使用 Android 虚拟设备（AVD）来在电脑上模拟一个 Android 设备。
