...
在某些情况下，你可能希望拥有一个原生模块，监听平台层的一些事件，然后将它们发出到 JavaScript 层，让你的应用程序对这些原生事件做出反应。在其他情况下，你可能有一些长时间运行的操作可以发出事件，以便在发生时更新 UI。

这两者都是从原生模块发出事件的良好用例。在本指南中，你将学习如何做到这一点。

## 当存储中添加新键时发出事件

在此示例中，你将学习如何在将新键添加到存储时发出事件。更改键的值不会发出事件，但添加新键会。
...
通过 `import type` 语句，你从 `react-native` 导入了 `CodegenTypes`，其中包含 `EventEmitter` 类型。这允许你使用 `CodegenTypes.EventEmitter<KeyValuePair>` 定义 `onKeyAdded` 属性，指定该事件将发出类型为 `KeyValuePair` 的有效负载。
...
首先，你需要导入一些类型，用于创建需要从原生发送到 JS 的 eventData。这些导入是：
...
`NativeLocalStorageSpecBase` 是一个基类，提供 `emitOnKeyAdded` 方法及其基本实现和样板代码。多亏了这个类，你不必处理将事件发送到 JS 所需的 Objective-C 和 JSI 之间的所有转换。
