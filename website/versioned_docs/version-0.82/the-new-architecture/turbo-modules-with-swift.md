...
:::note
React Native 的核心主要用 C++ 编写，尽管 Apple 开发了 [互操作层](https://www.swift.org/documentation/cxx-interop/)，但 Swift 和 C++ 之间的互操作性并不理想。

因此，由于语言之间的不兼容性，本指南中你将编写的模块不会是纯 Swift 实现。你必须编写一些 Objective-C++ 胶水代码，但本指南的目标是最小化所需的 Objective-C++ 代码量。如果你正在将现有的原生模块从旧架构迁移到新架构，这种方法应该允许你复用大部分代码。
:::
...
你可以通过利用 [适配器](https://en.wikipedia.org/wiki/Adapter_pattern) 设计模式来实现这一点，将 Swift 模块与 Objective-C++ 层连接起来。
...
代码并没有真正改变。你不是直接创建对 `NSUserDefaults` 的引用，而是使用 Swift 实现创建一个新的 `NativeLocalStorage`，并且每当调用原生模块函数时，调用会被转发到 Swift 中实现的 `NativeLocalStorage`。

记得导入 `"SampleApp-Swift.h"` 头文件。这是一个由 Xcode 自动生成的头文件，其中包含 Swift 文件的公共 API，格式可供 Objective-C 使用。头文件中的 `SampleApp` 部分实际上是你的应用名称，因此如果你创建的应用名称与 `SampleApp` **不同**，你必须更改它。
...
这种方法在接口中引入了一些代码重复，但它允许你以很少的额外工作量复用代码库中可能已经拥有的 Swift 代码。

### 实现桥接头文件

:::note
如果你是库作者，开发一个将作为独立库分发的原生模块，则不需要此步骤。
:::
...
你的代码库中可能已经有一个桥接头文件，但如果没有，你可以通过以下步骤创建一个新的：

1. 在 Xcode 中，创建一个新文件并将其命名为 `"SampleApp-Bridging-Header.h"`
2. 如下所示更新 `"SampleApp-Bridging-Header.h"` 的内容：
...
现在你可以遵循 [原生模块指南](/docs/turbo-native-modules-introduction#build-and-run-your-code-on-a-simulator) 的最后一步，你应该会看到你的应用运行着用 Swift 编写的原生模块。
