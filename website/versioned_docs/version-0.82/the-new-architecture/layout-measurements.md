...
调用这些方法的最好方式是在 `useLayoutEffect` 钩子中：这将为您提供这些测量值的最新值，并允许您在计算测量值的同一帧中应用更改。
...
:::note
此处描述的方法适用于 React Native 提供的大多数默认组件。但是，它们 _不_ 适用于没有直接由原生视图支持的复合组件。这通常包括您在自己的应用中定义的大多数组件。
:::
