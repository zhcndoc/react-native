...
React Native 中的组件 [使用 JavaScript 设置样式](style)。颜色属性通常与 [Web 上的 CSS 工作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value) 匹配。以下是关于各平台颜色使用的通用指南：
...
:::caution
这看起来可能与 Android [Color](https://developer.android.com/reference/android/graphics/Color) 整数表示法相似，但在 Android 上值存储在 SRGB 颜色模式下 (0xaarrggbb)。
:::
