...
要安装模拟器，打开 **Xcode > 设置...（或偏好设置...）** 并选择 **平台（或组件）** 标签页。选择一个与你希望使用的 iOS 版本相对应的模拟器。

如果你使用 Xcode 14.0 或更高版本安装模拟器，打开 **Xcode > 设置 > 平台** 标签页，然后点击 "+" 图标并选择 **iOS…** 选项。
...
这是将构建基础设施与系统版本的 `node` 解耦的 **建议方法**。如果与默认值不同，你应该使用自己的路径或自己的 `node` 版本管理器自定义此变量。
