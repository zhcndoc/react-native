...
控制应用状态栏的组件。状态栏是屏幕顶部的区域，通常会显示当前时间、Wi‑Fi 和蜂窝网络信息、电池电量和/或其他状态图标。

### 与 Navigator 一起使用

可以同时挂载多个 `StatusBar` 组件。各个 `StatusBar` 组件的属性会按照挂载顺序进行合并。
...
在不适合使用组件的场景下，也提供了一个作为组件静态函数暴露的命令式 API。不过，不建议对同一个属性同时使用静态 API 和组件，因为通过静态 API 设置的任何值都会在下一次渲染时被组件设置的值覆盖。
...
状态栏属性变更之间的过渡是否应带有动画。支持 `backgroundColor`、`barStyle` 和 `hidden` 属性。
...
:::warning
由于 Android 15 引入的边到边强制要求，在 API 级别 35 中设置状态栏背景颜色已被弃用，设置后将不会生效。你可以在[这里](https://github.com/react-native-community/discussions-and-proposals/discussions/827)阅读更多关于我们的边到边建议。
:::
...
在 Android 上，这仅会对 API 版本 23 及以上产生影响。
...
状态栏是否为半透明。将 `translucent` 设为 `true` 时，应用将绘制在状态栏下方。这在使用半透明状态栏颜色时很有用。

:::warning
由于 Android 15 引入的边到边强制要求，在 API 级别 35 中将状态栏设为半透明已被弃用，设置后将不会生效。你可以在[这里](https://github.com/react-native-community/discussions-and-proposals/discussions/827)阅读更多关于我们的边到边建议。
:::
