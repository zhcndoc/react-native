...
对于不使用组件更理想的情况，还有一个作为组件上的静态函数暴露的命令式 API。然而，不建议对同一属性同时使用静态 API 和组件，因为静态 API 设置的任何值将在下一次渲染中被组件设置的值覆盖。
...
在 Android 上，这只会影响 23 及以上版本的 API。
...
:::warning
由于 Android 15 引入的边到边（edge-to-edge）强制要求，在 API 级别 35 中设置状态栏为半透明已弃用，设置它将无效。您可以在此处阅读更多关于我们的 [边到边建议](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
...
将一个 StatusBar 条目推送到栈上。返回值应在完成时传递给 `popStackEntry`。
...
:::warning[Deprecated]
状态栏网络活动指示器不再受 iOS 13 及更高版本支持。此功能将在未来版本中移除。
:::
