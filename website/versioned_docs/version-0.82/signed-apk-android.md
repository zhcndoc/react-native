...
:::note[关于使用 git 的说明]
将上述 Gradle 变量保存到 `~/.gradle/gradle.properties` 而不是 `android/gradle.properties` 中，可以避免它们被提交到 git。您可能需要先在用户主目录中创建 `~/.gradle/gradle.properties` 文件，然后才能添加这些变量。
:::

:::note[关于安全性的说明]
如果您不希望以明文形式存储密码，并且您正在使用 macOS，您也可以 [将凭据存储在 Keychain Access 应用中](https://pilloxa.gitlab.io/posts/safer-passwords-in-gradle/)。这样您就可以跳过 `~/.gradle/gradle.properties` 中最后两行。
:::
...
:::caution[重要]
如果您启用了 Proguard，请务必彻底测试您的应用。Proguard 通常需要针对您使用的每个本机库进行特定配置。请参阅 `app/proguard-rules.pro`。
:::
