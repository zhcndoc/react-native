...
4. 编写第一个 React Native 屏幕的 TypeScript 代码。
5. 使用 `RCTRootView` 将 React Native 与你的 iOS 代码集成。
6. 通过运行 bundler 并查看你的应用程序运行来测试你的集成。

## 使用社区模板

在你遵循本指南时，我们建议你使用 [React Native 社区模板](https://github.com/react-native-community/template/) 作为参考。该模板包含一个 **最小化的 iOS 应用**，并将帮助你理解如何将 React Native 集成到现有的 iOS 应用中。

## 先决条件

遵循 [设置你的开发环境](set-up-your-environment) 指南，并使用 [不使用框架的 React Native](getting-started-without-a-framework) 来配置你的开发环境，以便为 iOS 构建 React Native 应用。
本指南还假设你熟悉 iOS 开发的基础知识，例如创建 `UIViewController` 和编辑 `Podfile` 文件。

### 1. 设置目录结构

为了确保顺畅的体验，为你的集成 React Native 项目创建一个新文件夹，然后 **将你现有的 iOS 项目移动** 到 `/ios` 子文件夹。
...
这将会从社区模板复制 `package.json` <RNTemplateRepoLink href="template/package.json">文件</RNTemplateRepoLink> 到你的项目。
...
安装命令行工具。在 Xcode 菜单中选择 **设置... (或 偏好设置...)**。转到位置面板，通过在命令行工具下拉菜单中选择最新版本来安装工具。
...
Xcode 16 生成项目的方式与之前的 Xcode 版本略有不同，你需要最新的 CocoaPods 和 Xcodeproj gems 才能使其正常工作。
:::
...
:::note
记得更改 <RNTemplateRepoLink href="template/ios/Podfile#L17">此行</RNTemplateRepoLink>。
:::
...
我们将编写的第一段代码是实际用于新屏幕的 React Native 代码，该屏幕将被集成到我们的应用程序中。

### 创建 `index.js` 文件

首先，在你的 React Native 项目根目录中创建一个空的 `index.js` 文件。

`index.js` 是 React Native 应用程序的起点，并且始终是必需的。它可以是一个 `import` 属于你 React Native 组件或应用程序的其他文件的小文件，也可以包含所需的所有代码。
...
这里是作为参考的 <RNTemplateRepoLink href="template/App.tsx">社区模板文件</RNTemplateRepoLink>。
...
一旦类被初始化，你可以提供 `UIWindow` 对象来启动 React Native 视图，或者你可以要求工厂生成一个 `UIView`，你可以将其加载到任何 `UIViewController` 中。
...
我们已经有了初始的 `ViewController`，我们可以让它展示 `ReactViewController`。有几种方法可以做到这一点，具体取决于你的应用。对于此示例，我们假设你有一个按钮可以模态地展示 React Native。
...
确保禁用 Sandbox 脚本。要实现这一点，在 Xcode 中，点击你的应用，然后点击构建设置。过滤脚本并将 `User Script Sandboxing` 设置为 `NO`。此步骤需要正确切换我们随 React Native 提供的 [Hermes 引擎](https://github.com/facebook/hermes/blob/main/README.md) 的 Debug 和 Release 版本。

![禁用 Sandboxing](/docs/assets/disable-sandboxing.png)

最后，确保将 `UIViewControllerBasedStatusBarAppearance` 键添加到你的 `Info.plist` 文件中，值为 `NO`。

![禁用 UIViewControllerBasedStatusBarAppearance](/docs/assets/disable-UIViewControllerBasedStatusBarAppearance.png)

## 6. 测试你的集成

你已经完成了将 React Native 集成到应用中的所有基本步骤。现在我们将启动 [Metro 打包器](https://metrobundler.dev/) 将你的 TypeScript 应用代码构建为 bundle。Metro 的 HTTP 服务器将 bundle 从你开发环境中的 `localhost` 共享到模拟器或设备。这允许进行 [热重载](https://reactnative.dev/blog/2016/03/24/introducing-hot-reloading)。

首先，你需要在项目根目录创建一个 `metro.config.js` 文件，如下所示：
...
你可以查看社区模板文件中的 <RNTemplateRepoLink href="template/metro.config.js">`metro.config.js` 文件</RNTemplateRepoLink> 作为参考。

然后，你需要在项目根目录创建一个 `.watchmanconfig` 文件。该文件必须包含一个空 JSON 对象：

```sh
echo {} > .watchmanconfig
```

一旦配置文件就位，你就可以运行打包器。在项目根目录运行以下命令：
...
你也可以使用 Xcode 来创建发布构建！唯一额外的步骤是添加一个脚本，该脚本在应用构建时执行，以将你的 JS 和图片打包到 iOS 应用中。
...
## 7. 传递初始 props 给 React Native 视图

在某些情况下，你可能希望将一些信息从原生应用传递给 JavaScript。例如，你可能想要将当前登录用户的用户 ID 传递给 React Native，以及一个可用于从数据库检索信息的 token。

这是通过使用 `RCTReactNativeFactory` 类的 `view(withModuleName:initialProperty)` 重载的 `initialProperties` 参数来实现的。以下步骤向你展示如何操作。
...
### 更新原生代码以传递初始属性给 JavaScript。

<Tabs groupId="ios-language" queryString defaultValue={constants.defaultAppleLanguage} values={constants.appleLanguages}>
<TabItem value="objc">

修改 `ReactViewController.mm` 以传递初始属性给 JavaScript。
...
修改 `ReactViewController.swift` 以传递初始属性给 React Native 视图。
...
3. 再次运行你的应用。在呈现 `ReactViewController` 后，你应该看到以下屏幕：
