...
用于驱动 2D 动画（例如平移手势）的 2D 值。API 与普通的 [`Animated.Value`](animatedvalue) 几乎完全相同，但是是多路复用的。底层包含两个常规的 `Animated.Value`。
...
设置一个偏移量，该偏移量会应用于任何设置的值之上，无论是通过 `setValue`、动画还是 `Animated.event` 设置的。对于补偿平移手势的开始位置等情况很有用。
