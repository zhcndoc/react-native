...
- 为了限制内存并启用平滑滚动，内容在屏幕外异步渲染。这意味着有可能滚动速度快于填充率，并暂时看到空白内容。这是一个可以根据每个应用的需求进行调整的权衡，我们正在幕后改进它。
- 默认情况下，列表查找每个项上的 `key` 属性并将其用作 React 键。或者，你可以提供自定义的 `keyExtractor` 属性。
...
每个部分中每个项的默认渲染器。可以按部分覆盖。应返回一个 React 元素。
...
- 'item' (object) - 此部分的 `data` 键中指定的项对象
- 'index' (number) - 项在部分内的索引。
- 'section' (object) - `sections` 中指定的完整部分对象。
...
一个用于告诉列表重新渲染的标记属性（因为它实现了 `PureComponent`）。如果你的任何 `renderItem`、Header、Footer 等函数依赖于 `data` 属性之外的任何内容，将其放在这里并将其视为不可变的。
...
在初始批次中渲染多少项。这应该足以填充屏幕，但不要太多。注意这些项永远不会作为窗口化渲染的一部分被卸载，以提高滚动到顶部操作的感知性能。
...
用于提取给定项在指定索引处的唯一键。键用于缓存并作为 React 键来跟踪项重新排序。默认提取器检查 `item.key`，然后 `item.id`，然后回退到使用索引，就像 React 所做的那样。注意这为每个项设置键，但每个整体部分仍然需要自己的键。
...
渲染在每个部分的顶部。默认情况下，这些在 iOS 上固定在 `ScrollView` 的顶部。参见 `stickySectionHeadersEnabled`。
...
渲染在每个部分的顶部和底部（注意这与 `ItemSeparatorComponent` 不同，后者仅渲染在项之间）。这些旨在将部分与上方和下方的标题分开，通常具有与 `ItemSeparatorComponent` 相同的高亮响应。还接收 `highlighted`、`[leading/trailing][Item/Section]` 以及来自 `separators.updateProps` 的任何自定义属性。
...
使部分标题固定在屏幕顶部，直到下一个标题将其推离。仅在 iOS 上默认启用，因为那是那里的平台标准。
...
通知列表已发生交互，这将触发可见性计算，例如如果 `waitForInteractions` 为 true 且用户尚未滚动。这通常由点击项目或导航操作调用。
...
滚动到指定 `sectionIndex` 和 `itemIndex`（在分区内）的项目，并将其定位在可见区域内，其中 `viewPosition` 设置为 `0` 将其置于顶部（可能会被粘性头部覆盖），`1` 置于底部，`0.5` 居中。
...
- 'itemIndex' (number) - 要滚动到的项目在分区内的索引。必需。
- 'sectionIndex' (number) - 包含要滚动到的项目的分区索引。必需。
- 'viewOffset' (number) - 偏移最终目标位置的固定像素数，例如用于补偿粘性头部。
- 'viewPosition' (number) - 值 `0` 将索引指定的项目置于顶部，`1` 置于底部，`0.5` 居中。

## 类型定义

### Section

一个对象，用于标识要渲染的给定分区的数据。
...
| 名称                                                      | 类型               | 描述                                                                                                                                                         |
| --------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| data <div className="label basic required">必需</div> | array              | 用于渲染此分区中项目的数据。对象数组，类似于 [`FlatList` 的 data 属性](flatlist#required-data)。                                         |
| key                                                       | string             | 可选的 key，用于跟踪分区重新排序。如果您不打算重新排序分区，默认将使用数组索引。                              |
| renderItem                                                | function           | 可选地为该分区定义任意项目渲染器，覆盖列表的默认 [`renderItem`](sectionlist#renderitem)。                          |
| ItemSeparatorComponent                                    | component, element | 可选地为该分区定义任意项目分隔符，覆盖列表的默认 [`ItemSeparatorComponent`](sectionlist#itemseparatorcomponent)。 |
| keyExtractor                                              | function           | 可选地为该分区定义任意 key 提取器，覆盖默认的 [`keyExtractor`](sectionlist#keyextractor)。                                   |
