...
更便捷的 [`<FlatList>`](flatlist.md) 和 [`<SectionList>`](sectionlist.md) 组件的基础实现，这两个组件的文档也更完善。一般来说，只有当你需要比 [`FlatList`](flatlist.md) 提供的更灵活的灵活性时才应该使用这个，例如用于不可变数据而不是普通数组。
...
提供对底层 scroll responder 的句柄。注意 `this._scrollRef` 可能不是 `ScrollView`，所以我们在调用之前需要检查它是否响应 `getScrollResponder`。
