...
`navigationBarTranslucent` 属性决定模态框是否应显示在系统导航栏下方。不过，`statusBarTranslucent` 也需要设为 `true`，才能使导航栏透明。
...
当模态框显示期间方向发生变化时，会调用 `onOrientationChange` 回调。传入的方向仅为 `portrait` 或 `landscape`。无论当前方向如何，在初始渲染时也会调用此回调。
...
挂载时会分配给一个 [元素节点](element-nodes) 的 ref setter。
...
在 iOS 上，当使用 `presentationStyle` 为 `pageSheet` 或 `formSheet` 的拖拽手势关闭 Modal 时，会调用此回调。启用 `allowSwipeDismissal` 后，关闭模态框之后也会调用此回调。
...
`presentationStyle` 属性控制模态框的显示方式（通常用于 iPad 或 Plus 尺寸 iPhone 等较大设备）。详情请参阅 https://developer.apple.com/reference/uikit/uimodalpresentationstyle。

可选值：

- `fullScreen` 完全覆盖屏幕
- `pageSheet` 居中覆盖竖屏宽度视图（仅在较大设备上）
- `formSheet` 居中覆盖窄宽度视图（仅在较大设备上）
...
`supportedOrientations` 属性允许模态框旋转到所指定的任意方向。在 iOS 上，模态框仍会受应用 Info.plist 中 UISupportedInterfaceOrientations 字段的限制。
...
`transparent` 属性决定模态框是否会填充整个视图。将其设为 `true` 时，模态框会以透明背景渲染。
