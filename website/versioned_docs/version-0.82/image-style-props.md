...
### `overlayColor` <div className="label android">安卓</div>

当图像具有圆角时，指定 overlayColor 会导致角落的剩余空间填充纯色。这在某些 Android 实现不支持圆角的情况下很有用：
...
- `center`: 沿两个维度将图像居中于视图中。如果图像大于视图，则均匀缩小以便包含在视图中。
