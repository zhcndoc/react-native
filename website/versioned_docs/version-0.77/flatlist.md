...
- 通过传递 `extraData={selectedId}` 给 `FlatList`，我们确保当状态改变时 `FlatList` 本身会重新渲染。如果不设置此属性，`FlatList` 将不知道它需要重新渲染任何项目，因为它是 `PureComponent` 并且属性比较不会显示任何变化。
...
这是 [`<VirtualizedList>`](virtualizedlist.md) 的便捷包装器，因此继承了其属性（以及 [`<ScrollView>`](scrollview.md) 的属性），除非此处明确列出，同时具有以下注意事项：
...
- 为了限制内存并实现平滑滚动，内容是在屏幕外异步渲染的。这意味着有可能滚动速度快于填充率，并暂时看到空白内容。这是一种可以根据每个应用程序的需求进行调整的权衡，我们正在幕后改进它。
- 默认情况下，列表会在每个项目上查找 `key` 属性并将其用作 React 键。或者，你可以提供自定义的 `keyExtractor` 属性。
...
提供额外的元数据，如 `index`（如果你需要的话），以及更通用的 `separators.updateProps` 函数，它允许你设置任何想要更改的属性，以改变前导分隔符或尾随分隔符的渲染，以防更常见的 `highlight` 和 `unhighlight`（它们设置 `highlighted: boolean` 属性）不足以满足你的用例。
...
渲染在每两个项目之间，但不在顶部或底部。默认情况下，提供 `highlighted` 和 `leadingItem` 属性。`renderItem` 提供 `separators.highlight`/`unhighlight` 将更新 `highlighted` 属性，但你也可以使用 `separators.updateProps` 添加自定义属性。可以是 React 组件（例如 `SomeComponent`），或 React 元素（例如 `<SomeComponent />`）。
...
`getItemLayout` 是一个可选的优化，如果你提前知道项目的大小（高度或宽度），它可以跳过动态内容的测量。如果你有固定大小的项目，`getItemLayout` 非常有效，例如：
...
添加 `getItemLayout` 可以极大地提高几百个项目列表的性能。如果你指定了 `ItemSeparatorComponent`，记得在你的偏移计算中包含分隔符长度（高度或宽度）。
...
初始批次中要渲染多少个项目。这应该足以填充屏幕，但不要太多。注意，作为窗口化渲染的一部分，这些项目永远不会被卸载，以提高滚动到顶部操作的感知性能。
...
用于提取给定项目在指定索引处的唯一键。键用于缓存并作为 react 键来跟踪项目重新排序。默认提取器检查 `item.key`，然后 `item.id`，然后回退到使用索引，就像 React 所做的那样。
...
项目在触发可见性回调之前必须物理可见的最短时间（毫秒）。较高的数字意味着不停止地滚动内容不会将内容标记为可见。
