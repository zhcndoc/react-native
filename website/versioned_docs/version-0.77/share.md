...
在 iOS 上，返回一个 Promise，该 Promise 将以一个包含 `action` 和 `activityType` 的对象进行调用。如果用户取消了对话框，Promise 仍会被解决，action 为 `Share.dismissedAction`，所有其他键为 undefined。请注意，某些分享选项可能不会出现在 iOS 模拟器上或无法正常工作。

在 Android 上，返回一个 Promise，该 Promise 将始终被解决，action 为 `Share.sharedAction`。
