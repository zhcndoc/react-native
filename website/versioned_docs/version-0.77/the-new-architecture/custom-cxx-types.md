...
如果你想在应用或库中添加对新自定义类型的支持，则需要提供必要的 `bridging` 头文件。
...
上述方法可以推广到任何类型的类型。对于结构化类型，React Native 提供了一些辅助函数，使它们更容易从 JS 桥接到 C++，反之亦然。
...
第一个类型是 `Address` 的定义。第二个类型包含将所有基础设施从 JS 桥接到 C++ 所需的内容，反之亦然。我们需要添加的唯一额外步骤是定义扩展 `NativeSampleModuleAddressBridging` 类型的 `Bridging` 结构体。
...
一旦我们手动解析了对象，就可以实现我们需要的逻辑。

:::note
如果你想了解更多关于 `JSI` 及其工作原理的信息，请查看 App.JS 2024 的这场 [精彩演讲](https://youtu.be/oLmGInjKU2U?feature=shared)
:::
