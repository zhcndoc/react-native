...
React Native 拥有丰富的生态系统库来解决常见问题。我们在 [reactnative.directory](https://reactnative.directory) 网站收集 React Native 库，这是每位 React Native 开发者都值得收藏的好资源。
...
你可以使用 [`create-react-native-library`](https://callstack.github.io/react-native-builder-bob/create) 工具来创建一个新库。此工具会设置一个包含所有所需样板代码的新库：所有配置文件以及各种平台所需的所有文件。它还带有一个不错的交互式菜单来指导你完成库的创建。
...
指南的其余部分假设你在你的应用中有一个本地 Turbo Native Module，它是按照网站其他指南中显示的准则创建的：特定平台的 Turbo Native Modules，或 [跨平台 Turbo Native Modules](./pure-cxx-modules)。但它也适用于组件和旧架构模块及组件。你需要调整需要复制和更新的文件。
...
在某些场景下，你可能想要将库作为应用的本地模块复用，而不发布到 NPM。

在这种情况下，你可能会遇到库作为应用的同级目录存在的情况。
...
1. 通过导航到 `App` 文件夹并运行 `yarn add ../Library` 将库添加到你的应用。
2. 仅针对 iOS，导航到 `App/ios` 文件夹并运行 `bundle exec pod install` 安装依赖。
...
:::note
仅针对 iOS，每当你安装带有原生代码的新模块时，你必须重新安装 CocoaPods，运行 `bundle exec pod install`（推荐）或 `pod install`（如果你不使用 Ruby 的 Bundler，不推荐）。
:::
