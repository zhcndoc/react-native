...
目前，从头创建 React Native 平台的过程文档还不够完善——即将到来的架构重构（[Fabric](/blog/2018/06/14/state-of-react-native-2018)）的目标之一就是让维护平台变得更容易。
...
`"providesModuleNodeModules"` 是一个模块数组，它们将被添加到 Haste 模块搜索路径中，而 `"platforms"` 是一个平台后缀数组，它们将被添加为有效平台。
