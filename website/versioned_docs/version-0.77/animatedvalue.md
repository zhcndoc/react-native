...
用于驱动动画的标准值。一个 `Animated.Value` 可以以同步方式驱动多个属性，但一次只能由一种机制驱动。使用新机制（例如启动新动画，或调用 `setValue`）将停止任何之前的机制。
...
设置一个偏移量，该偏移量会应用于任何设置的值之上，无论是通过 `setValue`、动画还是 `Animated.event`。适用于补偿平移手势开始等情况。
...
为值添加一个异步监听器，以便您可以观察动画的更新。这很有用，因为无法同步读取该值，因为它可能是由原生驱动的。
...
停止任何正在运行的动画或追踪。`callback` 在停止动画后使用最终值调用，这对于更新状态以匹配动画位置与布局很有用。
