...
您可以通过提供原生颜色对应的字符串值，使用 `PlatformColor` 函数来访问目标平台上的原生颜色。您将一个字符串传递给 `PlatformColor` 函数，如果该平台上存在该颜色，它将返回相应的原生颜色，您可以将其应用于应用程序的任何部分。
...
> 如果您熟悉设计系统，另一种思考方式是，`PlatformColor` 让您可以利用本地设计系统的颜色令牌，从而使您的应用完美融合！
...
> **注意：** 您可以在 [PlatformColorExample.js](https://github.com/facebook/react-native/blob/main/packages/rn-tester/js/examples/PlatformColor/PlatformColorExample.js) 中找到一个完整的示例，演示 `PlatformColor` 的正确预期用法。
