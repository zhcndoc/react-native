...
- 如果你指定一个按钮，它将是 '正面' 按钮（例如 'OK'）
- 两个按钮意味着 '负面'、'正面'（例如 'Cancel'、'OK'）
- 三个按钮意味着 '中性'、'负面'、'正面'（例如 'Later'、'Cancel'、'OK'）

Android 上的警告框可以通过点击警告框外部来关闭。默认情况下禁用，可以通过提供可选的 [AlertOptions](alert#alertoptions) 参数并将 cancelable 属性设置为 `true` 来启用，即<br/>`{cancelable: true}`。
