...
> **警告：实验性。** 破坏性变更可能会频繁发生，且不会可靠地宣布。整个功能可能会被删除，谁知道呢？使用风险自负。
...
设置一个函数用于预处理样式属性值。这在内部用于处理颜色和变换值。除非你真正知道自己在做什么并且已经穷尽其他选项，否则不应使用此方法。
...
一种非常常见的模式是使用绝对定位和零定位创建覆盖层（`position: 'absolute', left: 0, right: 0, top: 0, bottom: 0`），因此 `absoluteFill` 可用于方便起见并减少这些重复样式的复制。如果你愿意，`absoluteFill` 可用于在 StyleSheet 中创建自定义条目，例如：
...
此常数始终将是像素的整数（因此由它定义的线条看起来可能很清晰），并将尝试匹配底层平台上细线的标准宽度。但是，你不应该依赖它是一个恒定大小，因为在不同的平台和屏幕密度上，其值的计算方式可能不同。

如果你的模拟器被缩小，具有发丝宽度的线条可能不可见。
