...
在实际应用中，你可能不会使用计时器来设置 state。当你从服务器获得新数据或从用户输入获得数据时，可能会设置 state。你也可以使用状态容器如 [Redux](https://redux.js.org/) 或 [MobX](https://mobx.js.org/) 来控制数据流。在这种情况下，你将使用 Redux 或 MobX 来修改状态，而不是直接调用 `setState`。

当调用 setState 时，BlinkApp 将重新渲染其组件。通过在 Timer 内调用 setState，组件将在每次 Timer 滴答时重新渲染。
