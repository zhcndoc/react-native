...
在手机上，按下按钮时很难做到非常精确。确保所有交互元素的大小为 44x44 或更大。实现这一点的一种方法是为元素留出足够的空间，`padding`、`minWidth` 和 `minHeight` 样式值对此很有用。或者，您可以使用 [`hitSlop` 属性](touchablewithoutfeedback.md#hitslop) 来增加交互区域而不影响布局。这是一个演示：
...
Android API 21+ 使用 Material Design 涟漪效果，在用户触摸屏幕上的可交互区域时提供反馈。React Native 通过 [`TouchableNativeFeedback` 组件](touchablenativefeedback.md) 暴露了这一功能。使用这种触摸效果而不是透明度或高亮效果，通常会让您的应用在平台上感觉更加合适。也就是说，使用时需要小心，因为它在 iOS 或 Android API < 21 上不起作用，因此您需要在 iOS 上回退到使用其他 Touchable 组件之一。您可以使用像 [react-native-platform-touchable](https://github.com/react-community/react-native-platform-touchable) 这样的库来为您处理平台差异。
...
[Material Design](https://material.io/) 和 [人机界面指南](https://developer.apple.com/design/human-interface-guidelines) 是了解更多关于移动平台设计的绝佳资源。
