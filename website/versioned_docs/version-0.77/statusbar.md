...
对于不使用组件更理想的情况，还有一个作为组件上的静态函数暴露的命令式 API。但是不建议对同一个属性同时使用静态 API 和组件，因为静态 API 设置的任何值将在下一次渲染中被组件设置的值覆盖。
...
:::warning
由于 Android 15 引入的边到边（edge-to-edge）强制要求，在 API 级别 35 中设置状态栏背景颜色已弃用，设置它将无效。您可以在 [这里](https://github.com/react-native-community/discussions-and-proposals/discussions/827) 阅读更多关于我们要边到边建议的信息。
:::
...
在 Android 上，这只会影响 API 版本 23 及以上。
...
:::warning
由于 Android 15 引入的边到边（edge-to-edge）强制要求，在 API 级别 35 中设置状态栏为半透明已弃用，设置它将无效。您可以在 [这里](https://github.com/react-native-community/discussions-and-proposals/discussions/827) 阅读更多关于我们要边到边建议的信息。
:::
...
将 StatusBar 条目推送到堆栈上。返回值应在完成时传递给 `popStackEntry`。

**参数:**

| 名称                                                       | 类型 | 描述                                                      |
| ---------------------------------------------------------- | ---- | ---------------------------------------------------------------- |
| props <div className="label basic required">必填</div> | any  | 包含要在堆栈条目中使用的 StatusBar 属性的对象。 |
...
| props <div className="label basic required">必填</div> | any  | 包含要在替换堆栈条目中使用的 StatusBar 属性的对象。 |
