...
React Native 中的组件通过 [JavaScript 设置样式](style)。颜色属性通常与 [Web 上的 CSS 工作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value) 相匹配。各平台颜色使用的通用指南如下：
...
- [PlatformColor](platformcolor) 让您能够引用平台的颜色系统。
...
:::caution
这看起来可能类似于 Android [Color](https://developer.android.com/reference/android/graphics/Color) 整数的表示形式，但在 Android 上，值存储在 SRGB 颜色模式 (0xaarrggbb) 中。
:::
