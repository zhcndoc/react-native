...
如果您有一些 [框架](/architecture/glossary#react-native-framework) 无法很好地满足的限制条件，或者您更喜欢编写自己的框架，您可以不使用框架来创建 React Native 应用。

为此，您首先需要 [设置您的环境](set-up-your-environment)。设置完成后，请继续以下步骤来创建应用并开始开发。

### 步骤 1：创建新应用

<RemoveGlobalCLI />

您可以使用 [React Native Community CLI](https://github.com/react-native-community/cli) 生成一个新项目。让我们创建一个名为 "AwesomeProject" 的新 React Native 项目：
...
您也可以使用 `--template` 参数通过自定义 React Native 模板启动项目，在此处 [阅读更多](https://github.com/react-native-community/cli/blob/main/docs/init.md#initializing-project-with-custom-template)。
...
:::note
如果您熟悉 Web 开发，Metro 类似于 Vite 和 webpack 等打包工具，但它是为 React Native 端到端设计的。例如，Metro 使用 [Babel](https://babel.dev/) 将 JSX 等语法转换为可执行的 JavaScript。
:::
...
- 如果您好奇想了解更多关于 React Native 的信息，请查看 [React Native 介绍](getting-started)。
