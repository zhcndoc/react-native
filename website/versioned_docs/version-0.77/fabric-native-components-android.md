...
该类定义了三个 Android 构造函数，但将其实际实现延迟到私有 `configureComponent` 函数。此函数负责初始化所有组件特定的属性：在这种情况下，你正在设置 `WebView` 的布局，并且你正在定义用于自定义 `WebView` 行为的 `WebClient`。在此代码中，`ReactWebView` 通过实现 `WebClient` 的 `onPageFinished` 方法，在页面完成加载时发出事件。
...
- `OnScriptLoadedEventResult`，带有 `OnScriptLoaded` 事件的可能结果。
...
最后，如果组件可以发出事件，你需要通过重写冒泡事件的 `getExportedCustomBubblingEventTypeConstants` 或直接事件的 `getExportedCustomDirectEventTypeConstants` 来映射事件名称。
...
最后，你需要在应用程序中注册 `ReactWebViewPackage`。我们通过修改 `MainApplication` 文件来实现这一点，将 `ReactWebViewPackage` 添加到由 `getPackages` 函数返回的包列表中。
