...
你可以使用 `--simulator` 标志后跟设备名称字符串来指定模拟器应运行的设备。默认值是 `"iPhone 14"`。如果你希望在 iPhone SE (第 3 代) 上运行应用，请运行以下命令：
...
你可以指定 `xcrun simctl list devices` 命令返回的设备 UDID。例如，要用 UDID `AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA` 运行应用，请运行以下命令：
