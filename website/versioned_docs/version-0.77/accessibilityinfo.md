...
有时候知道设备是否当前激活了屏幕阅读器是很有用的。`AccessibilityInfo` API 就是为此设计的。你可以使用它来查询屏幕阅读器的当前状态，以及注册监听器以便在屏幕阅读器状态变化时收到通知。
...
| `announcementFinished`<br/><div className="label two-lines ios">iOS</div>                | 当屏幕阅读器完成朗读时触发。事件处理器的参数是一个包含以下键的对象：<ul><li>`announcement`：屏幕阅读器朗读的字符串。</li><li>`success`：一个布尔值，指示朗读是否成功。</li></ul> |
| `boldTextChanged`<br/><div className="label two-lines ios">iOS</div>                     | 当粗文本切换状态变化时触发。事件处理器的参数是一个布尔值。当启用粗文本时布尔值为 `true`，否则为 `false`。                                                                                                                             |
...
发布一个字符串供屏幕阅读器朗读，带有修改选项。默认情况下，朗读会中断任何现有的语音，但在 iOS 上，可以通过在选项对象中将 `queue` 设置为 `true` 将其排队在现有语音之后。
...
| options <div className="label basic required">必需</div>      | object | `queue` - 将朗读排队在现有语音之后 <div className="label ios">iOS</div> |
