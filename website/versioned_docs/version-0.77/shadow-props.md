...
- `dropShadow` 是一个 alpha 遮罩，所以只有具有正 alpha 值的像素才会“投射”阴影。`boxShadow` 将围绕元素的边框盒投射阴影，无论其内容如何（除非它是 inset）。
- `dropShadow` 仅在安卓上可用，`boxShadow` 在 iOS 和安卓上均可用。
...
此属性仅在安卓 API 28 及以上版本有效。对于较低安卓 API 的类似功能，请使用 [`elevation` 属性](view-style-props#elevation-android)。
