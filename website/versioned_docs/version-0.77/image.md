...
一个用于显示不同类型图片的 React 组件，包括网络图片、静态资源、临时本地图片以及来自本地磁盘（如相机胶卷）的图片。

此示例展示了从本地存储获取和显示图片，以及从网络甚至从 `'data:'` URI 方案提供的数据中显示图片。

> 请注意，对于网络和数据图片，你需要手动指定图片的尺寸！

## 示例

```SnackPlayer name=图片%20示例
...
```groovy
dependencies {
  // 如果你的应用支持冰激凌三明治 (Ice Cream Sandwich) 之前的 Android 版本 (API 级别 14)
...
一个字符串，定义图片的替代文本描述，当用户与图片交互时将由屏幕阅读器读取。使用此项将自动标记该元素为可访问。
...
当图片调整大小时，由 `capInsets` 指定的大小的角将保持固定大小，但图片的中心内容和边框将被拉伸。这对于创建可调整大小的圆角按钮、阴影和其他可调整大小的资源很有用。更多信息请参阅 [官方 Apple 文档](https://developer.apple.com/library/ios/documentation/UIKit/Reference/UIImage_Class/index.html#//apple_ref/occ/instm/UIImage/resizableImageWithCapInsets)。
...
一个关键字字符串，指定获取图片资源时使用的 CORS 模式。其工作方式类似于 HTML 中的 crossorigin 属性。

- `anonymous`：图片请求中不交换用户凭证。
- `use-credentials`：在图片请求中将 `Access-Control-Allow-Credentials` 头值设置为 `true`。
...
当图片的部分加载完成时调用。“部分加载”的定义取决于加载器，但这旨在用于渐进式 JPEG 加载。
...
一个字符串，指示获取资源时使用哪个引用者。在图片请求中设置 `Referrer-Policy` 头的值。工作方式类似于 HTML 中的 `referrerpolicy` 属性。
...
当图片尺寸与图片视图尺寸不同时，应使用何种机制来调整图片大小。默认为 `auto`。

- `auto`：使用启发式方法在 `resize` 和 `scale` 之间选择。

- `resize`：一种软件操作，在解码之前更改内存中的编码图片。当图片远大于视图时，应使用此方法而不是 `scale`。

- `scale`：图片被向下或向上缩放绘制。与 `resize` 相比，`scale` 更快（通常硬件加速）并产生更高质量的图片。如果图片小于视图，应使用此方法。如果图片略大于视图，也应使用此方法。

- `none`：不执行采样，图片以其完整分辨率显示。这应该只在少数情况下使用，因为它被认为是不安全的，因为当尝试渲染消耗过多内存的图片时，Android 将抛出运行时异常。
...
确定当帧与原始图片尺寸不匹配时如何调整图片大小。默认为 `cover`。
...
- `center`：沿两个维度将图片居中在视图中。如果图片大于视图，统一缩小以便包含在视图中。
...
当 `resizeMethod` 设置为 `resize` 时，目标尺寸乘以此值。`scale` 方法用于执行剩余的调整大小。默认值 `1.0` 表示位图大小设计为适合目标尺寸。大于 `1.0` 的乘数将设置比目标尺寸更大的调整大小选项，并且生成的位图将从硬件大小缩小。默认为 `1.0`。

此属性在目标尺寸相当小且源图片显著较大的情况下最有用。`resize` 调整大小方法执行下采样，源和目标图片尺寸之间会丢失大量图片质量，通常导致图片模糊。通过使用乘数，解码后的图片略大于目标尺寸但小于源图片（如果源图片足够大）。这允许混叠伪影通过缩放操作在乘数图片上产生伪质量。
...
此属性还可以包含多个远程 URL，与它们的宽度和高度一起指定，可能带有缩放/其他 URI 参数。原生端将根据图片容器的测量大小选择最佳的 `uri` 来显示。可以添加 `cache` 属性来控制网络请求如何与本地缓存交互。（更多信息见 [图片的缓存控制](images#cache-control)）。

当前支持的格式有 `png`, `jpg`, `jpeg`, `bmp`, `gif`, `webp`, `psd`（仅 iOS）。此外，iOS 支持几种 RAW 图片格式。请参阅 Apple 的文档以获取当前支持的相机型号列表（对于 iOS 12，见 https://support.apple.com/en-ca/HT208967）。

请注意，`webp` 格式仅在 **仅** 与 JavaScript 代码捆绑时在 iOS 上受支持。
...
一个字符串，表示逗号分隔的可能候选图片源列表。每个图片源包含一个图片 URL 和一个像素密度描述符。如果未指定描述符，则默认为 `1x` 描述符。
...
获取显示前图像的宽度和高度（像素）。如果找不到图像或下载失败，此方法可能会失败。

为了获取图像尺寸，图像可能需要先加载或下载，之后会被缓存。这意味着原则上你可以使用此方法来预加载图像，但它并未针对该目的进行优化，并且将来可能会以实现不完全加载/下载图像数据的方式实施。正确的、受支持的预加载图像方式将作为单独的 API 提供。
...
获取显示前图像的宽度和高度（像素），并提供为请求设置头信息的能力。如果找不到图像或下载失败，此方法可能会失败。它也不适用于静态图像资源。
...
预取远程图像以供后续使用，将其下载到磁盘缓存。返回一个解析为布尔值的 promise。
...
执行缓存查询。返回一个 promise，解析为从 URL 到缓存状态的映射，例如 "disk"、"memory" 或 "disk/memory"。如果请求的 URL 不在映射中，意味着它不在缓存中。
...
| <div className="wideColumn">名称</div>                      | 类型                                     | 描述                                                                  |
| ----------------------------------------------------------- | ---------------------------------------- | ---------------------------------------------------------------------------- |
| source <div className="label basic required">必需</div> | [ImageSource](image#imagesource), number | 数字（`require('./foo.png')` 返回的不透明类型）或 ImageSource。 |
...
- `force-cache`: 现有缓存数据将用于满足请求，无论其年龄或过期日期如何。如果缓存中没有对应请求的现有数据，则从原始源加载数据。
- `only-if-cached`: 现有缓存数据将用于满足请求，无论其年龄或过期日期如何。如果缓存中没有对应 URL 加载请求的现有数据，则不会尝试从原始源加载数据，并且加载被视为失败。
...
| width                                      | number                                     | 如果在构建时已知，可以指定，此时该值将用于设置默认 `<Image/>` 组件维度。                                                     |
| height                                     | number                                     | 如果在构建时已知，可以指定，此时该值将用于设置默认 `<Image/>` 组件维度。                                                     |
| scale                                      | number                                     | 用于指示图像的缩放因子。如果未指定，默认为 `1.0`，意味着一个图像像素等于一个显示点 / DIP。                                   |
| bundle<div className="label ios">iOS</div> | string                                     | 图像所在的 iOS 资产 bundle。如果未设置，默认为 `[NSBundle mainBundle]`。                                                                        |
| method                                     | string                                     | 要使用的 HTTP 方法。如果未指定，默认为 `'GET'`。                                                                                                                        |
| headers                                    | object                                     | 一个对象，表示随远程图像请求一起发送的 HTTP 头。                                                                                           |
| body                                       | string                                     | 随请求发送的 HTTP 主体。这必须是有效的 UTF-8 字符串，并将按指定确切发送，不应用额外编码（例如 URL 转义或 base64）。 |
| cache<div className="label ios">iOS</div>  | [ImageCacheEnum](image#imagecacheenum-ios) | 确定请求如何处理潜在缓存的响应。                                                                                                                    |

**如果传递数字：**

- `number` - 不透明类型，由类似 `require('./image.jpg')` 返回。
