...
Android 和 iOS 都允许你通过用特定格式（如粗体或彩色文本）注解字符串的范围来显示格式化文本（iOS 上为 `NSAttributedString`，Android 上为 `SpannableString`）。实际上，这非常繁琐。对于 React Native，我们决定为此使用 Web 范式，你可以嵌套文本来达到相同的效果。
...
幕后，React Native 将其转换为扁平的 `NSAttributedString` 或 `SpannableString`，其中包含以下信息：
...
你还失去了为整个子树设置默认字体的能力。同时，`fontFamily` 只接受单个字体名称，这与 CSS 中的 `font-family` 不同。在整个应用程序中使用一致字体和大小的推荐方法是创建一个包含它们的组件 `MyAppText`，并在整个应用程序中使用此组件。你也可以使用此组件创建更具体的组件，如 `MyAppHeaderText` 用于其他类型的文本。
...
假设 `MyAppText` 是一个只将其子项渲染到带有样式的 `Text` 组件中的组件，那么 `MyAppHeaderText` 可以定义如下：
...
以这种方式组合 `MyAppText` 确保我们从顶层组件获取样式，但使我们在特定用例中添加/覆盖它们的能力。
...
覆盖用户与元素交互时屏幕阅读器读取的文本。默认情况下，标签是通过遍历所有子项并累积所有以空格分隔的 `Text` 节点构建的。
...
在 iOS 上，这些角色映射到相应的无障碍特性。图像按钮的功能与将特性设置为“image”和“button”相同。参见 [无障碍指南](accessibility.md#accessibilitytraits-ios) 获取更多信息。
...
参见 [无障碍指南](accessibility#accessible-ios-android) 获取更多信息。
...
要在此元素上应用的 [动态类型](https://developer.apple.com/documentation/uikit/uifont/scaling_fonts_automatically) 梯度。
...
- `head` - 行的显示方式使得末尾适合容器，行开头缺失的文本由省略号字形指示。例如，"...wxyz"
- `middle` - 行的显示方式使得开头和末尾适合容器，中间缺失的文本由省略号字形指示。"ab...yz"
- `tail` - 行的显示方式使得开头适合容器，行末尾缺失的文本由省略号字形指示。例如，"abcd..."
- `clip` - 行不会绘制在文本容器边缘之外。
...
用于在计算文本布局（包括换行）后用省略号截断文本，使得总行数不超过此数字。将此属性设置为 `0` 将导致取消设置此值，这意味着不会应用行限制。
...
此视图想要“主张”触摸响应吗？当 `View` 不是响应者时，每次触摸移动都会调用此函数。
...
当触摸接触时立即调用，在 `onPressOut` 和 `onPress` 之前。
...
在 Android 上，从此回调返回 true 以防止任何其他原生组件在此响应者终止之前成为响应者。
...
响应者已从 `View` 被接管。可能在调用 `onResponderTerminationRequest` 后被其他视图接管，或者可能被操作系统未经询问而接管（例如，在 iOS 上与控制中心/通知中心一起发生）。
...
其他 `View` 想要成为响应者并要求此 `View` 释放其响应者。返回 `true` 允许其释放。
...
当 `true` 时，文本被按下时不会进行视觉更改。默认情况下，按下时灰色椭圆高亮显示文本。
...
| xHeight   | number | 否       | 基线与行中线之间的距离（主体大小）。 |
