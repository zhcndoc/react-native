...
:::info
只有当父组件的尺寸大于 `0` 时，组件才能扩展以填充可用空间。如果父组件既没有固定的 `width` 和 `height` 也没有 `flex`，父组件的尺寸将为 `0`，并且 `flex` 子组件将不可见。
:::
...
如果你想填充屏幕的某一部分，但 _不_ 想使用 `flex` 布局，你 _可以_ 在组件的样式中使用 **百分比值**。与弹性尺寸类似，百分比尺寸需要父组件具有定义的尺寸。
