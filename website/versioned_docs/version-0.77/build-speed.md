...
如果你频繁进行原生构建（C++ 或 Objective-C），使用 **编译器缓存** 可能会受益。
...
Ccache 可在大多数操作系统的包管理器中找到。在 macOS 上，我们可以使用 `brew install ccache` 安装 ccache。
...
1. 在 CI 上，我们建议进行完全干净构建，以避免缓存污染问题。如果你遵循上一段提到的方法，你应该能够并行化 4 个不同 ABI 的原生构建，并且很可能不需要在 CI 上使用 `ccache`。
