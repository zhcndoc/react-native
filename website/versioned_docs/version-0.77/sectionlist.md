...
这是 [`<VirtualizedList>`](virtualizedlist.md) 的便捷包装器，因此继承了其属性（以及 [`<ScrollView>`](scrollview.md) 的属性），除非这里明确列出，同时需要注意以下事项：
...
- 为了限制内存并实现平滑滚动，内容是在屏幕外异步渲染的。这意味着有可能滚动速度快于填充速度，暂时看到空白内容。这是一个可以根据每个应用的需求进行调整的权衡，我们正在幕后改进它。
...
- 'item' (object) - 此分区的 `data` 键中指定的项对象
...
用于为指定索引处的给定项提取唯一键。键用于缓存并作为 React 键来跟踪项重新排序。默认提取器检查 `item.key`，然后 `item.id`，然后像 React 一样回退到使用索引。注意，这为每个项设置键，但每个整体分区仍然需要自己的键。
...
使分区头粘滞在屏幕顶部，直到下一个分区头将其挤走。默认仅在 iOS 上启用，因为那是该平台的标准。
...
告诉列表发生了交互，这应该触发可见性计算，例如如果 `waitForInteractions` 为 true 且用户未滚动。这通常由项上的点击或导航操作调用。
...
- 'itemIndex' (number) - 要滚动到的项在分区内的索引。必需。
- 'sectionIndex' (number) - 包含要滚动到的项的分区索引。必需。
- 'viewOffset' (number) - 偏移最终目标位置的固定像素数，例如补偿粘性头。
...
| 名称                                                      | 类型               | 描述                                                                                                                                                         |
| --------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| data <div className="label basic required">必需</div> | array              | 用于渲染该区段中项的数据。对象数组，很像 [`FlatList` 的 data 属性](flatlist#required-data)。                                         |
| key                                                       | string             | 可选的 key，用于跟踪区段的重新排序。如果您不打算重新排序区段，默认将使用数组索引。                              |
| renderItem                                                | function           | 可选地为该区段定义任意项渲染器，覆盖列表的默认 [`renderItem`](sectionlist#renderitem)。                          |
| ItemSeparatorComponent                                    | component, element | 可选地为该区段定义任意项分隔符，覆盖列表的默认 [`ItemSeparatorComponent`](sectionlist#itemseparatorcomponent)。 |
