...
一个用于通过键盘向应用输入文本的基础组件。Props 提供了几种功能的可配置性，例如自动纠正、自动大写、占位符文本和不同的键盘类型，例如数字键盘。

最基本的用例是放置一个 `TextInput` 并订阅 `onChangeText` 事件以读取用户输入。还有其他事件，例如 `onSubmitEditing` 和 `onFocus` 可以订阅。一个最小化的示例：
...
通过原生元素暴露的两个方法是 `.focus()` 和 `.blur()`，它们将以编程方式聚焦或取消聚焦 TextInput。

请注意，某些 props 仅在 `multiline={true/false}` 时可用。此外，仅适用于元素一侧的边框样式（例如 `borderBottomColor`, `borderLeftWidth` 等）在 `multiline=true` 时将不会应用。要实现相同的效果，你可以将 `TextInput` 包裹在 `View` 中：
...
`TextInput` 默认在其视图底部有一个边框。此边框的内边距由系统提供的背景图像设置，无法更改。避免这种情况的解决方案是要么不显式设置高度，在这种情况下系统将负责在正确位置显示边框，要么通过将 `underlineColorAndroid` 设置为 transparent 来不显示边框。
...
指定字体是否应缩放以尊重文本大小辅助功能设置。默认值为 `true`。
...
如果为 `true`，提交时文本字段将失去焦点。单行字段的默认值为 true，多行字段的默认值为 false。请注意，对于多行字段，将 `blurOnSubmit` 设置为 `true` 意味着按下返回键将使字段失去焦点并触发 `onSubmitEditing` 事件，而不是在字段中插入换行符。
...
提供一个初始值，当用户开始输入时将更改。适用于你不想处理监听事件和更新 value prop 以保持受控状态同步的用例。
...
提供时，它将设置组件中光标（或 "caret"）的颜色。与 `selectionColor` 的行为不同，光标颜色将独立于文本选择框的颜色进行设置。
...
当为 `false` 时，如果文本输入周围可用空间很小（例如手机横向模式），操作系统可能会选择让用户在全屏文本输入模式下编辑文本。当为 `true` 时，此功能被禁用，用户将始终直接在文本输入内编辑文本。默认为 `false`。
...
如果为 `true`，当没有文本时键盘禁用返回键，当有文本时自动启用。默认值为 `false`。
...
确定返回键上应显示什么文本。优先级高于 `returnKeyType` prop。
...
告诉操作系统你的应用中的各个字段是否应包含在用于 Android API Level 26+ 自动填充目的的视图结构中。可能的值为 `auto`, `no`, `noExcludeDescendants`, `yes`, 和 `yesExcludeDescendants`。默认值为 `auto`。
...
作用类似于 HTML 中的 `inputmode` 属性，它确定打开哪个键盘，例如 `numeric`，并且优先级高于 `keyboardType`。
...
在此处 [查看](https://davidl.fr/blog/keyboard-react-native-ios-android#all-react-native-keyboard-type-examples-i-os-on-the-left-android-on-the-right) 所有类型的截图。
...
指定当 `allowFontScaling` 启用时字体可达到的最大缩放比例。可能值：
...
限制可输入的最大字符数。使用此方法而不是在 JS 中实现逻辑以避免闪烁。
...
:::note
重要的是要注意，这在 iOS 上将文本顶部对齐，而在 Android 上居中。在两个平台上获得相同行为，请配合使用 `textAlignVertical` 设置为 `top`。
:::
...
> 注意：如果你试图从 `nativeEvent` 访问 `text` 值，请记住你得到的结果值可能是 `undefined`，这可能导致意外错误。如果你试图查找 TextInput 的最后一个值，可以使用 [`onEndEditing`](textinput#onendediting) 事件，它在编辑完成时触发。
...
当按下键时调用的回调。这将使用对象调用，其中 `keyValue` 对于相应键为 `'Enter'` 或 `'Backspace'`，否则为输入的字符，包括 `' '` 表示空格。在 `onChange` 回调之前触发。注意：在 Android 上仅处理软键盘的输入，不处理硬件键盘输入。
...
在内容滚动时调用。可能还包含来自 `ScrollEvent` 的其他属性，但在 Android 上出于性能原因不提供 `contentSize`。
...
如果为 `true`，允许 TextInput 将触摸事件传递给父组件。这允许诸如 SwipeableListView 之类的组件在 iOS 上从 TextInput 滑动，就像 Android 上的默认情况一样。如果为 `false`，TextInput 始终请求处理输入（除非被禁用）。默认值为 `true`。
...
如果为 `true`，文本输入会模糊输入的文本，以便密码等敏感文本保持安全。默认值为 `false`。不适用于 `multiline={true}`。
...
当为 `false` 时，它将防止字段聚焦时显示软键盘。默认值为 `true`。
...
如果为 `false`，iOS 系统在粘贴操作后不会插入额外空格，也不会在剪切或删除操作后删除一两个空格。
...
- `'submit'` 将仅发送提交事件而不使输入失去焦点
- `'blurAndSubmit`' 将使输入失去焦点并发送提交事件
...
`newPassword` 可用于指示用户可能想要保存在钥匙链中的新密码输入，`oneTimeCode` 可用于指示字段可以通过短信到达的代码自动填充。
...
当在 iOS 上使用 `textContentType` 作为 `newPassword` 时，我们可以让操作系统知道密码的最低要求，以便它可以生成满足这些要求的密码。为了创建有效的 `PasswordRules` 字符串，请查看 [Apple 文档](https://developer.apple.com/password-rules/)。

> 如果密码生成对话框没有出现，请确保：
>
> - 自动填充已启用：**设置** → **密码与账户** → 切换 "开" **自动填充密码**，
> - iCloud 钥匙串已使用：**设置** → **Apple ID** → **iCloud** → **钥匙串** → 切换 "开" **iCloud 钥匙串**。
...
请注意，并非所有 Text 样式都受支持，不支持的内容的不完整列表包括：
...
在 Android API Level 23+ 上设置文本换行策略，可能值为 `simple`, `highQuality`, `balanced` 默认值为 `highQuality`。
...
要为文本输入显示的值。`TextInput` 是一个受控组件，这意味着如果提供了此 value prop，原生值将被强制匹配此值。对于大多数用途，这效果很好，但在某些情况下这可能导致闪烁 - 一个常见原因是通过保持值相同来防止编辑。除了设置相同的值外，要么设置 `editable={false}`，要么设置/更新 `maxLength` 以防止不必要的编辑而不闪烁。
...
如果为 `true`，键盘快捷键（撤销/重做和复制按钮）被禁用。默认值为 `false`。
