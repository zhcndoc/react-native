...
`PanResponder` 将多个触摸协调为一个手势。它使单点触控手势能够抵御额外的触摸，并可用于识别基本的多点触控手势。
...
- `moveX` - 最近一次移动的触摸点的最新屏幕坐标
- `moveY` - 最近一次移动的触摸点的最新屏幕坐标
...
`config` 对象为所有 responder 回调提供了增强版本，这些回调不仅提供 [`PressEvent`](pressevent)，还提供 `PanResponder` 手势状态；其方式是在每个典型的 `onResponder*` 回调中将单词 `Responder` 替换为 `PanResponder`。例如，`config` 对象如下所示：
...
请谨慎使用 `onStartShould*` 回调。它们只会反映已经更新的 `gestureState`，适用于会 bubble/capture 到 Node 的 start/end 事件。一旦该节点成为 responder，你就可以依赖每个 start/end 事件都会被手势处理，并且 `gestureState` 会相应更新。（numberActiveTouches）在你不是 responder 时可能并不完全准确。
