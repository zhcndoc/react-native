...
在使用 React Native 时，你的 JavaScript 代码最多会在三个环境中运行：

- 在大多数情况下，React Native 会使用 [Hermes](hermes)，这是一款为 React Native 优化的开源 JavaScript 引擎。
- 如果禁用 Hermes，React Native 会使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)，也就是为 Safari 提供支持的 JavaScript 引擎。请注意，在 iOS 上，由于 iOS 应用中不存在可写的可执行内存，JavaScriptCore 不会使用 JIT。
- 在使用 Chrome 调试时，所有 JavaScript 代码都在 Chrome 本身中运行，并通过 WebSockets 与原生代码通信。Chrome 使用 [V8](https://v8.dev/) 作为其 JavaScript 引擎。

尽管这些环境非常相似，你仍可能遇到一些不一致之处。最好避免依赖任何运行时的特定细节。

## JavaScript 语法转换器

语法转换器通过允许你使用新的 JavaScript 语法，而不必等待所有解释器都支持它，从而让编写代码更愉快。

React Native 随附 [Babel JavaScript 编译器](https://babeljs.io)。有关其支持的转换的更多细节，请查看 [Babel 文档](https://babeljs.io/docs/plugins/#transform-plugins)。
...
<tbody>
  ...
  <TableRow name="展开调用" code="Math.max(...array);" url="https://babeljs.io/docs/learn-es2015/#default-rest-spread" />
  ...
  <TableRow name="默认导出导入" code="export v from 'mod';" url="https://babeljs.io/docs/en/babel-plugin-proposal-export-default-from" />
