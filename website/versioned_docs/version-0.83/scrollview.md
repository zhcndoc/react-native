...
滚动到指定的 x、y 偏移位置，可立即滚动或以平滑动画方式滚动。
...
:::note
之所以有这个奇怪的函数签名，是因为出于历史原因，该函数既接受一个对象参数，也接受分开的多个参数。由于参数顺序（y 在 x 之前）会造成歧义，这种形式已废弃，强烈不建议使用分开的参数形式。
:::
...
如果是纵向 `ScrollView`，则滚动到底部；如果是横向，则滚动到右侧。

使用 `scrollToEnd({animated: true})` 可进行平滑动画滚动，`scrollToEnd({animated: false})` 则会立即滚动。如果不传参数，`animated` 默认为 `true`。