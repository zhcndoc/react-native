...
一个包装了平台 `DrawerLayout` 的 React 组件（仅限 Android）。抽屉（通常用于导航）通过 `renderNavigationView` 渲染，直接子元素则作为主视图（内容区域）。导航视图最初不会显示在屏幕上，但可以从 `drawerPosition` 属性指定的窗口一侧滑入，其宽度可以通过 `drawerWidth` 属性设置。
    ...
    <SafeAreaProvider>
      <DrawerLayoutAndroid
        ...
        <SafeAreaView style={styles.container}>
          <Text style={styles.paragraph}>抽屉在 {drawerPosition} 边！</Text>
          <Button
            title="更改抽屉位置"
...
指定抽屉的背景颜色。默认值为 `white`。如果你想设置抽屉的不透明度，请使用 rgba。例如：
...
- locked-closed，表示抽屉将保持关闭状态且不会响应手势。
- locked-open，表示抽屉将保持打开状态且不会响应手势。抽屉仍然可以通过程序方式打开和关闭（`openDrawer`/`closeDrawer`）。
...
- settling，表示曾与导航视图发生交互，而导航视图现在正在完成其关闭或打开动画
...
让抽屉占据整个屏幕，并绘制状态栏的背景，以便它可以覆盖状态栏打开。它仅在 API 21+ 上生效。
