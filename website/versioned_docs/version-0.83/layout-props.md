...
:::info
关于这些属性的更详细示例可以在 [使用 Flexbox 的布局](flexbox) 页面找到。
:::

### 示例

以下示例展示了不同属性如何影响或塑造 React Native 布局。你可以尝试在更改属性 `flexWrap` 的值时，例如添加或移除 UI 中的正方形。
...
`alignItems` 控制子元素在交叉方向上的对齐方式。例如，如果子元素按垂直方向排列，`alignItems` 控制它们如何在水平方向对齐。它的作用类似于 CSS 中的 `align-items`（默认值：stretch）。
...
- 对于已设置 flex basis 的节点，如果交叉轴尺寸未设置，宽高比控制节点在交叉轴上的尺寸
- 对于带有 measure 函数的节点，宽高比的作用就像 measure 函数在测量 flex basis 一样
- 对于具有 flex grow/shrink 的节点，如果交叉轴尺寸未设置，宽高比控制节点在交叉轴上的尺寸
- 宽高比会考虑最小/最大尺寸
...
更多关于 `bottom` 如何影响布局的详情请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/bottom)。
...
`boxSizing` 定义元素的各类尺寸属性（`width`、`height`、`minWidth`、`minHeight` 等）如何计算。如果 `boxSizing` 是 `border-box`，这些尺寸将应用于元素的边框盒。如果是 `content-box`，它们将应用于元素的内容盒。默认值是 `border-box`。如果你想了解更多这个属性的工作方式，[Web 文档](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) 是很好的信息来源。
...
`direction` 指定用户界面的方向性流动。默认值为 `inherit`，但根节点除外，根节点的值会基于当前语言区域。
...
在 React Native 中，`flex` 的工作方式与 CSS 中不同。`flex` 是数字而不是字符串，并且它根据 [Yoga](https://github.com/facebook/yoga) 布局引擎运作。

当 `flex` 为正数时，它会使组件具有弹性，并按其 flex 值的比例确定大小。因此，`flex` 设置为 `2` 的组件将占据 `flex` 设置为 `1` 的组件两倍的空间。`flex: <正数>` 等同于 `flexGrow: <正数>, flexShrink: 1, flexBasis: 0`。

当 `flex` 为 `0` 时，组件会根据 `width` 和 `height` 来确定尺寸，并且它是非弹性的。

当 `flex` 为 `-1` 时，组件通常会根据 `width` 和 `height` 来确定尺寸。但是，如果空间不足，组件会收缩到其 `minWidth` 和 `minHeight`。
...
`flexBasis` 是一种与轴无关的方式，用于提供项目在主轴上的默认尺寸。为子元素设置 `flexBasis` 类似于在其父元素是 `flexDirection: row` 的容器时为该子元素设置 `width`，或者在其父元素是 `flexDirection: column` 的容器时为该子元素设置 `height`。项目的 `flexBasis` 是其默认尺寸，也就是在执行任何 `flexGrow` 和 `flexShrink` 计算之前的尺寸。
...
`flexDirection` 控制容器中的子元素朝哪个方向排列。`row` 表示从左到右，`column` 表示从上到下，另外两个你大概也能猜到。它的作用类似于 CSS 中的 `flex-direction`，不同之处在于默认值是 `column`。
...
`flexGrow` 描述容器内的空间应如何沿主轴分配给子元素。布局完成后，容器会根据子元素指定的 flex grow 值分配剩余空间。
...
[`flexShrink`](layout-props#flexshrink) 描述在主轴上子元素总尺寸超过容器主轴尺寸时，如何收缩子元素。`flexShrink` 与 `flexGrow` 非常相似；如果将任何溢出尺寸视为负的剩余空间，也可以按同样方式理解它。这两个属性也能很好地协同工作，使子元素按需伸缩。

`flexShrink` 接受任何大于等于 0 的浮点数，默认值为 0。容器会按子元素的 `flexShrink` 值加权收缩子元素。
...
请注意，它不再与 `alignItems: stretch`（默认值）一起工作，因此你可能需要改用 `alignItems: flex-start`，例如（破坏性更改详情：https://github.com/facebook/react-native/releases/tag/v0.28.0）。
...
`justifyContent` 控制子元素在主方向上的对齐方式。例如，如果子元素按垂直方向排列，`justifyContent` 控制它们如何在垂直方向对齐。它的作用类似于 CSS 中的 `justify-content`（默认值：flex-start）。
...
更多关于 `left` 如何影响布局的详情请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/left)。
...
设置 `paddingHorizontal` 的效果类似于同时设置 `paddingLeft` 和 `paddingRight`。
...
设置 `paddingVertical` 的效果类似于同时设置 `paddingTop` 和 `paddingBottom`。
...
React Native 中的 `position` 类似于[普通 CSS](https://developer.mozilla.org/en-US/docs/Web/CSS/position)，但默认都会设置为 `relative`。

`relative` 会按照布局的正常流来定位元素。内边距（`top`、`bottom`、`left`、`right`）会相对于此布局进行偏移。

`absolute` 会使元素脱离布局的正常流。内边距会相对于其[包含块](./flexbox.md#the-containing-block)进行偏移。

`static` 会按照布局的正常流来定位元素。内边距不会产生任何影响。
`static` 元素不会为绝对定位的后代形成包含块。

更多信息请参见 [Flexbox 布局文档](./flexbox.md#position)。此外，[Yoga 文档](https://www.yogalayout.dev/docs/styling/position) 提供了更多关于 `position` 在 React Native 与 CSS 之间差异的细节。
...
更多关于 `right` 如何影响布局的详情请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/right)。
...
更多关于 `top` 如何影响布局的详情请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/top)。
...
`zIndex` 控制哪些组件显示在其他组件之上。通常不需要使用 `zIndex`。组件会根据它们在文档树中的顺序进行渲染，因此后面的组件会覆盖前面的组件。如果你有动画或自定义模态界面，并且不希望出现这种行为，`zIndex` 可能会很有用。

它的作用类似于 CSS 的 `z-index` 属性——`zIndex` 更大的组件会显示在上层。可以把 z 方向想象成从手机指向你眼睛的方向。

在 iOS 上，`zIndex` 可能要求 `View` 彼此作为兄弟节点才能按预期工作。
