...
:::warning[Caution]
**请注意不要无意中存储或暴露敏感信息。** 例如，可能会意外发生这种情况：将敏感表单数据保存到 redux 状态中，并把整个状态树持久化到 Async Storage。或者将用户令牌和个人信息发送到 Sentry 或 Crashlytics 等应用监控服务。
:::
...
:::warning[Caution]
使用 SSL 固定时，您应注意证书过期问题。证书每 1-2 年会过期一次，一旦过期，就需要在应用中以及服务器上同时更新。只要服务器上的证书更新了，任何内嵌旧证书的应用都将停止工作。
:::
