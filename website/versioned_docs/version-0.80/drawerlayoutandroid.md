...
一个封装了平台 `DrawerLayout` 的 React 组件（仅 Android）。抽屉（通常用于导航）通过 `renderNavigationView` 渲染，直接子组件则是主视图（内容显示的位置）。导航视图最初不会显示在屏幕上，但可以从 `drawerPosition` 属性指定的窗口一侧滑入，其宽度可以通过 `drawerWidth` 属性设置。
...
- locked-open，表示抽屉会保持打开且不响应手势。抽屉仍然可以通过程序打开和关闭（`openDrawer`/`closeDrawer`）。
...
- settling，表示曾与导航视图发生交互，而导航视图正在完成其关闭或打开动画
...
使抽屉占据整个屏幕，并绘制状态栏背景，从而允许它在状态栏上方打开。它仅在 API 21+ 上生效。
