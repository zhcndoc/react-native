...
`Easing` 模块实现了常见的缓动函数。该模块被 [`Animated.timing()`](animated.md#timing) 使用，以便在动画中传达物理上可信的运动。

你可以在 https://easings.net/ 找到一些常见缓动函数的可视化效果

### 预定义动画

`Easing` 模块通过以下方法提供几种预定义动画：

- [`back`](easing.md#back) 提供一个基本动画，对象在向前移动之前稍微回退
- [`bounce`](easing.md#bounce) 提供一个弹跳动画
- [`ease`](easing.md#ease) 提供一个基本惯性动画
- [`elastic`](easing.md#elastic) 提供一个基本弹簧交互
...
[`poly`](easing.md#poly) 函数可用于实现四次、五次和其他更高幂次的函数。
...
默认弹性系数为 1，会稍微超调一次。0 弹性系数完全不会超调，弹性系数为 N > 1 时将超调大约 N 次。
...
与 `Animated.parallel()` 配合使用，创建一个基本效果，对象在动画开始时稍微回退。
...
使任何缓动函数对称。缓动函数将在一半持续时间正向运行，然后在剩余持续时间反向运行。
