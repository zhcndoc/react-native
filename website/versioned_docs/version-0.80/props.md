...
使用 `name` 作为 prop 让我们可以定制 `Greeting` 组件，因此我们可以为每个问候语复用该组件。此示例还在 JSX 中使用 `Greeting` 组件，类似于 [核心组件](intro-react-native-components)。能够做到这一点是 React 如此酷的原因——如果你发现自己希望有一套不同的 UI 原语来使用，你可以发明新的。

这里出现的另一个新事物是 [`View`](view.md) 组件。[`View`](view.md) 作为其他组件的容器很有用，有助于控制样式和布局。
