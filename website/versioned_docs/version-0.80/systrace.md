...
`Systrace` 是一个标准的基于标记的 Android 性能分析工具（在安装 Android platform-tools 包时会一并安装）。被分析的代码块被开始/结束标记包围，随后以彩色图表格式可视化。Android SDK 和 React Native 框架都提供可可视化的标准标记。
...
beginEvent/endEvent 用于在同一调用栈帧内开始然后结束一个性能分析。
...
beginAsyncEvent/endAsyncEvent 用于开始然后结束一个性能分析，其中结束可能发生在另一个线程或当前栈帧之外，例如等待返回的 cookie 变量应作为输入用于 endAsyncEvent 调用来结束性能分析。
...
将该值注册到 systrace 时间线上的 profileName。
