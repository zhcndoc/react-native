...
本指南将向您展示如何编写 [Web Storage API](https://html.spec.whatwg.org/multipage/webstorage.html#dom-localstorage-dev) 的实现：`localStorage`。对于可能在您的项目上编写应用程序代码的 React 开发人员来说，此 API 是相关的。
...
React Native 提供了一个名为 [Codegen](/the-new-architecture/what-is-codegen.mdx) 的工具，它会读取用 TypeScript 或 Flow 编写的规范，并为 Android 和 iOS 生成平台特定代码。该规范声明了将在您的原生代码与 React Native JavaScript 运行时之间来回传递的方法和数据类型。Turbo 原生模块既包括您的规范、您编写的原生代码，也包括根据您的规范生成的 Codegen 接口。
...
一切都为 Codegen 配置好后，我们需要准备原生代码以挂钩到生成的代码。
...
:::note
本指南向您展示如何创建一个仅适用于新架构的 Turbo 原生模块。如果您需要同时支持新架构和旧架构，请参阅我们的 [向后兼容性指南](https://github.com/reactwg/react-native-new-architecture/blob/main/docs/backwards-compat.md)。
:::
