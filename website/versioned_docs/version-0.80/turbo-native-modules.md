...
你的 React Native 应用代码可能需要与原生平台 API 交互，而这些 API 并未由 React Native 或现有库提供。你可以使用 **Turbo 原生模块** 自行编写集成代码。本指南将向你展示如何编写一个。
...
3. **使用你的规范编写应用代码**；以及
4. **使用生成的接口编写原生平台代码**，将你的原生代码编写并接入 React Native 运行时环境。

让我们通过构建一个示例 Turbo 原生模块来逐步完成这些步骤。本指南的其余部分假设你已经通过以下命令创建了应用：
...
本指南将向你展示如何实现 [Web Storage API](https://html.spec.whatwg.org/multipage/webstorage.html#dom-localstorage-dev)：`localStorage`。这个 API 对于可能正在项目中编写应用代码的 React 开发者来说非常熟悉。

要让它在移动端可用，我们需要使用 Android 和 iOS API：
...
React Native 提供了一个名为 [Codegen](/the-new-architecture/what-is-codegen.mdx) 的工具，它会读取用 TypeScript 或 Flow 编写的规范，并为 Android 和 iOS 生成平台特定代码。该规范声明了你的原生代码与 React Native JavaScript 运行时之间来回传递的方法和数据类型。Turbo 原生模块既包括你的规范、你编写的原生代码，也包括由你的规范生成的 Codegen 接口。
...
:::info
你可以在 [附录](/appendix.md) 文档中查看规范中可以使用的所有类型，以及生成的原生类型。
:::

:::info
如果你想更改模块名称以及相关规范文件的名称，请务必始终使用 `Native` 作为前缀（例如 `NativeStorage` 或 `NativeUsersDefault`）。
:::
...
该规范会被 React Native Codegen 工具用于为我们生成平台特定接口和样板代码。为此，Codegen 需要知道在哪里找到我们的规范，以及如何处理它。请将你的 `package.json` 更新为包含：
...
完成 Codegen 相关配置后，我们需要准备原生代码，以便接入生成的代码。
...
Codegen 会作为脚本阶段的一部分运行，该脚本阶段会由 CocoaPods 自动添加到项目中。
...
使用 `NativeLocalStorage`，这里有一个修改后的 `App.tsx`，它包含我们想要持久化的一些文本、一个输入框，以及一些用于更新该值的按钮。
...
完成所有准备后，我们就开始编写原生平台代码。我们分两部分进行：
