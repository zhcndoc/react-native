...
在使用它之前，尝试先用 `setState` 和 [`shouldComponentUpdate`](https://react.dev/reference/react/Component#shouldcomponentupdate) 来解决你的问题。
:::

## 使用 setNativeProps 编辑 TextInput 值

`setNativeProps` 的另一个非常常见的用例是编辑 TextInput 的值。当 `bufferDelay` 较低且用户输入非常快时，TextInput 的 `controlled` prop 有时会丢失字符。一些开发者倾向于完全跳过此 prop，而是在必要时使用 `setNativeProps` 直接操作 TextInput 值。
