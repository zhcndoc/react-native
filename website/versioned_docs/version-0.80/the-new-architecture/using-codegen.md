...
- `name:` 代码生成配置的名称。这将自定义代码生成输出：文件名和代码。
- `type:`
  - `modules:` 仅为模块生成代码。
  - `components:` 仅为组件生成代码。
  - `all`: 为所有内容生成代码。
...
- `android`: Android 的 Codegen 配置（均为可选）：
  - `.javaPackageName`: 配置 Android Java 代码生成输出的包名。
- `ios`: iOS 的 Codegen 配置（均为可选）：
  - `.modules[moduleName]:`
    - `.className`: 此模块的 ObjC 类。或者，如果它是一个 [仅 C++ 模块](/docs/next/the-new-architecture/pure-cxx-modules)，则是其 `RCTModuleProvider` 类。
...
本指南的其余部分假设你已经在项目中设置了 Native Turbo Module、Native Fabric Component 或两者兼有。我们还假设你在 `package.json` 中指定的 `jsSrcsDir` 中有有效的规范文件。

### Android

Android 的 **Codegen** 与 React Native Gradle Plugin (RNGP) 集成。RNGP 包含一个可以调用的任务，该任务读取 `package.json` 文件中定义的配置并执行 **Codegen**。要运行 gradle 任务，首先导航到项目的 `android` 文件夹内。然后运行：
...
运行上面的 gradle 命令后，你将在 `SampleApp/android/app/build` 文件夹中找到代码生成代码。结构将如下所示：
...
最后在 `jni` 文件夹中，这里有所有将 JS 连接到 Android 的样板代码。
...
此结构是通过为 `codegenConfig.type` 字段使用值 `all` 生成的。如果你使用值 `modules`，期望看不到 `react/renderer/components/` 文件夹。如果你使用值 `components`，期望看不到任何其他文件。
