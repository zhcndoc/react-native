...
- 为您的自定义类型显式特化 `Bridging` 结构体。在本例中，模板指定了 `int64_t` 类型。
...
第一个类型是 `Address` 的定义。第二个类型包含将所有基础结构从 JS 桥接到 C++ 所需的内容，反之亦然。我们需要添加的唯一额外步骤是定义扩展 `NativeSampleModuleAddressBridging` 类型的 `Bridging` 结构。
...
然后，代码通过扩展由 Codegen 生成的 `NativeSampleModuleAddressBridging` 为新 `Address` 类型添加 `Bridging` 特化。

:::note
生成这些类型遵循一个约定：

- 名称的第一部分始终是模块的类型。在本例中为 `NativeSampleModule`。
- 名称的第二部分始终是在规范中定义的 JS 类型的名称。在本例中为 `Address`。
  :::
