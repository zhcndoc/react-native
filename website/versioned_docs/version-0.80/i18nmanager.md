...
`I18nManager` 模块提供用于管理从右到左 (RTL) 布局支持的工具，适用于阿拉伯语、希伯来语和其他语言。它提供用于控制 RTL 行为和检查当前布局方向的方法。
...
- 否则，在以下情况下 `isRTL` 将为 `true`：
  - **iOS:**
    - 设备上的用户首选语言是 RTL 语言
    - 应用定义的本地化包括用户选择的语言（如在 Xcode 项目文件中定义的那样（`knownRegions = (...)`））
  - **Android:**
    - 设备上的用户首选语言是 RTL 语言
