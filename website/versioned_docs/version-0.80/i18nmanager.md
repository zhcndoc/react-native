...
如果你将元素绝对定位以与其他 flexbox 元素对齐，它们在 RTL 语言中可能不会对齐。可以使用 `isRTL` 来调整对齐或动画。
...
避免在生产应用中强制启用 RTL，因为它需要完整重启应用才能生效，这会带来较差的用户体验。
