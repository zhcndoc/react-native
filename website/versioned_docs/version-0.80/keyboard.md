...
Keyboard 模块允许你监听原生事件并做出响应，也可以对键盘进行更改，例如将其收起。
...
| callback <div className="label basic two-lines required">必需</div>  | function | 事件触发时要调用的函数                                 |
...
> 注意：在 Android 上，仅可使用 `keyboardDidShow` 和 `keyboardDidHide` 事件。如果你的 activity 将 `android:windowSoftInputMode` 设置为 `adjustNothing`，那么在 Android 10 及以下版本中，这些事件不会被触发。
