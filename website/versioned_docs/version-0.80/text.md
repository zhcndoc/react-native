...
在 Web 上，为整个文档设置字体家族和大小的通常方法是利用继承的 CSS 属性，如下所示：
...
你还失去了为整个子树设置默认字体的能力。同时，`fontFamily` 只接受单个字体名称，这与 CSS 中的 `font-family` 不同。推荐的方式是创建一个组件 `MyAppText`，其中包含它们，并在整个应用中使用此组件。你也可以使用此组件创建更具体的组件，如 `MyAppHeaderText` 用于其他类型的文本。
...
假设 `MyAppText` 是一个只将其子级渲染到带有样式的 `Text` 组件的组件，那么 `MyAppHeaderText` 可以定义如下：
...
以这种方式组合 `MyAppText` 确保我们获得顶层组件的样式，但使我们在特定用例中添加/覆盖它们的能力。
...
- (开发者) React 组件的设计考虑到强隔离性：你应该能够将组件放在应用中的任何地方，相信只要属性相同，它的外观和行为方式就会相同。可以从属性外部继承的文本属性将破坏这种隔离。

- (实现者) React Native 的实现也简化了。我们不需要在每个元素上都有一个 `fontFamily` 字段，也不需要每次显示文本节点时都可能遍历树直到根节点。样式继承仅编码在本机 Text 组件内，不会泄漏到其他组件或系统本身。
...
无障碍提示帮助用户理解当他们在无障碍元素上执行操作时会发生什么，当该结果从无障碍标签中不清楚时。
...
一个值，指示屏幕阅读器在用户与元素交互时应使用哪种语言。它应遵循 [BCP 47 规范](https://www.rfc-editor.org/info/bcp47)。

参见 [iOS `accessibilityLanguage` 文档](https://developer.apple.com/documentation/objectivec/nsobject/1615192-accessibilitylanguage) 以获取更多信息。
...
覆盖用户与元素交互时屏幕阅读器读取的文本。默认情况下，标签是通过遍历所有子级并累积所有用空格分隔的 `Text` 节点构建的。
...
在 iOS 上，这些角色映射到相应的无障碍特征。图像按钮的功能与将特征设置为“image”和“button”相同。参见 [无障碍指南](accessibility.md#accessibilitytraits-ios) 以获取更多信息。

在 Android 上，这些角色在 TalkBack 上具有类似的功能，就像在 iOS 上的 Voiceover 中添加无障碍特征一样。

| 类型                                                 |
| ---------------------------------------------------- |
| [无障碍角色](accessibility#accessibilityrole) |
...
| 类型                                                   |
| ------------------------------------------------------ |
| [无障碍状态](accessibility#accessibilitystate) |
...
参见 [无障碍指南](accessibility.md#accessibility-actions) 以获取更多信息。
...
参见 [无障碍指南](accessibility#accessible-ios-android) 以获取更多信息。
...
设置在确定 Android API Level 23+ 上的单词断行时使用的自动连字符频率。
...
要应用于此元素的 [动态类型](https://developer.apple.com/documentation/uikit/uifont/scaling_fonts_automatically) 等级。
...
- `head` - 显示该行以便末尾适合容器，行开头缺失的文本由省略号指示。例如，"...wxyz"
- `middle` - 显示该行以便开头和末尾适合容器，中间缺失的文本由省略号指示。"ab...yz"
- `tail` - 显示该行以便开头适合容器，行末尾缺失的文本由省略号指示。例如，"abcd..."
- `clip` - 行不会绘制在文本容器边缘之外。
...
| 类型                                                   |
| ------------------------------------------------------ |
| `md ({nativeEvent: [按压事件](pressevent)}) => void` |

---

### `onMoveShouldSetResponder`

此视图是否想“主张”触摸响应？当 `View` 不是响应者时，每次在 `View` 上触摸移动时调用。

| 类型                                                      |
| --------------------------------------------------------- |
| `md ({nativeEvent: [按压事件](pressevent)}) => boolean` |
...
View 现在正在响应触摸事件。这是高亮并向用户显示正在发生的事情的时候。

在 Android 上，从此回调返回 true 以防止任何其他原生组件成为响应者，直到此响应者终止。

| 类型                                                              |
| ----------------------------------------------------------------- |
| `md ({nativeEvent: [按压事件](pressevent)}) => void ｜ boolean` |
...
响应者已从 `View` 被移除。可能在调用 `onResponderTerminationRequest` 后被其他视图接管，或者可能被操作系统接管而不询问（例如，在 iOS 上发生在控制中心/通知中心）。
...
其他某个 `View` 想成为响应者，并要求此 `View` 释放其响应者身份。返回 `true` 允许其释放。
...
如果父 `View` 想阻止子 `View` 在触摸开始时成为响应者，它应该拥有这个返回 `true` 的处理程序。
...
| 类型                                                 |
| ---------------------------------------------------- |
| ([`文本布局事件`](text#textlayoutevent)) => mixed |

---

### `pressRetentionOffset`

当滚动视图被禁用时，这定义在停用按钮之前，你的触摸可以移出按钮多远。一旦停用，尝试移回它，你会看到按钮再次被激活！在滚动视图禁用时来回移动几次。确保你传入一个常量以减少内存分配。
...
当 `true` 时，按下文本时不会进行视觉更改。默认情况下，按下时一个灰色椭圆会高亮文本。
...
在 Android API Level 23+ 上设置文本断行策略，可能的值是 `simple`, `highQuality`, `balanced`。
...
| 名称   | 类型                                    | 可选 | 描述                                           |
| ------ | --------------------------------------- | -------- | ----------------------------------------------------- |
| lines  | [TextLayout](text#textlayout) 对象数组 | 否       | 提供每个渲染行的 TextLayout 数据。 |
