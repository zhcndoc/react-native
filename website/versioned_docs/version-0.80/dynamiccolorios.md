...
`DynamicColorIOS` 接受单个参数，该参数是一个对象，包含两个必填键：`dark` 和 `light`，以及两个可选键 `highContrastLight` 和 `highContrastDark`。这些对应于你在 iOS 上用于“浅色模式”和“深色模式”的颜色，以及当启用高对比度辅助功能模式时，它们的高对比度版本。
