...
- 默认情况下，如果你尝试在应用处于前台时运行任务，应用将会崩溃。这是为了防止开发者因在任务中做大量工作而拖慢 UI，从而自食其果。你可以传递第四个 `boolean` 参数来控制此行为。
...
以下几行展示了用于注册广播接收器的 Android 清单文件的一部分。
...
广播接收器随后处理在 onReceive 函数中广播的 intent。这是检查应用是否处于前台的好地方。如果应用不在前台，我们可以准备要启动的 intent，不带信息或使用 `putExtra` 捆绑额外信息（请记住 bundle 只能处理可序列化值）。最后服务被启动并且唤醒锁被获取。
