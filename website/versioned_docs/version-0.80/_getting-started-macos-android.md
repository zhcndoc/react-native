...
如果你是 Android 开发新手，设置开发环境可能会有些繁琐。如果你已经熟悉 Android 开发，可能有一些事情需要配置。无论哪种情况，请务必仔细遵循以下步骤。
...
在 SDK Manager 中选择"SDK Platforms"标签，然后勾选右下角"Show Package Details"旁边的复选框。查找并展开 `Android 15 (VanillaIceCream)` 条目，然后确保选中以下项目：
...
接下来，选择"SDK Tools"标签，并同样勾选此处"Show Package Details"旁边的复选框。查找并展开"Android SDK Build-Tools"条目，然后确保选中 `35.0.0` 和 `Android SDK Command-line Tools (latest)`。
...
你需要一个 Android 设备来运行你的 React Native Android 应用。这可以是物理 Android 设备，或者更常见的是，你可以使用 Android 虚拟设备（Android Virtual Device），它允许你在电脑上模拟 Android 设备。
