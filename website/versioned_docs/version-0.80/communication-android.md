...
同样的概念也适用于 React Native。只要我们纯粹在框架内构建应用程序，我们就可以通过属性和回调来驱动应用程序。但是，当我们混合使用 React Native 和原生组件时，我们需要一些特定的跨语言机制，以便允许我们在它们之间传递信息。
...
跨语言属性的主要缺点是不支持回调，这将允许我们处理自底向上的数据绑定。想象一下，你有一个小的 RN 视图，由于 JS 操作的结果，你希望将其从原生父视图中移除。使用属性无法做到这一点，因为信息需要自底向上传递。

虽然我们有一种跨语言回调的风格（[在此描述](legacy/native-modules-android#callbacks)），但这些回调并不总是我们需要的。主要问题是它们不作为属性传递。相反，这种机制允许我们从 JS 触发原生操作，并在 JS 中处理该操作的结果。
...
React Native 允许你执行跨语言函数调用。你可以从 JS 执行自定义原生代码，反之亦然。不幸的是，取决于我们工作的一侧，我们通过不同的方式实现相同的目标。对于原生——我们使用事件机制来调度 JS 中处理函数的执行，而对于 React Native，我们直接调用原生模块导出的方法。
...
- 由于事件可以从任何地方发送，它们可能会将面条式依赖引入你的项目中。
