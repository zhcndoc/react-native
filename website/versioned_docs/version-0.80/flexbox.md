...
:::caution
Flexbox 在 React Native 中的工作方式与 Web 上的 CSS 相同，有一些例外。
...
在下面的示例中，红色、橙色和绿色视图都是设置了 `flex: 1` 的容器视图中的子元素。红色视图使用 `flex: 1`，橙色视图使用 `flex: 2`，绿色视图使用 `flex: 3`。**1+2+3 = 6**，这意味着红色视图将获得 `1/6` 的空间，橙色获得 `2/6` 的空间，绿色获得 `3/6` 的空间。
...
[`flexDirection`](layout-props#flexdirection) 控制节点子元素的布局方向。这也被称为主轴。交叉轴是垂直于主轴的轴，或者是换行所在的轴。
...
布局 [`direction`](layout-props#direction) 指定层次结构中子元素和文本的布局方向。布局方向还影响 `start` 和 `end` 指代的边缘。默认情况下，React Native 使用 LTR 布局方向。在此模式下，`start` 指左侧，`end` 指右侧。
...
- `space-between` 沿容器主轴均匀分布子元素，剩余空间分布在子元素之间。

- `space-around` 沿容器主轴均匀分布子元素，剩余空间分布在子元素周围。与 `space-between` 相比，使用 `space-around` 会将空间分布到第一个子元素的开头和最后一个子元素的结尾。

- `space-evenly` 沿主轴在对齐容器内均匀分布子元素。每对相邻项目之间、主轴起始边缘与第一个项目之间、以及主轴结束边缘与最后一个项目之间的间距完全相同。
...
:::info
要使 `stretch` 生效，子元素在次要轴上不能有固定尺寸。在下面的示例中，直到从子元素中移除 `width: 50` 之前，设置 `alignItems: stretch` 没有任何效果。
:::
...
[alignContent](layout-props#aligncontent) 定义行沿交叉轴的分布方式。仅当使用 `flexWrap` 将项目换行到多行时，此属性才生效。
...
- `space-around` 沿容器交叉轴均匀分布换行后的行，将剩余空间分布在这些行周围。容器两端的空间大小是项目之间空间的一半。

- `space-evenly` 沿容器交叉轴均匀分布换行后的行，将剩余空间分布在这些行周围。每个空间的大小相同。

您可以 [在此处](https://www.yogalayout.dev/docs/styling/align-content) 了解更多。
...
[`flexWrap`](layout-props#flexwrap) 属性设置在容器上，它控制当子元素沿主轴溢出容器尺寸时会发生什么。默认情况下，子元素被强制放入单行（这可能会缩小元素）。如果允许换行，则在需要时沿主轴将项目换行到多行。

换行时，可以使用 `alignContent` 来指定行在容器中的放置方式。 [了解更多](https://www.yogalayout.dev/docs/styling/flex-wrap)。
...
- [`flexBasis`](layout-props#flexbasis) 是一种与轴无关的方式，用于提供项目沿主轴的默认尺寸。设置子元素的 `flexBasis` 类似于设置该子元素的 `width`（如果其父容器是 `flexDirection: row` 的容器）或设置子元素的 `height`（如果其父容器是 `flexDirection: column` 的容器）。项目的 `flexBasis` 是该项目的默认尺寸，即在执行任何 `flexGrow` 和 `flexShrink` 计算之前的项目尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述了容器内多少空间应沿主轴分配给其子元素。在布局其子元素后，容器将根据其子元素指定的 flex grow 值分配任何剩余空间。

  `flexGrow` 接受任何 >= 0 的浮点值，默认值为 0。容器将根据子元素的 `flexGrow` 值加权分配任何剩余空间。

- [`flexShrink`](layout-props#flexshrink) 描述了在子元素的总尺寸沿主轴溢出容器尺寸的情况下，如何沿主轴缩小子元素。`flexShrink` 与 `flexGrow` 非常相似，如果任何溢出尺寸被视为负剩余空间，则可以以相同的方式思考。这两个属性也配合得很好，允许子元素根据需要增长和缩小。

  `flexShrink` 接受任何 >= 0 的浮点值，默认值为 0（在 Web 上，默认值为 1）。容器将根据子元素的 `flexShrink` 值加权缩小其子元素。

您可以 [了解更多](https://www.yogalayout.dev/docs/styling/flex-basis-grow-shrink)。
...
- [`rowGap`](layout-props#rowgap) 设置元素行之间间隙（沟槽）的大小。

- [`columnGap`](layout-props#columngap) 设置元素列之间间隙（沟槽）的大小。

- [`gap`](layout-props#gap) 设置行和列之间间隙（沟槽）的大小。它是 `rowGap` 和 `columnGap` 的简写。
...
- relative (**默认值**) 默认情况下，元素是相对定位的。这意味着元素根据布局的正常流进行定位，然后根据 top、right、bottom 和 left 的值相对于该位置进行偏移。偏移不会影响任何兄弟元素或父元素的位置。

- absolute 当绝对定位时，元素不参与正常布局流。相反，它独立于其兄弟元素进行布局。位置根据 top、right、bottom 和 left 的值确定。这些值将相对于其包含块定位元素。

- static 当静态定位时，元素根据布局的正常流进行定位，并将忽略 top、right、bottom 和 left 的值。此 position 还会导致元素不为绝对后代元素形成包含块，除非存在其他优先样式属性（例如 transform）。这允许 absolute 元素定位到非父元素的某些东西上。注意：**static` 仅在新架构上可用**。
...
React Native 中包含块的工作方式与 [它们在 Web 上的工作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block) 非常相似，但由于缺少某些 Web 功能，做了一些简化。
...
此外，你可以查看来自 [Wix Engineers](https://medium.com/wix-engineering/the-full-react-native-layout-cheat-sheet-a4147802405c) 的一些示例。
