...
[`flex`](layout-props#flex) 将定义你的项目如何沿主轴**“填充”**可用空间。空间会按照每个元素的 flex 属性进行分配。

在下面的示例中，红色、橙色和绿色视图都是容器视图的子元素，该容器设置了 `flex: 1`。红色视图使用 `flex: 1`，橙色视图使用 `flex: 2`，绿色视图使用 `flex: 3`。**1+2+3 = 6**，这意味着红色视图将获得空间的 `1/6`，橙色视图获得 `2/6`，绿色视图获得 `3/6`。
...
- `column`（**默认值**）将子元素从上到下排列。如果启用换行，则下一行将从容器顶部第一个项目的右侧开始。

- `row` 将子元素从左到右排列。如果启用换行，则下一行将从容器左侧第一个项目的下方开始。

- `column-reverse` 将子元素从下到上排列。如果启用换行，则下一行将从容器底部第一个项目的右侧开始。

- `row-reverse` 将子元素从右到左排列。如果启用换行，则下一行将从容器右侧第一个项目的下方开始。
...
布局 [`direction`](layout-props#direction) 指定层级中子元素和文本的布局方向。布局方向也会影响 `start` 和 `end` 分别指代哪一侧。默认情况下，React Native 使用 LTR 布局方向。在这种模式下，`start` 指左侧，`end` 指右侧。

- `LTR`（**默认值**）文本和子元素从左到右排列。应用于元素起始位置的 margin 和 padding 会应用在左侧。

- `RTL` 文本和子元素从右到左排列。应用于元素起始位置的 margin 和 padding 会应用在右侧。
...
[`justifyContent`](layout-props#justifycontent) 描述了如何在容器的主轴上对齐子元素。例如，你可以使用这个属性在 `flexDirection` 设置为 `row` 的容器中让子元素水平居中，或者在 `flexDirection` 设置为 `column` 的容器中让子元素垂直居中。
...
- `space-around` 在容器主轴上均匀分布子元素，并在子元素周围分配剩余空间。与 `space-between` 相比，使用 `space-around` 会在第一个子元素的起始处和最后一个子元素的末尾处分配空间。

- `space-evenly` 在对齐容器内沿主轴均匀分布子元素。每一对相邻项目之间的间距、主轴起始边缘与第一个项目之间的间距，以及主轴末尾边缘与最后一个项目之间的间距，都完全相同。
...
- `stretch`（**默认值**）拉伸容器的子元素，使其与容器交叉轴的 `height` 一致。
...
- `baseline` 沿共同的基线对齐容器的子元素。可以将单个子元素设置为其父元素的参考基线。

:::info
要让 `stretch` 生效，子元素在次轴上不能有固定尺寸。在下面的示例中，在从子元素中移除 `width: 50` 之前，设置 `alignItems: stretch` 不会产生任何效果。
:::
...
[`alignSelf`](layout-props#alignself) 与 `alignItems` 具有相同的选项和效果，但不同的是，它不是影响容器内的子元素，而是可以将这个属性应用到单个子元素上，以改变它在父容器中的对齐方式。`alignSelf` 会覆盖父组件通过 `alignItems` 设置的任何选项。
...
[alignContent](layout-props#aligncontent) 定义了沿交叉轴方向各行的分布方式。只有当项目使用 `flexWrap` 换行到多行时，它才会生效。

- `flex-start` (**默认值**) 将换行后的各行对齐到容器交叉轴的起始位置。

- `flex-end` 将换行后的各行对齐到容器交叉轴的末端。

- `stretch` (_在 web 上使用 Yoga 时的默认值_) 拉伸换行后的各行，使其匹配容器交叉轴的高度。
...
- `space-evenly` 在容器交叉轴上均匀分布换行后的各行，在各行周围分配剩余空间。每个间距的大小都相同。
...
在换行时，可以使用 `alignContent` 来指定这些行在容器中的排列方式。了解更多信息 [这里](https://www.yogalayout.dev/docs/styling/flex-wrap)。
...
- [`flexBasis`](layout-props#flexbasis) 是一种与轴无关的方式，用于提供项目在主轴上的默认尺寸。为子元素设置 `flexBasis`，类似于当其父元素是 `flexDirection: row` 的容器时为该子元素设置 `width`，或当其父元素是 `flexDirection: column` 的容器时为该子元素设置 `height`。项目的 `flexBasis` 是该项目的默认尺寸，也就是在执行任何 `flexGrow` 和 `flexShrink` 计算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述容器内有多少空间应沿主轴分配给子元素。在布局完成后，容器会根据子元素指定的 `flexGrow` 值分配任何剩余空间。

  `flexGrow` 接受任何大于等于 0 的浮点数，默认值为 0。容器会按照子元素的 `flexGrow` 值加权，将剩余空间分配给各个子元素。

- [`flexShrink`](layout-props#flexshrink) 描述在子元素总尺寸沿主轴超出容器尺寸时，如何沿主轴收缩子元素。`flexShrink` 与 `flexGrow` 非常相似，如果把任何溢出的尺寸视为负的剩余空间，也可以用相同的方式理解它。这两个属性也能很好地配合使用，使子元素能够按需要增长和收缩。

  `flexShrink` 接受任何大于等于 0 的浮点数，默认值为 0（在 Web 上默认值为 1）。容器会按照子元素的 `flexShrink` 值加权，收缩各个子元素。
...
- [`rowGap`](layout-props#rowgap) 设置元素各行之间间距（gutter）的大小。

- [`columnGap`](layout-props#columngap) 设置元素各列之间间距（gutter）的大小。

- [`gap`](layout-props#gap) 设置行与列之间间距（gutter）的大小。它是 `rowGap` 和 `columnGap` 的简写。
...
- `auto`（**默认值**）React Native 会根据元素的内容来计算宽/高，无论这些内容是其他子元素、文本还是图片。

- `pixels` 以绝对像素定义宽/高。根据组件上设置的其他样式，这可能会也可能不会成为节点的最终尺寸。

- `percentage` 分别以其父级宽度或高度的百分比来定义宽度或高度。
...
- `relative`（**默认值**）默认情况下，元素采用相对定位。这意味着元素会按照布局的正常流进行定位，然后根据 `top`、`right`、`bottom` 和 `left` 的值，相对于该位置进行偏移。该偏移不会影响任何兄弟元素或父元素的位置。

- `absolute` 当采用绝对定位时，元素不会参与正常的布局流。它会独立于其兄弟元素进行布局。其位置由 `top`、`right`、`bottom` 和 `left` 的值决定。这些值会将元素相对于其包含块进行定位。

- `static` 当采用静态定位时，元素会按照布局的正常流进行定位，并且会忽略 `top`、`right`、`bottom` 和 `left` 的值。这个 `position` 还会导致该元素不会为绝对定位的后代形成包含块，除非存在某个其他更高优先级的样式属性（例如 `transform`）。这使得 `absolute` 元素可以相对于不是其父元素的对象进行定位。请注意，**`static` 仅在新架构中可用**。
...
应用于绝对定位元素的百分比长度（例如：`width: '50%'` 或 `padding: '10%'`）会根据其包含块的尺寸来计算。例如，如果包含块宽度为 100 点，那么绝对定位元素上的 `width: 50%` 会使其宽度为 50 点。

下面的列表将帮助你确定任意给定元素的包含块：

- 如果该元素的 `position` 类型为 `relative` 或 `static`，那么它的包含块就是其父元素。
- 如果该元素的 `position` 类型为 `absolute`，那么它的包含块是满足以下任一条件的最近祖先：
...
我们已经介绍了基础内容，但布局中你可能还需要许多其他样式。控制布局的完整属性列表记录在[这里](./layout-props.md)。

另外，你也可以查看来自 [Wix 工程师](https://medium.com/wix-engineering/the-full-react-native-layout-cheat-sheet-a4147802405c) 的一些示例。
