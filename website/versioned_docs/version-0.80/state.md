...
在实际应用中，你可能不会使用计时器来设置 state。当你有来自服务器的新数据或用户输入时，可能会设置 state。你也可以使用状态容器，如 [Redux](https://redux.js.org/) 或 [MobX](https://mobx.js.org/) 来控制数据流。在这种情况下，你将使用 Redux 或 MobX 来修改状态，而不是直接调用 `setState`。

当调用 setState 时，BlinkApp 将重新渲染其组件。通过在计时器内调用 setState，组件将在每次计时器滴答时重新渲染。
