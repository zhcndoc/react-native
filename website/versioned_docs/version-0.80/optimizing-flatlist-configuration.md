...
- **Responsiveness:** 应用程序响应交互的能力。例如，低响应性是指当你触摸组件时，它会等待一会儿才响应，而不是按预期立即响应。

- **Blank areas:** 当 `VirtualizedList` 无法足够快地渲染你的项时，你可能会进入列表中没有渲染组件的部分，显示为空白空间。

- **Viewport:** 渲染为像素的可见内容区域。

- **Window:** 项应该挂载的区域，通常比视口大得多。
...
**优点：** 这减少了主线程上的耗时，从而通过从原生渲染和绘制遍历中排除视口之外的视图，降低了丢帧的风险。

**缺点：** 请注意，此实现可能存在错误，例如内容丢失（主要在 iOS 上观察到），尤其是当你使用复杂的变换和/或绝对定位时。另请注意，这不会节省大量内存，因为视图没有被释放，只是被分离了。
...
**缺点：** 每批更多项意味着更长的 JavaScript 执行时间，可能会阻塞其他事件处理，如点击，从而损害响应性。
...
**优点：** 将此属性与 `maxToRenderPerBatch` 结合使用，可以让你例如在不那么频繁的批次中渲染更多项，或在更频繁的批次中渲染更少项。

**缺点：** 较不频繁的批次可能导致空白区域，较频繁的批次可能导致响应性问题。
...
**优点：** 定义覆盖每种设备屏幕的确切项数。这可以大幅提升初始渲染的性能。
...
组件越重，渲染越慢。避免使用重型图片（对于列表项使用裁剪版本或缩略图，尽可能小）。与设计团队沟通，在列表中尽可能少地使用效果、交互和信息。在项的详情中展示它们。
...
你可以使用社区包（例如来自 [Dream11](https://github.com/ds-horizon) 的 [@d11/react-native-fast-image](https://github.com/ds-horizon/react-native-fast-image)）来获得性能更高的图片。列表中的每张图片都是一个 `new Image()` 实例。它越快到达 `loaded` 钩子，你的 JavaScript 线程就越快再次空闲。

### 使用 getItemLayout

如果所有列表项组件具有相同的高度（对于水平列表则为宽度），提供 [getItemLayout](flatlist#getitemlayout) 属性可以免除 `FlatList` 管理异步布局计算的需要。这是一个非常理想的优化技术。

如果你的组件具有动态大小且你确实需要性能，请考虑询问你的设计团队是否可以重新设计以获得更好的性能。

### 使用 keyExtractor 或 key

你可以为 `FlatList` 组件设置 [`keyExtractor`](flatlist#keyextractor) 属性。此属性用于缓存并作为 React `key` 来跟踪项的重新排序。
