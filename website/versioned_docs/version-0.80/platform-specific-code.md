...
在构建跨平台应用时，你希望尽可能多地复用代码。某些情况下代码需要有所不同是有意义的，例如你可能想要为 Android 和 iOS 实现独立的视觉组件。

React Native 提供了两种方式来组织你的代码并按平台分离：
...
某些组件可能拥有仅在一个平台上工作的属性。所有这些 props 都用 `@platform` 标注，并在网站上旁边有一个小徽章。

## Platform 模块

React Native 提供了一个模块来检测应用运行所在的平台。你可以使用检测逻辑来实现平台特定的代码。使用此选项当只有组件的小部分是平台特定的时。
...
还有一个可用的 `Platform.select` 方法，给定一个对象，其键可以是 `'ios' | 'android' | 'native' | 'default'` 之一，返回最适合当前运行平台的值。也就是说，如果你在手机上运行，`ios` 和 `android` 键将优先。如果未指定这些，将使用 `native` 键，然后是 `default` 键。
...
**注意**：`Version` 设置为 Android API 版本而不是 Android OS 版本。要查找映射请参考 [Android 版本历史](https://en.wikipedia.org/wiki/Android_version_history#Overview)。
...
```shell
Container.js # 被 webpack、Rollup 或任何其他 Web 打包器选取
Container.native.js # 被 React Native 打包器选取，适用于 Android 和 iOS (Metro)
```
...
**专业提示：** 配置你的 Web 打包器以忽略 `.native.js` 扩展名，以避免在生产打包文件中包含未使用的代码，从而减小最终打包文件的大小。
