...
你还需要安装 Xcode 命令行工具。打开 Xcode，然后从 Xcode 菜单中选择 **设置...（或偏好设置...）**。前往位置面板，通过在命令行工具下拉菜单中选择最新版本来安装工具。

![Xcode 命令行工具](/docs/assets/GettingStartedXcodeCommandLineTools.png)

#### 在 Xcode 中安装 iOS 模拟器

要安装模拟器，打开 **Xcode > 设置...（或偏好设置...）** 并选择 **平台（或组件）** 标签页。选择你希望使用的对应 iOS 版本的模拟器。

如果你使用的是 Xcode 14.0 或更高版本来安装模拟器，打开 **Xcode > 设置 > 平台** 标签页，然后点击 "+" 图标并选择 **iOS…** 选项。
...
你可能还希望确保你的 Xcode 项目的所有"shell 脚本构建阶段"都使用 `/bin/zsh` 作为其 shell。
:::
