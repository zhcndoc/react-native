...
- `background` - 应用正在后台运行。用户此时要么：
  - 在另一个应用中
  - 在主屏幕上
  - [Android] 在另一个 `Activity` 中，包括临时的系统活动，例如自动填充凭证选择器（即使由你的应用或系统启动）
- [iOS] `inactive` - 这是在前台与后台之间切换时出现的一种状态，以及在不活跃期间，例如进入多任务视图、打开通知中心或来电时。
...
此示例似乎只会显示 "Current state is: active"，因为应用仅在 `active` 状态下对用户可见。如果你想尝试这段代码，我们建议使用你自己的设备而不是嵌入式预览。
...
当用户未积极与应用交互时接收。适用于用户下拉 [通知抽屉](https://developer.android.com/guide/topics/ui/notifiers/notifications#bar-and-drawer) 的情况。`AppState` 不会改变，但会触发 `blur` 事件。
