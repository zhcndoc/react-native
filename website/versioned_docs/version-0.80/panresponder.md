...
`PanResponder` 将多个触摸协调为单个手势。它使单触摸手势不受额外触摸的影响，并可用于识别基本的多触摸手势。
...
原生事件是一种形式为 [PressEvent](pressevent) 的合成触摸事件。
...
小心 `onStartShould*` 回调。它们仅反映冒泡/捕获到节点的开始/结束事件的更新 `gestureState`。一旦节点成为响应者，你可以依赖每个开始/结束事件都被手势处理并且 `gestureState` 相应更新。(numberActiveTouches) 可能不完全准确，除非你是响应者。
