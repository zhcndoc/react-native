...
:::note
调试功能，例如开发菜单（Dev Menu）、LogBox 和 React Native DevTools，在发布（生产）构建中是禁用的。
:::
...
首次启动时，DevTools 将打开一个欢迎面板，以及一个打开的控制台抽屉，你可以在其中查看日志并与 JavaScript 运行时交互。从窗口顶部，你可以导航到其他面板，包括集成的 React 组件检查器（React Components Inspector）和 Profiler。

![React Native DevTools 打开至“欢迎”面板](/docs/assets/debugging-rndt-welcome.jpg)

React Native DevTools 由内置于 React Native 的专用调试架构提供支持，并使用 [Chrome DevTools](https://developer.chrome.com/docs/devtools) 前端界面的定制版本。这使我们能够提供熟悉的、与浏览器对齐的调试功能，这些功能深度集成并为端到端的可靠性而构建。

在我们的 [React Native DevTools 指南](./react-native-devtools) 中了解更多。

:::note
React Native DevTools 仅在使用 Hermes 引擎时可用，并且需要安装 Google Chrome 或 Microsoft Edge。
:::
...
React Native DevTools 取代了之前的 Flipper、Experimental Debugger 和 Hermes debugger (Chrome) 前端界面。如果你使用的是较旧版本的 React Native，请前往 [对应版本](/versions) 的文档。

对于使用 JavaScriptCore 而不是 Hermes 的应用，直接 JSC 调试仍然可用（参见 [其他调试方法](./other-debugging-methods)）。

React Native DevTools 旨在用于调试 React 应用相关问题，而不是取代原生工具。如果你想检查 React Native 的底层平台层（例如，在开发原生模块时），请使用 Xcode 和 Android Studio 中可用的调试工具（参见 [调试原生代码](/docs/next/debugging-native-code)）。

其他有用链接：

- <a href="https://shift.infinite.red/why-you-dont-need-flipper-in-your-react-native-app-and-how-to-get-by-without-it-3af461955109" target="_blank">为什么你的 React Native 应用不需要 Flipper……以及如何在没有它的情况下应对 ↗</a>
...
控制台错误和警告会显示为带有红色或黄色徽章的屏幕通知。

- **错误** 将显示通知计数。点击通知可查看展开视图并翻页查看其他日志。
...
在 Android 和 iOS 上，可以在开发过程中通过在开发菜单中选择 **"性能监控"** 来切换应用内性能覆盖层。[此处](/docs/performance) 了解更多关于此功能的信息。

![iOS 和 Android 上的性能监控器覆盖层](/docs/assets/debugging-performance-monitor.jpg)

:::info
性能监控器在应用内运行，仅供参考。我们建议在 Android Studio 和 Xcode 下调查原生工具以获得准确的性能测量。
:::
