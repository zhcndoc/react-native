...
2. 右键点击 app 并选择 <code>新建组</code>，将新组命名为 `NativeLocalStorage`。

<img className="half-size" alt="右键点击 app 并选择新建组" src="/docs/assets/turbo-native-modules/xcode/2.webp" />

3. 在 `NativeLocalStorage` 组中，创建 <code>新建</code>→<code>来自模板的文件</code>。
...
然后更新我们的实现以使用 `NSUserDefaults` 以及自定义的 [套件名称](https://developer.apple.com/documentation/foundation/nsuserdefaults/1409957-initwithsuitename)。
