...
- `active` - 应用正在前台运行
- `background` - 应用正在后台运行。用户此时可能处于以下情况之一：
  - 在另一个应用中
  - 在主屏幕上
  - [Android] 在另一个 `Activity` 上，包括临时的系统活动，例如自动填充凭据选择器（即使它是由你的应用或系统启动的）
- [iOS] `inactive` - 这是在前台与后台之间切换时出现的一种状态，也会在无操作期间出现，例如进入多任务视图、打开通知中心或接到来电时。
