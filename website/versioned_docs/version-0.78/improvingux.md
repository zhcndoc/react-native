...
在触屏手机上输入文本是一项挑战——屏幕小，还有软件键盘。但根据你需要什么样的数据，你可以通过正确配置文本输入来使其更容易：
...
软件键盘几乎占据了屏幕的一半。如果你有可能会被键盘覆盖的交互元素，请确保通过使用 [`KeyboardAvoidingView` 组件](keyboardavoidingview.md) 使它们仍然可访问。
...
在手机上按下按钮时很难非常精确。确保所有交互元素的大小为 44x44 或更大。做到这一点的一种方法是为元素留出足够的空间，`padding`、`minWidth` 和 `minHeight` 样式值对此很有用。或者，您可以使用 [`hitSlop` 属性](touchablewithoutfeedback.md#hitslop) 来增加交互区域而不影响布局。这是一个演示：
...
Android API 21+ 使用 Material Design 涟漪效果，在用户触摸屏幕上的交互区域时提供反馈。React Native 通过 [`TouchableNativeFeedback` 组件](touchablenativefeedback.md) 暴露了这一功能。使用这种触摸效果而不是透明度或高亮效果，通常会让你的应用在平台上感觉更加合适。也就是说，使用时需要小心，因为它在 iOS 或 Android API < 21 上不起作用，因此你需要在 iOS 上回退使用其他 Touchable 组件之一。你可以使用像 [react-native-platform-touchable](https://github.com/react-community/react-native-platform-touchable) 这样的库来为你处理平台差异。
...
默认情况下，多种屏幕方向应该能正常工作，除非你使用了 `Dimensions` API 且没有处理方向变化。如果你不想支持多种屏幕方向，可以将屏幕方向锁定为纵向或横向。

在 iOS 上，在 Xcode 的 General 标签页和 Deployment Info 部分中启用你想要支持的设备方向（在进行更改时确保你已从 Devices 菜单中选择了 iPhone）。对于 Android，打开 AndroidManifest.xml 文件，并在 activity 元素内添加 `'android:screenOrientation="portrait"'` 以锁定为纵向，或添加 `'android:screenOrientation="landscape"'` 以锁定为横向。

# 了解更多

[Material Design](https://material.io/) 和 [人类界面指南](https://developer.apple.com/design/human-interface-guidelines) 是学习更多关于移动平台设计的绝佳资源。
