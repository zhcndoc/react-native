...
然后更新我们的实现以使用 `NSUserDefaults` 和自定义的 [suite 名称](https://developer.apple.com/documentation/foundation/nsuserdefaults/1409957-initwithsuitename)。
...
- `RCT_EXPORT_MODULE` 导出并使用我们将在 JavaScript 环境中访问它的标识符注册模块：`NativeLocalStorage`。详见这些 [文档](./legacy/native-modules-ios#module-name)。
