...
返回一个 Promise，如果用户选择了日期，该 Promise 将解析为一个包含 `action`、`year`、`month` (0-11)、`day` 的对象。如果用户关闭了对话框，Promise 仍然会被 resolve，其中 action 为 `DatePickerAndroid.dismissedAction`，所有其他键为 undefined。**始终**检查 `action` 是否等于 `DatePickerAndroid.dateSetAction` 后再读取值。
