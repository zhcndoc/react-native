...
:::info
如果您使用的是 Expo，请阅读 Expo 指南 [部署到应用商店](https://docs.expo.dev/distribution/app-stores/) 以构建并提交您的应用到 Apple App Store。此指南适用于任何 React Native 应用以自动化部署流程。
:::
...
现在您可以通过按 <kbd>Cmd ⌘</kbd> + <kbd>B</kbd> 或从菜单栏选择 **Product** → **Build** 来构建发布版本的应用。一旦构建为发布版本，您将能够将应用分发给测试人员并提交应用到 App Store。
...
- 根据您的需要在 `Automatically manage signing` 和 `Manually manage signing` 之间选择。
- 点击 `Upload`。
...
Apple Store 要求您提供最新设备的截图。此类设备的参考信息可在 [这里](https://developer.apple.com/help/app-store-connect/reference/screenshot-specifications/) 找到。请注意，如果已提供其他尺寸的截图，则不需要某些显示尺寸的截图。
