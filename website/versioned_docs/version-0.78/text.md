...
在下面的示例中，嵌套的标题和正文文本将从 `styles.baseText` 继承 `fontFamily`，但标题提供了其自己的额外样式。由于字面换行符，标题和正文将相互堆叠：
...
Android 和 iOS 都允许你通过用特定格式（如粗体或彩色文本）注解字符串的范围来显示格式化文本（iOS 上为 `NSAttributedString`，Android 上为 `SpannableString`）。实际上，这非常繁琐。对于 React Native，我们决定为此使用 Web 范式，你可以嵌套文本来实现相同的效果。
...
幕后，React Native 将其转换为包含以下信息的扁平 `NSAttributedString` 或 `SpannableString`：
...
你还会失去为整个子树设置默认字体的能力。同时，`fontFamily` 只接受单个字体名称，这与 CSS 中的 `font-family` 不同。跨应用程序使用一致字体和大小的推荐方法是创建一个包含它们的组件 `MyAppText`，并在整个应用程序中使用此组件。你还可以使用此组件为其他类型的文本创建更具体的组件，如 `MyAppHeaderText`。
...
假设 `MyAppText` 是一个仅将其子项渲染到带有样式的 `Text` 组件中的组件，那么 `MyAppHeaderText` 可以定义如下：
...
以这种方式组合 `MyAppText` 确保我们从顶级组件获取样式，但使我们在特定用例中添加/覆盖它们的能力。
...
- (Developer) React 组件的设计考虑到强隔离性：你应该能够将组件放在应用程序的任何地方，相信只要 props 相同，它的外观和行为方式就会相同。可以从 props 外部继承的文本属性将破坏这种隔离。

- (Implementor) React Native 的实现也简化了。我们不需要在每个元素上都有一个 `fontFamily` 字段，也不需要每次显示文本节点时都可能遍历树直到根节点。样式继承仅编码在本机 Text 组件内，不会泄漏到其他组件或系统本身。
...
无障碍提示帮助用户理解当他们在无障碍元素上执行操作且结果从无障碍标签中不清楚时会发生什么。
...
表示屏幕阅读器在用户与元素交互时应使用哪种语言的值。它应遵循 [BCP 47 规范](https://www.rfc-editor.org/info/bcp47)。
...
覆盖用户与元素交互时屏幕阅读器读取的文本。默认情况下，标签是通过遍历所有子项并累积所有由空格分隔的 `Text` 节点构建的。
...
在 iOS 上，这些角色映射到相应的无障碍特性。图像按钮的功能与将特性设置为“image”和“button”相同。参见 [无障碍指南](accessibility.md#accessibilitytraits-ios) 以获取更多信息。
...
无障碍操作允许辅助技术以编程方式调用组件的操作。`accessibilityActions` 属性应包含操作对象列表。每个操作对象应包含字段名称和标签。
...
设置在 Android API Level 23+ 上确定单词断字时使用的自动连字符频率。
...
确定文本元素中转换为可点击 URL 的数据类型。默认情况下，不检测数据类型。
...
要应用于 iOS 上此元素的 [动态类型](https://developer.apple.com/documentation/uikit/uifont/scaling_fonts_automatically)  ramp。
...
- `head` - 行的显示方式使得末尾适合容器，行开头缺失的文本由省略号符号指示。例如，"...wxyz"
- `middle` - 行的显示方式使得开头和末尾适合容器，中间缺失的文本由省略号符号指示。"ab...yz"
- `tail` - 行的显示方式使得开头适合容器，行末尾缺失的文本由省略号符号指示。例如，"abcd..."
- `clip` - 行不会绘制在文本容器的边缘之外。
...
指定当 `allowFontScaling` 启用时字体可以达到的最大可能缩放比例。可能值：
...
当触摸 engaged 时立即调用，在 `onPressOut` 和 `onPress` 之前。
...
在 Android 上，从此回调返回 true 以防止任何其他本机组件成为响应者，直到此响应者终止。
...
响应者已从 `View` 被拿走。可能在调用 `onResponderTerminationRequest` 后被其他视图拿走，或者可能被操作系统未经询问拿走（例如，在 iOS 上与控制中心/通知中心一起发生）。
...
当滚动视图被禁用时，这定义了你的触摸可以在按钮上移动多远，然后才停用按钮。一旦停用，尝试移回它，你会看到按钮再次被激活！当滚动视图被禁用时，来回移动几次。确保传入一个常量以减少内存分配。
...
当 `true` 时，按下文本时不进行视觉更改。默认情况下，按下时灰色椭圆高亮显示文本。
