...
当用户在 Android 上按下硬件返回按钮，或在 Apple TV 上按下菜单按钮时，会调用 `onRequestClose` 回调。由于这是必需的属性，请注意，只要模态框处于打开状态，`BackHandler` 事件就不会被触发。
在 iOS 上，当 `presentationStyle` 为 `pageSheet` 或 `formSheet` 时，使用拖拽手势关闭 Modal 会调用此回调。
