...
React Native 提供了一个名为 [Codegen](/the-new-architecture/what-is-codegen.md) 的工具，它接受用 TypeScript 或 Flow 编写的规范，并为 Android 和 iOS 生成特定平台的代码。该规范声明了将在您的原生代码和 React Native JavaScript 运行时之间来回传递的方法和数据结构。Turbo 原生模块既包括您的规范，也包括您编写的原生代码，以及从您的规范生成的 Codegen 接口。
