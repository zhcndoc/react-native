...
> **针对模态框用户的警告：** 如果你的应用显示了一个打开的 `Modal`，`BackHandler` 将不会发布任何事件（[见 `Modal` 文档](modal#onrequestclose)）。
...
`BackHandler.addEventListener` 创建一个事件监听器 & 返回一个 `NativeEventSubscription` 对象，该对象应该使用 `NativeEventSubscription.remove` 方法清除。

## 与 React Navigation 一起使用

如果你正在使用 React Navigation 在不同屏幕间导航，你可以参考他们的指南 [自定义 Android 返回按钮行为](https://reactnavigation.org/docs/custom-android-back-button-handling/)
