...
InteractionManager 允许在任何交互/动画完成后调度长时间运行的工作。特别是，这允许 JavaScript 动画流畅运行。
...
`runAfterInteractions` 接受普通回调函数，或具有 `gen` 方法（返回 `Promise`）的 `PromiseTask` 对象。如果提供了 `PromiseTask`，则在开始下一个可能更早同步排队的任务之前，它将完全解析（包括通过 `runAfterInteractions` 调度更多任务的异步依赖项）。

默认情况下，排队任务在一个 `setImmediate` 批次中一起在循环中执行。如果调用 `setDeadline` 并传入正数，则任务只会执行到截止日期（就 JS 事件循环运行时间而言）接近为止，此时执行将通过 setTimeout 让出，允许触摸等事件启动交互并阻止排队任务执行，使应用程序响应更灵敏。
...
调度一个函数在所有交互完成后运行。返回一个可取消的"promise"。
