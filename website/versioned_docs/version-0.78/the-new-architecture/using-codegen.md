...
- `ios`: `ios` 字段是一个对象，应用开发者和库维护者可以使用它来提供一些高级功能。以下所有字段都是**可选的**。
...
在 `java` 文件夹中，你可以在 `com/facebook/viewmanagers` 子文件夹中找到 Fabric 原生组件生成的代码。
...
在 `codegenConfig.android.javaPackageName` 中设置的文件夹中，你可以找到 Turbo 原生模块必须实现以执行其任务的抽象类。
