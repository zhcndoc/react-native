...
假设我们要桥接一个具有以下属性的自定义 `Address` 类型：
...
第一个类型是 `Address` 的定义。第二个类型包含将所有基础结构从 JS 桥接到 C++ 反之亦然所需的内容。我们需要添加的唯一额外步骤是定义扩展 `NativeSampleModuleAddressBridging` 类型的 `Bridging` 结构体。
