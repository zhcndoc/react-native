...
用于驱动 2D 动画（例如平移手势）的 2D 值。与普通的 [`Animated.Value`](animatedvalue) API 几乎相同，但是是多路复用的。底层包含两个常规的 `Animated.Value`。
...
设置一个偏移量，该偏移量会应用于任何设置的值之上，无论是通过 `setValue`、动画还是 `Animated.event` 设置的。对于补偿平移手势开始时的位置等情况很有用。
...
向值添加一个异步监听器，以便你可以观察来自动画的更新。这很有用，因为无法同步读取该值，因为它可能是由原生驱动的。
