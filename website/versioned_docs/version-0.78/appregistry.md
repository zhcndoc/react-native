...
<div className="banner-native-code-required">
  <h3>需要原生代码的项目</h3>
  <p>如果您使用的是托管的 Expo 工作流，则只有一个入口组件注册到 <code>AppRegistry</code>，它是自动处理的（或通过 <a href="https://docs.expo.dev/versions/latest/sdk/register-root-component/">registerRootComponent</a>）。您不需要使用此 API。</p>
</div>
...
当视图应该被销毁时“停止”应用，请使用传递给 `runApplication` 的标签调用 `AppRegistry.unmountApplicationComponentAtRootTag`。这两者应该总是成对使用。
...
这是一种当应用在后台时运行 JavaScript 任务的方式。例如，它可以用于同步新数据、处理推送通知或播放音乐。
