...
你通常会结合使用 `flexDirection`、`alignItems` 和 `justifyContent` 来实现正确的布局。
...
[`flex`](layout-props#flex) 将定义你的元素如何沿着主轴"**填充**"可用空间。空间将根据每个元素的 flex 属性进行划分。
...
[`flexDirection`](layout-props#flexdirection) 控制节点子元素的布局方向。这也被称为主轴。交叉轴是垂直于主轴的轴，或者是换行布局所在的轴。
...
[`justifyContent`](layout-props#justifycontent) 描述了如何在容器的主轴上对齐子元素。例如，您可以使用此属性在 `flexDirection` 设置为 `row` 的容器中将子元素水平居中，或在 `flexDirection` 设置为 `column` 的容器中将子元素垂直居中。
...
- `space-around` 沿容器主轴均匀分布子元素，将剩余空间分布在子元素周围。与 `space-between` 相比，使用 `space-around` 会导致空间分布在第一个子元素的开头和最后一个子元素的结尾。
...
[`alignItems`](layout-props#alignitems) 描述了如何沿容器的交叉轴对齐子元素。它与 `justifyContent` 非常相似，但 `alignItems` 应用于交叉轴而不是主轴。
...
[alignContent](layout-props#aligncontent) 定义沿交叉轴的行分布。仅当使用 `flexWrap` 将项目换行到多行时，此属性才有效。
...
换行时，可以使用 `alignContent` 指定行在容器中的放置方式。了解更多 [这里](https://www.yogalayout.dev/docs/styling/flex-wrap)。
...
- [`flexBasis`](layout-props#flexbasis) 是一种与轴无关的方式，用于提供项目沿主轴的默认大小。设置子元素的 `flexBasis` 类似于设置该子元素的 `width`（如果其父容器是 `flexDirection: row` 的容器）或设置子元素的 `height`（如果其父容器是 `flexDirection: column` 的容器）。项目的 `flexBasis` 是该项目的默认大小，即执行任何 `flexGrow` 和 `flexShrink` 计算之前的大小。

- [`flexGrow`](layout-props#flexgrow) 描述了容器内的多少空间应沿主轴分配给其子元素。布局子元素后，容器将根据其子元素指定的 flex grow 值分配任何剩余空间。

  `flexGrow` 接受任何 >= 0 的浮点值，默认值为 0。容器将根据子元素的 `flexGrow` 值加权分配任何剩余空间。

- [`flexShrink`](layout-props#flexshrink) 描述了在子元素的总大小沿主轴溢出容器大小的情况下，如何沿主轴缩小子元素。`flexShrink` 与 `flexGrow` 非常相似，如果任何溢出大小被视为负剩余空间，则可以以相同方式思考。这两个属性也配合得很好，允许子元素根据需要增长和缩小。

  `flexShrink` 接受任何 >= 0 的浮点值，默认值为 0（在 Web 上，默认值为 1）。容器将根据子元素的 `flexShrink` 值加权缩小其子元素。

您可以 [这里](https://www.yogalayout.dev/docs/styling/flex-basis-grow-shrink) 了解更多。
...
- `auto`（**默认值**）React Native 根据元素的内容（无论是其他子元素、文本还是图像）计算宽度/高度。

- `pixels` 以绝对像素定义宽度/高度。根据组件上设置的其他样式，这可能不是节点的最终尺寸。

- `percentage` 分别以其父元素的宽度或高度的百分比定义宽度或高度。
...
- `relative` (**默认值**) 默认情况下，元素是相对定位的。这意味着元素根据布局的正常流进行定位，然后根据 `top`、`right`、`bottom` 和 `left` 的值相对于该位置进行偏移。偏移不会影响任何兄弟或父元素的位置。

- `absolute` 当绝对定位时，元素不参与正常布局流。相反，它独立于其兄弟元素进行布局。位置根据 `top`、`right`、`bottom` 和 `left` 的值确定。这些值将使元素相对于其包含块进行定位。

- `static` 当静态定位时，元素根据布局的正常流进行定位，并将忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 还将导致元素不为绝对定位的后代形成包含块，除非存在其他优先的样式属性（例如 `transform`）。这允许 `absolute` 元素被定位到非父元素的东西上。注意：**`static` 仅在新架构上可用**。
...
React Native 中包含块的工作方式与 [它们在 Web 上的工作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block) 非常相似，但由于缺少一些 Web 功能而做了一些简化。
...
查看交互式 [yoga 演练场](https://www.yogalayout.dev/playground)，你可以用它来更好地理解 flexbox。
