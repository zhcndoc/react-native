...
React Native DevTools 由 React Native 内置的专用调试架构提供支持，并使用 [Chrome DevTools](https://developer.chrome.com/docs/devtools) 前端的定制版本。这使我们能够提供熟悉的、与浏览器一致的调试功能，这些功能深度集成并为端到端的可靠性而构建。
...
当发生不可恢复的错误（例如 JavaScript 语法错误）时，LogBox 将打开并显示错误位置。在这种状态下，LogBox 无法关闭，因为你的代码无法执行。一旦语法错误被修复，LogBox 将自动关闭——要么通过快速刷新，要么在手动重新加载后。
