...
更便捷的 [`<FlatList>`](flatlist.md) 和 [`<SectionList>`](sectionlist.md) 组件的基础实现，它们的文档也更完善。通常，只有在你需要比 [`FlatList`](flatlist.md) 提供的灵活性更高时才真正使用这个，例如用于不可变数据而不是普通数组。
...
用于提取指定索引处给定项的唯一键。键用于缓存并作为 React key 跟踪项重新排序。默认提取器检查 `item.key`，然后 `item.id`，然后回退到使用索引，就像 React 一样。
...
当行的可见性变化时调用，由 `viewabilityConfig` prop 定义。
...
参见 `ViewabilityHelper.js` 了解 flow 类型及进一步文档。
...
`ViewabilityConfig`/`onViewableItemsChanged` 对的列表。当对应的 `ViewabilityConfig` 条件满足时，将调用特定的 `onViewableItemsChanged`。参见 `ViewabilityHelper.js` 了解 flow 类型及进一步文档。
...
参数 `offset` 指定要滚动到的偏移量。如果 `horizontal` 为 true，则偏移量为 x 值，在其他任何情况下，偏移量为 y 值。
