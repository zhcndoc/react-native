...
- 大多数情况下，React Native 将使用 [Hermes](hermes)，这是一个为 React Native 优化的开源 JavaScript 引擎。
- 如果禁用了 Hermes，React Native 将使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)，这是驱动 Safari 的 JavaScript 引擎。请注意，在 iOS 上，JavaScriptCore 不使用 JIT，因为 iOS 应用中不存在可写的可执行内存。
...
语法转换器让编写代码更愉快，允许你使用新的 JavaScript 语法，而无需等待所有解释器都支持。

React Native 附带了 [Babel JavaScript 编译器](https://babeljs.io)。查看 [Babel 文档](https://babeljs.io/docs/plugins/#transform-plugins) 了解其支持的转换详情。
