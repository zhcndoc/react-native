...
- `alertTitle` : 显示为通知警报标题的文本。
- `alertBody` : 显示在通知警报中的消息。
- `fireDate` : 通知何时触发。使用 `fireDate` 或 `fireIntervalSeconds` 调度通知，`fireDate` 优先。
...
- `soundName` : 通知触发时播放的声音（可选）。
- `alertAction` : 已弃用。这是用于 iOS 遗留的 UILocalNotification。
- `repeatInterval` : 已弃用。改用 `fireDate` 或 `fireIntervalSeconds`。
