...
可选地提供按钮列表。点击任何按钮将触发相应的 onPress 回调并关闭警告框。默认情况下，唯一的按钮将是一个 'OK' 按钮。

这是一个同时在 Android 和 iOS 上工作的 API，可以显示静态警告框。提示用户输入某些信息的警告框仅在 iOS 上可用。
...
在 Android 上最多可以指定三个按钮。Android 有中性、负面和正面按钮的概念：
...
Android 上的警告框可以通过点击警告框外部来关闭。默认情况下它是禁用的，可以通过提供可选的 [AlertOptions](alert#alertoptions) 参数并将 cancelable 属性设置为 `true` 来启用，即。<br/>`{cancelable: true}`。
...
| callbackOrButtons                                          | 函数<hr/>[AlertButton](alert#alertButton)[] | 如果传递的是函数，当用户点击 'OK' 时，它将带有提示框的值被调用。<br/>`(text: string) => void`。<hr/>如果传递的是数组，按钮将根据数组内容进行配置。 |
| type                                                       | [AlertType](alert#alerttype-ios)                | 这配置了文本输入框。                                                                                                                                                                       |
| defaultValue                                               | string                                          | 文本输入框中的默认文本。                                                                                                                                                                       |
| keyboardType                                               | string                                          | 第一个文本字段（如果存在）的键盘类型。之一是 TextInput [keyboardTypes](textinput#keyboardtype)。                                                                                          |
| options                                                    | [AlertOptions](alert#alertoptions)              | 可选的 Alert 配置。                                                                                                                                                                      |
...
| onDismiss <div className="label android">Android</div>  | function | 当警告框被关闭时触发的回调函数。                                                                    |
