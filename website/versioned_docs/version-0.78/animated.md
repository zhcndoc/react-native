...
> 不要直接修改动画值。您可以使用 [`useRef` Hook](https://react.dev/reference/react/useRef) 返回一个可变的 ref 对象。此 ref 对象的 `current` 属性初始化为给定的参数，并在组件生命周期中持久存在。
...
参阅 [动画](animations#animated-api) 指南以查看 `Animated` 的其他实际示例。

## 概述

您可以将两种值类型与 `Animated` 一起使用：
...
`Animated` 提供三种类型的动画。每种动画类型提供特定的动画曲线，控制您的值如何从初始值动画到最终值：
...
在大多数情况下，您将使用 `timing()`。默认情况下，它使用对称的 easeInOut 曲线，传达物体逐渐加速到全速并通过逐渐减速到停止来结束。

### 使用动画

通过调用动画上的 `start()` 来启动动画。`start()` 接受一个完成回调，该回调将在动画完成时调用。如果动画正常完成，完成回调将使用 `{finished: true}` 调用。如果动画是因为在完成之前调用了 `stop()` 而完成（例如，因为它被手势或另一个动画中断），那么它将收到 `{finished: false}`。
...
通过使用原生驱动，我们在启动动画之前将有关动画的所有内容发送到原生端，允许原生代码在 UI 线程上执行动画，而无需在每一帧上都通过桥接器。一旦动画开始，JS 线程可以被阻塞而不会影响动画。

您可以通过在动画配置中指定 `useNativeDriver: true` 来使用原生驱动。请参阅 [动画](animations#using-the-native-driver) 指南以了解更多。

### 可动画组件

只有可动画组件才能被动画化。这些独特的组件完成了将动画值绑定到属性的魔法，并进行有针对性的原生更新，以避免每一帧上 React 渲染和协调过程的成本。它们还处理卸载时的清理，因此默认情况下是安全的。
...
动画还可以通过将一个动画的 `toValue` 设置为另一个 `Animated.Value` 来链接在一起。请参阅动画指南中的 [跟踪动态值](animations#tracking-dynamic-values)。

默认情况下，如果一个动画停止或中断，则组中的所有其他动画也会停止。

### 组合动画值

您可以通过加法、减法、乘法、除法或取模来组合两个动画值，以制作一个新的动画值：
...
`interpolate()` 函数允许输入范围映射到不同的输出范围。默认情况下，它将外推给定范围之外的曲线，但您也可以让它限制输出值。默认情况下它使用线性插值，但也支持缓动函数。
...
例如，当使用水平滚动手势时，您将执行以下操作以将 `event.nativeEvent.contentOffset.x` 映射到 `scrollX`（一个 `Animated.Value`）：

```tsx
 onScroll={Animated.event(
   // scrollX 对应 e.nativeEvent.contentOffset.x
   [{nativeEvent: {
        contentOffset: {
          x: scrollX
        }
...
当给定值为 ValueXY 而不是 Value 时，每个配置选项可以是形式为 `{x: ..., y: ...}` 的向量，而不是标量。
...
沿定时缓动曲线动画化一个值。[`Easing`](easing) 模块有很多预定义的曲线，或者您可以使用自己的函数。
...
- `velocity`: 附着在弹簧上的物体的初始速度。默认 0（物体处于静止状态）。
- `overshootClamping`: 布尔值，指示弹簧是否应被限制且不反弹。默认 false。
...
创建一个作为提供的 Animated 值的（非负）模的新 Animated 值
...
创建一个限制在 2 个值之间的新 Animated 值。它使用与最后一个值的差值，因此即使值远离边界，当值再次开始接近时它也将开始变化。(`value = clamp(value + diff, min, max)`).
...
同时启动动画数组。默认情况下，如果其中一个动画停止，它们都将停止。您可以使用 `stopTogether` 标志覆盖此行为。
...
动画数组可以并行运行（重叠），但按顺序启动并具有连续的延迟。适合做拖尾效果。
...
连续循环给定的动画，以便每次到达结束时，它重置并从头开始。如果子动画设置为 `useNativeDriver: true`，则循环将不会阻塞 JS 线程。此外，循环可以防止基于 `VirtualizedList` 的组件在动画运行时渲染更多行。您可以在子动画配置中传递 `isInteraction: false` 来修复此问题。
...
用于监视通过 props 传入的动画事件的高级命令式 API。它允许向现有的 `AnimatedEvent` 添加新的 javascript 监听器。如果 `animatedEvent` 是一个 javascript 监听器，它将把 2 个监听器合并为一个，如果 `animatedEvent` 是 null/undefined，它将直接分配 javascript 监听器。尽可能直接使用值。
...
通过调用动画上的 start() 来启动动画。start() 接受一个完成回调，该回调将在动画完成时或动画因在完成之前调用 stop() 而完成时调用。

**参数：**

| 名称     | 类型                                    | 必填 | 描述                                                                                                                                                     |
| -------- | --------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| callback | `(result: {finished: boolean}) => void` | 否       | 动画正常完成运行后或动画因在完成之前调用 stop() 而完成时将调用的函数 |
...
用于将动画值附加到视图事件上的命令式 API。如果可能，建议优先使用带有 `useNativeDriver: true` 的 `Animated.event`。
