...
> **警告：实验性功能。** 可能会经常发生破坏性变更，并且不会可靠地公告。整个功能可能会被删除，谁知道呢？使用风险自负。
...
设置一个函数用于预处理样式属性值。这在内部用于处理颜色和变换值。除非你真的知道自己在做什么并且已经尝试完其他选项，否则不应使用此功能。
...
一个非常常见的模式是使用绝对定位和零定位创建覆盖层（`position: 'absolute', left: 0, right: 0, top: 0, bottom: 0`），因此 `absoluteFill` 可用于提供便利并减少这些重复样式的复制。如果你愿意，可以使用 `absoluteFill` 在 StyleSheet 中创建自定义条目，例如：
...
这个常数将始终是一个整数像素值（因此由它定义的线条看起来可能很清晰），并将尝试匹配底层平台上细线的标准宽度。但是，你不应该依赖它是一个恒定大小，因为在不同的平台和屏幕密度上，其值的计算方式可能不同。
