...
React Native 中的组件使用 [JavaScript 进行样式设置](style)。颜色属性通常与 [Web 上的 CSS 工作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value) 匹配。可以在下面找到各平台颜色使用的通用指南：
...
React Native 拥有多种颜色 API，旨在让您充分利用平台的设计和用户偏好。

- [PlatformColor](platformcolor) 允许您引用平台的颜色系统。
- [DynamicColorIOS](dynamiccolorios) 是 iOS 特有的，允许您指定在浅色模式或深色模式下应使用哪些颜色。

## 颜色表示

### 红绿蓝 (RGB)

React Native 支持十六进制和函数表示法的 `rgb()` 和 `rgba()`：
...
React Native 支持函数表示法的 `hsl()` 和 `hsla()`：
...
:::caution
这看起来可能类似于 Android [Color](https://developer.android.com/reference/android/graphics/Color) 整数的表示形式，但在 Android 上值存储在 SRGB 颜色模式下 (0xaarrggbb)。
:::
...
这是 `rgba(0,0,0,0)` 的简写，与 [CSS3](https://www.w3.org/TR/css-color-3/#transparent) 中的用法相同。
