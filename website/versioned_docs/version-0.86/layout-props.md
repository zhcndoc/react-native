...
以下示例展示了不同属性如何影响或塑造 React Native 布局。你可以尝试在更改属性 `flexWrap` 的值时，例如添加或移除 UI 中的方块。
...
`alignContent` 控制行在交叉方向上的对齐方式，会覆盖父元素的 `alignContent`。
...
`alignItems` 控制子元素在交叉方向上的对齐方式。例如，如果子元素是垂直排列的，`alignItems` 控制它们在水平方向上的对齐方式。它的作用类似于 CSS 中的 `align-items`（默认值：stretch）。
...
`alignSelf` 控制子元素在交叉方向上的对齐方式，会覆盖父元素的 `alignItems`。它的作用类似于 CSS 中的 `align-self`（默认值：auto）。
...
- 对于已设置 flex basis 的节点，如果未设置，宽高比控制节点在交叉轴上的大小
- 对于带有 measure function 的节点，宽高比的作用就像 measure function 在测量 flex basis
- 对于具有 flex grow/shrink 的节点，如果未设置，宽高比控制节点在交叉轴上的大小
- 宽高比会考虑最小/最大尺寸
...
它的作用类似于 CSS 中的 `bottom`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。

更多关于 `bottom` 如何影响布局的详情，请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/bottom)。
...
`boxSizing` 定义元素的各种尺寸属性（`width`、`height`、`minWidth`、`minHeight` 等）如何计算。如果 `boxSizing` 是 `border-box`，这些尺寸适用于元素的边框盒。如果它是 `content-box`，这些尺寸适用于元素的内容盒。默认值是 `border-box`。如果你想进一步了解这个属性的工作方式，[web 文档](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) 是很好的信息来源。
...
`direction` 指定用户界面的方向性流动。默认值是 `inherit`，但根节点除外，其值取决于当前区域设置。
...
在 React Native 中，`flex` 的工作方式与 CSS 不同。`flex` 是一个数字而不是字符串，并且它遵循 [Yoga](https://github.com/facebook/yoga) 布局引擎的规则。

当 `flex` 为正数时，它会让组件具有弹性，并按其 `flex` 值成比例分配大小。因此，`flex` 设置为 `2` 的组件会占据 `flex` 设置为 1 的组件两倍的空间。`flex: <positive number>` 等同于 `flexGrow: <positive number>, flexShrink: 1, flexBasis: 0`。

当 `flex` 为 `0` 时，组件会按照 `width` 和 `height` 进行大小调整，并且不具有弹性。

当 `flex` 为 `-1` 时，组件通常会按照 `width` 和 `height` 进行大小调整。但是，如果空间不足，组件会缩小到其 `minWidth` 和 `minHeight`。
...
`flexBasis` 是一种与轴无关的方式，用于提供项目在主轴上的默认尺寸。设置子元素的 `flexBasis` 类似于在父元素是 `flexDirection: row` 的容器时设置该子元素的 `width`，或者在父元素是 `flexDirection: column` 的容器时设置该子元素的 `height`。项目的 `flexBasis` 是其默认尺寸，即在进行任何 `flexGrow` 和 `flexShrink` 计算之前的尺寸。
...
`flexDirection` 控制容器内子元素的排列方向。`row` 表示从左到右，`column` 表示从上到下，另外两个的含义你大概也能猜到。它的作用类似于 CSS 中的 `flex-direction`，只是默认值是 `column`。
...
`flexGrow` 描述容器内的剩余空间应如何沿主轴分配给其子元素。布局完成后，容器会根据子元素指定的 flex grow 值分配剩余空间。

`flexGrow` 接受任何大于等于 0 的浮点值，默认值为 0。容器会按照子元素的 `flexGrow` 值的权重在子元素之间分配剩余空间。
...
[`flexShrink`](layout-props#flexshrink) 描述在子元素总尺寸超出容器主轴尺寸时，如何沿主轴缩小子元素。`flexShrink` 与 `flexGrow` 非常相似，如果将任何溢出尺寸视为负的剩余空间，它们的理解方式也可以相同。这两个属性也可以很好地协同工作，使子元素能够按需伸缩。

`flexShrink` 接受任何大于等于 0 的浮点值，默认值为 0。容器会按照子元素的 `flexShrink` 值的权重缩小子元素。
...
`flexWrap` 控制子元素在达到 flex 容器末尾后是否可以换行。它的作用类似于 CSS 中的 `flex-wrap`（默认值：nowrap）。

请注意，它不再与 `alignItems: stretch`（默认值）一起工作，因此你可能希望改用例如 `alignItems: flex-start`（破坏性更改详情：https://github.com/facebook/react-native/releases/tag/v0.28.0）。
...
它的作用类似于 CSS 中的 `height`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。
...
`justifyContent` 控制子元素在主方向上的对齐方式。例如，如果子元素是垂直排列的，`justifyContent` 控制它们在垂直方向上的对齐方式。它的作用类似于 CSS 中的 `justify-content`（默认值：flex-start）。
...
它的作用类似于 CSS 中的 `left`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。

更多关于 `left` 如何影响布局的详情，请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/left)。
...
它的作用类似于 CSS 中的 `max-height`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。
...
它的作用类似于 CSS 中的 `max-width`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。
...
它的作用类似于 CSS 中的 `min-height`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。
...
它的作用类似于 CSS 中的 `min-width`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。
...
`overflow` 控制子元素的测量和显示方式。`overflow: hidden` 会使视图被裁剪，而 `overflow: scroll` 会使视图独立于父级主轴进行测量。它的作用类似于 CSS 中的 `overflow`（默认值：visible）。
...
React Native 中的 `position` 与[常规 CSS](https://developer.mozilla.org/en-US/docs/Web/CSS/position) 类似，但默认全部设置为 `relative`。

`relative` 会按照布局的正常流来定位元素。偏移（`top`、`bottom`、`left`、`right`）会相对于该布局进行偏移。

`absolute` 会将元素从布局的正常流中取出。偏移会相对于它的 [包含块](./flexbox.md#the-containing-block) 进行偏移。

`static` 会按照布局的正常流来定位元素。偏移不会产生任何效果。
`static` 元素不会为绝对定位的后代形成包含块。

更多信息请参见 [Flexbox 布局文档](./flexbox.md#position)。另外，[Yoga 文档](https://www.yogalayout.dev/docs/styling/position) 也提供了更多关于 `position` 在 React Native 和 CSS 之间差异的细节。
...
它的作用类似于 CSS 中的 `right`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。

更多关于 `right` 如何影响布局的详情，请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/right)。
...
它的作用类似于 CSS 中的 `top`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。

更多关于 `top` 如何影响布局的详情，请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/top)。
...
它的作用类似于 CSS 中的 `width`，但在 React Native 中必须使用 point 或百分比。Ems 和其他单位不受支持。
...
`zIndex` 控制哪些组件显示在其他组件之上。通常情况下，你不会使用 `zIndex`。组件会按照它们在文档树中的顺序进行渲染，因此后面的组件会覆盖前面的组件。如果你有动画或自定义模态界面，并且不希望出现这种行为，那么 `zIndex` 可能会很有用。

它的作用类似于 CSS 的 `z-index` 属性——具有更大 `zIndex` 的组件会显示在上层。可以把 z 方向想象成从手机指向你的眼睛。

在 iOS 上，`zIndex` 可能要求 `View` 彼此互为兄弟节点才能按预期工作。
