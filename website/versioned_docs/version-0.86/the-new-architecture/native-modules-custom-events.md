...
在深入学习本指南之前，请确保你已经熟悉那份指南，并且可能已经实现了其中的示例。
...
Codegen 作为脚本阶段的一部分运行，该脚本阶段会被 CocoaPods 自动添加到生成的项目中。
...
假设你已经按照 [原生模块指南](/docs/turbo-native-modules-introduction?platforms=android&language=typescript#3-write-application-code-using-the-turbo-native-module) 中描述的 Android 指南完成了操作，那么接下来要做的就是把发送事件的代码接入到你的应用中。
...
假设你已经按照 [原生模块指南](/docs/turbo-native-modules-introduction?platforms=ios&language=typescript#3-write-application-code-using-the-turbo-native-module) 中描述的 iOS 指南完成了操作，那么接下来要做的就是把发送事件的代码接入到你的应用中。
...
`NativeLocalStorageSpecBase` 是一个基类，它提供了 `emitOnKeyAdded` 方法及其基本实现和样板代码。得益于这个类，你不必处理将事件发送到 JS 所需的 Objective-C 与 JSI 之间的所有转换。

对于复杂类型，比如规范中定义的 `KeyValuePair`，Codegen 会在原生侧生成一个你可以填充的通用字典。你有责任确保你在字典中使用的键与 JS 中规范类型里定义的属性相同。
