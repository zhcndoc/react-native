...
纯 C++ Turbo Native Modules 也是 Turbo Native Modules。它们需要一个规范文件（也称 spec 文件），这样 Codegen 才能为我们创建脚手架代码。规范文件也是我们在 JS 中访问 Turbo Native Module 的方式。
...
此配置告诉 Codegen 在 `specs` 文件夹中查找 spec 文件。它还指示 Codegen 只为 `modules` 生成代码，并将生成的代码命名空间设为 `AppSpecs`。

## 3. 编写原生代码

编写 C++ Turbo Native Module 可以让你在 Android 和 iOS 之间共享代码。因此我们将只编写一次代码，然后查看为了让 C++ 代码被识别，我们需要对各个平台做哪些修改。
...
- `NativeSampleModule.h` 文件是纯 C++ TurboModule 的头文件。`include` 语句确保我们包含由 Codegen 创建的规范，其中包含我们需要实现的接口和基类。
- 该模块位于 `facebook::react` 命名空间中，以便访问该命名空间中的所有类型。
- `NativeSampleModule` 类是实际的 Turbo Native Module 类，它继承了 `NativeSampleModuleCxxSpec` 类；该类包含一些胶水代码和样板代码，使这个类能够表现为 Turbo Native Module。
- 最后，我们有构造函数，它接收一个指向 `CallInvoker` 的指针，以便在需要时与 JS 通信，以及我们必须实现的函数原型。

`NativeSampleModule.cpp` 文件是我们 Turbo Native Module 的实际实现，它实现了我们在 spec 中声明的构造函数和方法。

## 4. 在平台中注册模块

接下来的步骤将使我们能够在平台中注册该模块。这一步会将原生代码暴露给 JS，以便 React Native 应用最终可以从 JS 层调用原生方法。

这也是我们唯一一次需要编写一些平台特定代码的地方。
...
# 这个文件包含让你构建 React Native 应用所需的一切
include(${REACT_ANDROID_DIR}/cmake-utils/ReactNative-application.cmake)

# 定义附加源代码所在的位置。我们需要从 jni、main、src、app、android 文件夹一路返回
target_sources(${CMAKE_PROJECT_NAME} PRIVATE ../../../../../shared/NativeSampleModule.cpp)

# 定义 CMake 可以找到附加头文件的位置。同样在这种情况下，我们需要从 jni 文件夹一路返回
target_include_directories(${CMAKE_PROJECT_NAME} PUBLIC ../../../../../shared)
```

这个 CMake 文件执行了以下操作：

- 定义 `appmodules` 库，应用的所有 C++ 代码都将包含在其中。
- 加载 React Native 的基础 CMake 文件。
- 使用 `target_sources` 指令添加我们需要构建的模块 C++ 源代码。默认情况下，React Native 已经会为 `appmodules` 库填充默认源代码，这里我们包含自己的自定义源代码。你可以看到，我们需要从 `jni` 文件夹一路返回到存放 C++ Turbo Module 的 `shared` 文件夹。
- 指定 CMake 可以找到模块头文件的位置。同样，在这种情况下，我们也需要从 `jni` 文件夹一路返回。
...
这个块告诉 Gradle 文件去哪里查找 CMake 文件。路径是相对于 `build.gradle` 文件所在文件夹的，因此我们需要添加指向 `jni` 文件夹中 `CMakeLists.txt` 文件的路径。

#### 3. 注册新的 Turbo Native Module

最后一步是在运行时中注册新的 C++ Turbo Native Module，这样当 JS 请求这个 C++ Turbo Native Module 时，应用就知道去哪里找到它并返回它。
...
std::shared_ptr<TurboModule> cxxModuleProvider(
  ...
  // 这里你可以提供来自
  // 你的应用或外部库的 CXX Turbo Modules。要遵循的方法
  // 类似于下面这样（针对一个名为 `NativeCxxModuleExample` 的模块）：
  //
  // if (name == NativeCxxModuleExample::kModuleName) {
  //   return std::make_shared<NativeCxxModuleExample>(jsInvoker);
  // }

+  // 这段代码会注册该模块，这样当 JS 端请求它时，应用就可以返回它
+  if (name == NativeSampleModule::kModuleName) {
+    return std::make_shared<NativeSampleModule>(jsInvoker);
+  }

  // 然后我们回退到自动链接的 CXX module providers
...
现在，你可以从项目根目录运行 `yarn android`，查看应用是否成功构建。
...
我们需要执行的第一步是每次准备 iOS 应用时都会执行的常规步骤。CocoaPods 是我们用来设置和安装 React Native 依赖的工具，在此过程中，它也会为我们运行 Codegen。
...
这一步会将 `shared` 文件夹添加到项目中，以便 Xcode 可以看到它。
...
要在你的应用中注册纯 Cxx Turbo Native Module，你需要：
...
3. 添加名称 `NativeSampleModuleProvider`（其他字段保持为 `Subclass of: NSObject` 和 `Language: Objective-C`）
4. 点击 Next 生成文件。
...
最后一步是更新 `package.json`，告诉 React Native 原生模块的 JS 规范与这些规范在原生代码中具体实现之间的关联。
...
- `const revString = SampleTurboModule.reverseString(value);` 在 `onPress` 回调中：这就是你在应用中使用 Turbo Native Module 的方式。

:::warning
为了这个示例并尽可能保持简短，我们直接在应用中导入了 spec 文件。
在这种情况下，更好的实践是创建一个单独的文件来封装这些 specs，并在你的应用中使用该文件。
