...
纯 C++ Turbo Native Modules 属于 Turbo Native Modules。它们需要一个规格文件（也称为 spec 文件），这样 Codegen 才能为我们创建脚手架代码。规格文件也是我们在 JS 中访问 Turbo Native Module 的方式。
...
:::warning
所有原生 Turbo Module spec 文件都必须以 `Native` 作为前缀，否则 Codegen 会忽略它们。
:::
...
这段配置告诉 Codegen 去 `specs` 文件夹中查找 spec 文件。它还指示 Codegen 仅为 `modules` 生成代码，并将生成的代码命名空间设为 `AppSpecs`。

## 3. 编写原生代码

编写 C++ Turbo Native Module 可以让你在 Android 和 iOS 之间共享代码。因此，我们只需要编写一次代码，然后再看看需要对各个平台做哪些调整，以便 C++ 代码能够被拾取。
...
- `NativeSampleModule` 类就是实际的 Turbo Native Module 类，它继承自 `NativeSampleModuleCxxSpec` 类；该类包含一些胶水代码和样板代码，使这个类能够作为 Turbo Native Module 工作。
...
接下来的步骤将让我们在平台中注册该模块。这一步会将原生代码暴露给 JS，这样 React Native 应用最终就可以从 JS 层调用原生方法了。

这是我们唯一一次需要编写平台特定代码。

### Android

为了确保 Android 应用可以正常构建 C++ Turbo Native Module，我们需要：
...
# 定义附加源代码所在位置。我们需要从 jni、main、src、app、android 文件夹逐级返回
target_sources(${CMAKE_PROJECT_NAME} PRIVATE ../../../../../shared/NativeSampleModule.cpp)

# 定义 CMake 可以找到附加头文件的位置。同样需要从 jni 文件夹逐级返回
target_include_directories(${CMAKE_PROJECT_NAME} PUBLIC ../../../../../shared)
```

CMake 文件执行以下操作：

- 定义 `appmodules` 库，所有应用的 C++ 代码都将包含在这里。
- 加载 React Native 的基础 CMake 文件。
- 使用 `target_sources` 指令添加我们需要构建的模块 C++ 源代码。默认情况下，React Native 已经会用默认源文件填充 `appmodules` 库，这里我们加入自定义源文件。你可以看到，我们需要从 `jni` 文件夹一路返回到 `shared` 文件夹，也就是 C++ Turbo Module 所在的位置。
- 指定 CMake 可以在哪里找到模块头文件。在这种情况下同样需要从 `jni` 文件夹逐级返回。
...
这个代码块告诉 Gradle 文件去哪里查找 CMake 文件。路径是相对于 `build.gradle` 文件所在文件夹的，所以我们需要添加到 `jni` 文件夹中 `CMakeLists.txt` 文件的路径。

#### 3. 注册新的 Turbo Native Module

最后一步是将新的 C++ Turbo Native Module 注册到运行时中，这样当 JS 请求这个 C++ Turbo Native Module 时，应用就知道去哪里找到它并返回它。
...
这些步骤会从 React Native 下载原始的 `OnLoad.cpp` 文件，这样我们就可以安全地覆盖它，在应用中加载 C++ Turbo Native Module。
...
现在，你可以从项目根目录运行 `yarn android` 来查看你的应用是否成功构建。

### iOS

为了确保 iOS 应用可以正常构建 C++ Turbo Native Module，我们需要：
...
我们需要运行的第一步，就是每次准备 iOS 应用时都会执行的常规步骤。CocoaPods 是我们用来设置和安装 React Native 依赖的工具，在这个过程中，它也会替我们运行 Codegen。
...
这一步会把 `shared` 文件夹添加到项目中，使其在 Xcode 中可见。
...
最后一步是更新 `package.json`，告诉 React Native JS 规格文件中的原生模块名称与原生代码中具体实现之间的对应关系。

将 `package.json` 修改为如下所示：
...
在这种情况下，最佳实践是创建一个单独的文件来封装这些 spec，并在应用中使用那个文件。
这样可以让你为 spec 准备输入，并且在 JS 中对它们拥有更多控制。
:::
