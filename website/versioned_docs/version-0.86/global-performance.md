...
全局 [`performance`](https://developer.mozilla.org/en-US/docs/Web/API/Window/performance) 对象，按 Web 规范所定义。
...
| `executeJavaScriptBundleEntryPointStart` | number \| void | 应用程序包开始执行的时间。 |
...
:::warning 部分支持
提供的是从系统启动到现在的毫秒数，而不是从应用启动到现在的毫秒数。
:::
