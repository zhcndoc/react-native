...
- 如果禁用 Hermes，React Native 会使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)，也就是为 Safari 提供支持的 JavaScript 引擎。请注意，在 iOS 上，由于 iOS 应用中没有可写可执行内存，JavaScriptCore 不使用 JIT。
- 使用 Chrome 调试时，所有 JavaScript 代码都会在 Chrome 自身中运行，并通过 WebSockets 与原生代码通信。Chrome 使用 [V8](https://v8.dev/) 作为其 JavaScript 引擎。

虽然这些环境非常相似，但你仍可能遇到一些不一致之处。最好避免依赖任何运行时的特定实现细节。

## JavaScript 语法转换器

语法转换器让你能够使用新的 JavaScript 语法，而无需等待所有解释器都支持，从而让编码体验更愉快。

React Native 内置了 [Babel JavaScript 编译器](https://babeljs.io)。更多细节请查看 [Babel 文档](https://babeljs.io/docs/plugins/#transform-plugins) 中关于其支持的转换。
