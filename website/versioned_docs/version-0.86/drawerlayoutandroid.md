...
封装平台 `DrawerLayout`（仅限 Android）的 React 组件。Drawer（通常用于导航）通过 `renderNavigationView` 渲染，直接子元素则是主视图（内容放置的位置）。导航视图最初不会显示在屏幕上，但可以从 `drawerPosition` 属性指定的窗口一侧拉出，其宽度可以通过 `drawerWidth` 属性设置。
...
const App = () => {
  ...
  const navigationView = () => (
    <SafeAreaView style={[styles.container, styles.navigationContainer]}>
      <Text style={styles.paragraph}>我在 Drawer 里！</Text>
      <Button
        title="关闭抽屉"
...
- locked-closed，表示抽屉会保持关闭且不响应手势。
- locked-open，表示抽屉会保持打开且不响应手势。抽屉仍然可以通过程序方式打开和关闭（`openDrawer`/`closeDrawer`）。
...
使抽屉占据整个屏幕，并绘制状态栏的背景，以便它能够覆盖状态栏打开。仅在 API 21+ 上生效。
