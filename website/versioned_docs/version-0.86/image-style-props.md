...
为图片设置不透明度值。该数值应在 `0.0` 到 `1.0` 的范围内。
...
当图片具有圆角时，指定 `overlayColor` 会使角落中剩余的空间填充为纯色。这在 Android 对圆角的实现不支持以下情况时非常有用：

- 某些缩放模式，例如 `'contain'`
- 动画 GIF

此属性的一种典型用法是将图片显示在纯色背景上，并将 `overlayColor` 设置为与背景相同的颜色。

有关其底层工作方式的详细信息，请参阅 [Fresco 文档](https://frescolib.org/docs/rounded-corners-and-circles.html)。
...
- `cover`：按比例缩放图片（保持图片宽高比），使得：
  - 图片的两个维度（宽和高）都将等于或大于视图对应的维度（减去内边距）
  - 缩放后图片至少有一个维度将等于视图对应的维度（减去内边距）

- `contain`：按比例缩放图片（保持图片宽高比），使得图片的两个维度（宽和高）都将等于或小于视图对应的维度（减去内边距）。

- `stretch`：独立缩放宽度和高度，这可能会改变 src 的宽高比。

- `repeat`：重复图片以覆盖视图的框架。图片将保持其大小和宽高比，除非它大于视图，在这种情况下会按比例缩小，使其包含在视图中。

- `center`：在宽高两个维度上都将图片居中显示。如果图片大于视图，则按比例缩小，使其包含在视图中。
