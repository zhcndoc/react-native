...
变换是一种样式属性，可帮助你使用 2D 或 3D 变换来修改组件的外观和位置。然而，一旦应用了变换，布局在变换后的组件周围仍保持不变，因此它可能会与附近的组件重叠。你可以为变换后的组件或附近的组件添加 margin，或者为容器添加 padding，以防止此类重叠。
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContentContainer}>
        ...
        <View
          ...
          <Text style={styles.text}>放大 2 倍</Text>
        </View>
        ...
        <View
          ...
          <Text style={styles.text}>X 轴放大 2 倍</Text>
        </View>
        ...
        <View
          ...
          <Text style={styles.text}>Y 轴放大 2 倍</Text>
        </View>
...
`transform` 接受一个由变换对象组成的数组，或者以空格分隔的字符串值。每个对象都指定一个将被变换的属性作为键，以及在变换中使用的值。对象不应组合使用。每个对象只使用一个键/值对。

rotate 变换需要使用字符串，以便将变换表示为角度（deg）或弧度（rad）。例如：
...
skew 变换需要使用字符串，以便将变换表示为角度（deg）。例如：
...
`matrix` 变换接受一个 4x4 的变换矩阵，作为包含 16 个数字的数组。这使你能够在单个操作中应用组合了平移、旋转、缩放和倾斜的复杂变换。
...
:::note
当你需要应用预先计算好的变换矩阵时，例如来自动画库的矩阵，或者在构建 UI 编辑器应用时，矩阵变换会很有用。对于基本变换，建议使用单独的变换属性（scale、rotate、translate 等），因为它们更易读。
:::
...
`transformOrigin` 属性用于设置视图变换的原点。变换原点是应用变换时所围绕的点。默认情况下，变换的原点为 `center`。
...
- 该值必须是 `px`、`percentage`，或者关键字 `left`、`center`、`right`、`top` 和 `bottom` 之一。
...
- 第一个值（x 偏移）必须是 `px`、`percentage`，或者关键字 `left`、`center` 和 `right` 之一。
- 第二个值（y 偏移）必须是 `px`、`percentage`，或者关键字 `top`、`center` 和 `bottom` 之一。
...
`transformOrigin` 也支持数组语法。它使其更方便与 Animated API 配合使用。它还能避免字符串解析，因此效率更高。
...
你可以参考 MDN 关于 [Transform origin](https://developer.mozilla.org/en-US/docs/Web/CSS/transform-origin) 的指南以获取更多信息。
