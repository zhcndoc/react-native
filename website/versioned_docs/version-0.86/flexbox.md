...
:::caution
Flexbox 在 React Native 中的工作方式与其在 Web 端的 CSS 中相同，但有少数例外。
...
[`flex`](layout-props#flex) 将定义你的项目沿主轴如何 **“填充”** 可用空间。空间会根据每个元素的 flex 属性进行分配。

在下面的示例中，红色、橙色和绿色视图都是容器视图的子元素，并且容器设置了 `flex: 1`。红色视图使用 `flex: 1`，橙色视图使用 `flex: 2`，绿色视图使用 `flex: 3`。**1+2+3 = 6**，这意味着红色视图将获得空间的 `1/6`，橙色视图获得空间的 `2/6`，绿色视图获得空间的 `3/6`。
...
[`flexDirection`](layout-props#flexdirection) 控制节点子元素的布局方向。这也称为主轴。交叉轴是与主轴垂直的轴，或者说是折行布局中的行所沿用的轴。
...
布局 [`direction`](layout-props#direction) 指定层级中子元素和文本应如何布局。布局方向也会影响 `start` 和 `end` 分别指向哪个边。默认情况下，React Native 使用 LTR 布局方向。在此模式下，`start` 指向左侧，`end` 指向右侧。

- `LTR`（**默认值**）文本和子元素从左到右布局。应用于元素起始端的外边距和内边距会应用在左侧。

- `RTL` 文本和子元素从右到左布局。应用于元素起始端的外边距和内边距会应用在右侧。
...
[`justifyContent`](layout-props#justifycontent) 描述了如何在容器的主轴上对齐子元素。例如，你可以使用此属性在 `flexDirection` 设置为 `row` 的容器中使子元素水平居中，或者在 `flexDirection` 设置为 `column` 的容器中使子元素垂直居中。
...
- `space-between` 让子元素沿容器主轴均匀分布，并在子元素之间分配剩余空间。

- `space-around` 让子元素沿容器主轴均匀分布，并在子元素周围分配剩余空间。与 `space-between` 相比，使用 `space-around` 会使空间分配到第一个子元素的前面和最后一个子元素的后面。

- `space-evenly` 沿对齐容器的主轴均匀分布子元素。每一对相邻项目之间的间距、主轴起始边与第一个项目之间的间距，以及主轴结束边与最后一个项目之间的间距，三者完全相同。
...
[`alignItems`](layout-props#alignitems) 描述了如何在容器的交叉轴上对齐子元素。它与 `justifyContent` 非常相似，但 `justifyContent` 作用于主轴，而 `alignItems` 作用于交叉轴。
...
- `baseline` 将容器的子元素沿共同的基线对齐。单个子元素可以被设置为其父元素的参考基线。

:::info
要使 `stretch` 生效，子元素在次轴上不能有固定尺寸。在下面的示例中，在从子元素中移除 `width: 50` 之前，设置 `alignItems: stretch` 不会产生任何效果。
:::
...
[`alignSelf`](layout-props#alignself) 的选项和效果与 `alignItems` 相同，但不同于影响容器中的子元素，你可以将此属性应用到单个子元素，以改变它在父容器中的对齐方式。`alignSelf` 会覆盖父级通过 `alignItems` 设置的任何选项。
...
[alignContent](layout-props#aligncontent) 定义了沿着交叉轴的行分布方式。只有在使用 `flexWrap` 让项目换行到多行时，它才会生效。
...
- `space-between` 在容器交叉轴上均匀分布换行后的各行，将剩余空间分配在各行之间。

- `space-around` 在容器交叉轴上均匀分布换行后的各行，将剩余空间分配在各行周围。容器两端的空间大小是项目之间空间的一半。

- `space-evenly` 在容器交叉轴上均匀分布换行后的各行，将剩余空间分配在各行周围。每个间距大小都相同。
...
[`flexWrap`](layout-props#flexwrap) 属性设置在容器上，它控制当子元素在主轴方向上超出容器大小时会发生什么。默认情况下，子元素会被强制放在单行中（这可能会压缩元素）。如果允许换行，项目会在需要时沿主轴方向换成多行。

在换行时，可以使用 `alignContent` 来指定这些行在容器中的排列方式。了解更多 [这里](https://www.yogalayout.dev/docs/styling/flex-wrap)。
...
- [`flexBasis`](layout-props#flexbasis) 是一种与轴无关的方式，用于提供项目在主轴上的默认尺寸。为子元素设置 `flexBasis`，类似于当其父容器的 `flexDirection` 为 `row` 时设置该子元素的 `width`，或者当其父容器的 `flexDirection` 为 `column` 时设置该子元素的 `height`。项目的 `flexBasis` 是该项目的默认尺寸，也就是在执行任何 `flexGrow` 和 `flexShrink` 计算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述容器内沿主轴方向应如何在其子元素之间分配空间。在对子元素完成布局后，容器会根据子元素指定的 `flexGrow` 值分配剩余空间。

  `flexGrow` 接受任何大于等于 0 的浮点值，默认值为 0。容器会按子元素的 `flexGrow` 值加权，在它们之间分配剩余空间。

- [`flexShrink`](layout-props#flexshrink) 描述在子元素总尺寸在主轴方向上超出容器尺寸时，如何沿主轴方向缩小子元素。`flexShrink` 与 `flexGrow` 非常相似，如果将任何溢出的尺寸视为负的剩余空间，也可以用同样的方式来理解。它们也可以很好地协同工作，使子元素能够按需放大和缩小。

  `flexShrink` 接受任何大于等于 0 的浮点值，默认值为 0（在 web 上，默认值为 1）。容器会按子元素的 `flexShrink` 值加权，缩小其子元素。
...
- `auto`（**默认值**）React Native 会根据元素的内容来计算其宽/高，无论内容是其他子元素、文本还是图像。

- `pixels` 以绝对像素定义宽/高。根据组件上设置的其他样式，这可能是也可能不是节点的最终尺寸。
...
- `relative`（**默认值**）默认情况下，元素采用相对定位。这意味着元素会先按照布局的正常流进行定位，然后再根据 `top`、`right`、`bottom` 和 `left` 的值相对于该位置进行偏移。这个偏移不会影响任何兄弟元素或父元素的位置。

- `absolute` 绝对定位时，元素不会参与正常的布局流，而是独立于其兄弟元素进行布局。位置由 `top`、`right`、`bottom` 和 `left` 的值决定。这些值会使元素相对于其包含块进行定位。

- `static` 静态定位时，元素会按照布局的正常流进行定位，并且会忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 还会导致该元素不会为绝对定位的后代形成包含块，除非存在某些其他优先级更高的样式属性（例如 `transform`）。这使得 `absolute` 元素可以相对于不是其父元素的对象进行定位。请注意，**`static` 仅在新架构中可用**。
...
元素的包含块是一个祖先元素，它决定了该元素的位置和尺寸。React Native 中包含块的工作方式与 [Web 上的工作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block) 非常相似，但由于缺少某些 Web 特性，因此做了一些简化。
...
- 如果该元素的 `position` 类型为 `absolute`，则其包含块是最近的祖先元素，并且满足以下条件之一：
...
我们已经介绍了基础知识，但布局中你可能还需要许多其他样式。控制布局的 props 完整列表已在 [此处](./layout-props.md) 记录。
