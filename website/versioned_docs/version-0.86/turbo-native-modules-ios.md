...
现在是时候编写一些 iOS 平台代码，确保应用关闭后 `localStorage` 仍然存在。

## 准备你的 Xcode 项目

我们需要使用 Xcode 来准备你的 iOS 项目。完成这 **6 个步骤** 后，你将拥有实现生成的 `NativeLocalStorageSpec` 接口的 `RCTNativeLocalStorage`。
...
5. 使用 <code>Objective-C</code> 语言，将 Cocoa Touch Class 命名为 <code>RCTNativeLocalStorage</code>。

<img className="half-size" alt="创建一个 Objective-C 的 RCTNativeLocalStorage 类" src="/docs/assets/turbo-native-modules/xcode/5.webp" />
...
然后更新我们的实现，使用带有自定义 [suite name](https://developer.apple.com/documentation/foundation/nsuserdefaults/1409957-initwithsuitename) 的 `NSUserDefaults`。
...
最后一步是更新 `package.json`，以告知 React Native JS 端原生模块的规范与这些规范在原生代码中的具体实现之间的关联。
