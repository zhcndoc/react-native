...
用于使视图正确响应触摸的包装器。按下时，被包装视图的透明度会降低，从而使底层颜色显示出来，使视图变暗或带有色调。

底层颜色来自于将子组件包裹在一个新的 View 中，这可能会影响布局；如果使用不当，有时还会导致不希望出现的视觉瑕疵，例如被包装视图的 backgroundColor 没有明确设置为不透明颜色时。

TouchableHighlight 必须只有一个子元素（不能为 0 个，也不能多于 1 个）。如果你希望包含多个子组件，请将它们包裹在一个 View 中。
...
决定触摸激活时被包装视图的透明度应为何值。该值应介于 0 和 1 之间。默认值为 0.85。需要设置 `underlayColor`。
