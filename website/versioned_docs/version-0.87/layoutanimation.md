...
传递给 `create`、`update` 或 `delete` 的 config 包含以下键：

- `type`，要使用的[动画类型](layoutanimation.md#types)
- `property`，要设置动画的[布局属性](layoutanimation.md#properties)（可选，但建议用于 `create` 和 `delete`）
...
用于创建一个对象（包含 `create`、`update` 和 `delete` 字段）并传递给 [`configureNext`](layoutanimation.md#configurenext) 的辅助方法。`type` 参数是[动画类型](layoutanimation.md#types)，`creationProp` 参数是[布局属性](layoutanimation.md#properties)
...
用于 [`create`](layoutanimation.md#create) 方法，或用于 [`configureNext`](layoutanimation.md#configurenext) 的 `create`／`update`／`delete` 配置中的动画类型枚举（使用示例：`LayoutAnimation.Types.easeIn`）
...
用于 [`create`](layoutanimation.md#create) 方法，或用于 [`configureNext`](layoutanimation.md#configurenext) 的 `create`／`update`／`delete` 配置中的要设置动画的布局属性枚举（使用示例：`LayoutAnimation.Properties.opacity`）
...
要传递给 [`configureNext`](layoutanimation.md#configurenext) 的一组预定义动画配置
