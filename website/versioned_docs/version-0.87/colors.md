...
React Native 中的组件[使用 JavaScript 设置样式](style)。颜色属性通常与 [CSS 在 Web 上的工作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value)一致。下面列出了各个平台上颜色使用方式的常规指南：
...
- [DynamicColorIOS](dynamiccolorios) 专用于 iOS，允许你指定在浅色模式或深色模式下应使用哪些颜色。

## 颜色表示形式

### 红绿蓝（RGB）

React Native 支持以十六进制和函数表示法使用 `rgb()` 和 `rgba()`：
...
React Native 支持以函数表示法使用 `hsl()` 和 `hsla()`：
...
React Native 还支持将颜色表示为 `int` 值（在 RGB 颜色模式下）：

- `0xff00ff00`（0xrrggbbaa）

:::caution
这可能看起来与 Android 的 [Color](https://developer.android.com/reference/android/graphics/Color) 整数表示形式相似，但在 Android 上，值以 SRGB 颜色模式存储（0xaarrggbb）。
:::
