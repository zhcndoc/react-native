...
Element 节点表示 native view tree 中的原生组件（类似于 Web 上的 [Element](https://developer.mozilla.org/en-US/docs/Web/API/Element) 节点）。

所有原生组件以及许多内置组件都通过 refs 提供它们：
...
:::info
请注意，某些内置组件只是其他组件（包括原生组件）的容器。例如，`ScrollView` 内部会渲染一个原生滚动视图和一个原生视图，这些视图可以通过它提供的 ref，使用 `getNativeScrollRef()` 和 `getInnerViewRef()` 等方法访问。
:::
...
- 属性
  ...
  - [`tagName`](https://developer.mozilla.org/en-US/docs/Web/API/Element/tagName)
    - ℹ️ 返回带有 `RN:` 前缀的规范化原生组件名称，例如 `RN:View`。
...
- 属性
  ...
  - [`ownerDocument`](https://developer.mozilla.org/en-US/docs/Web/API/Node/ownerDocument)
    - ℹ️ 返回此组件渲染所在的[文档节点](/docs/next/document-nodes)。
