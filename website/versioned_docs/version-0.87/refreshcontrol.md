...
此组件用于在 ScrollView 或 ListView 内添加下拉刷新功能。当 ScrollView 处于 `scrollY: 0` 时，向下滑动会触发 `onRefresh` 事件。
    ...
    <SafeAreaProvider>
      <SafeAreaView style={styles.container}>
        <ScrollView
          ...
          <Text>向下拉动以查看 RefreshControl 指示器</Text>
        </ScrollView>
      </SafeAreaView>
    </SafeAreaProvider>
  );
...
:::note
`refreshing` 是一个受控属性，因此需要在 `onRefresh` 函数中将其设置为 `true`，否则刷新指示器会立即停止。
:::
