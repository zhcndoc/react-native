...
用于通过键盘向应用输入文本的基础组件。Props 提供了多个功能的配置选项，例如自动纠正、自动大写、占位文本以及不同的键盘类型，例如数字键盘。
...
`TextInput` 默认会在其视图底部显示边框。此边框的内边距由系统提供的背景图像设置，无法更改。要避免此问题，可以不显式设置高度，此时系统会负责在正确的位置显示边框；或者将 `underlineColorAndroid` 设置为透明以不显示边框。

请注意，在 Android 上对输入内容执行文本选择可能会将应用的 activity `windowSoftInputMode` 参数更改为 `adjustResize`。当键盘处于活动状态时，这可能会导致 `position: 'absolute'` 的组件出现问题。要避免此行为，可以在 AndroidManifest.xml（ https://developer.android.com/guide/topics/manifest/activity-element.html ）中指定 `windowSoftInputMode`，或者使用原生代码以编程方式控制此参数。
...
指定字体是否应缩放以遵循辅助功能中的文字大小设置。默认值为 `true`。
...
指定系统的自动完成提示，以便系统提供自动填充。在 Android 上，系统始终会尝试使用启发式方法识别内容类型来提供自动填充。要禁用自动完成，请将 `autoComplete` 设置为 `off`。
...
如果为 `true`，文本字段在提交时会取消聚焦。单行字段的默认值为 true，多行字段的默认值为 false。请注意，对于多行字段，将 `blurOnSubmit` 设置为 `true` 意味着按下回车键会取消字段聚焦并触发 `onSubmitEditing` 事件，而不是在字段中插入换行符。
...
确定文本输入中哪些类型的数据会被转换为可点击的 URL。仅当 `multiline={true}` 且 `editable={false}` 时有效。默认情况下不会检测任何数据类型。
...
提供此属性时，会设置组件中光标（或“插入符号”）的颜色。与 `selectionColor` 的行为不同，光标颜色会独立于文本选择框的颜色进行设置。
...
当值为 `false` 时，如果文本输入周围可用空间较小（例如手机处于横向模式），操作系统可能会让用户在全屏文本输入模式中编辑文本。当值为 `true` 时，此功能会被禁用，用户始终会直接在文本输入框中编辑文本。默认值为 `false`。
...
如果为 `true`，则在没有文本时禁用键盘的回车键，并在有文本时自动启用。默认值为 `false`。
...
告知操作系统，在 Android API Level 26 及更高版本中，应用中的各个字段是否应包含在用于自动填充的视图结构中。可能的值包括 `auto`、`no`、`noExcludeDescendants`、`yes` 和 `yesExcludeDescendants`。默认值为 `auto`。

- `auto`：让 Android System 使用其启发式方法确定该视图对于自动填充是否重要
- `no`：此视图对于自动填充不重要
...
如果已定义，则会在左侧渲染所提供的图像资源。图像资源必须位于 `/android/app/src/main/res/drawable` 中，并按如下方式引用：
...
用于将自定义 [InputAccessoryView](inputaccessoryview.md) 与此文本输入框关联的可选标识符。当此文本输入框获得焦点时，InputAccessoryView 会渲染在键盘上方。
...
用于覆盖默认 [InputAccessoryView](inputaccessoryview.md) 按钮标签的可选标签。
...
其作用类似于 HTML 中的 `inputmode` 属性，用于确定打开哪种键盘，例如 `numeric`，并且优先级高于 `keyboardType`。
...
所有类型的截图请参见[此处](https://davidl.fr/blog/keyboard-react-native-ios-android#all-react-native-keyboard-type-examples-i-os-on-the-left-android-on-the-right)。
...
在 iOS 14 及更高版本上设置换行策略。可能的值包括 `none`、`standard`、`hangul-word` 和 `push-out`。
...
在 iOS 上设置换行模式。可能的值包括 `wordWrapping`、`char`、`clip`、`head`、`middle` 和 `tail`。
...
指定启用 `allowFontScaling` 时字体可以达到的最大缩放比例。可能的值包括：
...
限制可输入字符的最大数量。使用此属性代替在 JS 中实现逻辑，可以避免闪烁。
...
:::note
需要注意的是，在 iOS 上，这会将文本对齐到顶部，而在 Android 上会将文本居中。结合将 `textAlignVertical` 设置为 `top` 使用，可以使两个平台的行为一致。
:::
...
设置 `TextInput` 的最大行数。与设置为 `true` 的 multiline 一起使用，才能填充这些行。
...
:::note
如果你尝试从 `nativeEvent` 访问 `text` 值，请注意，获取到的结果可能为 `undefined`，从而导致意外错误。如果你想查找 TextInput 的最后一个值，可以使用 [`onEndEditing`](textinput#onendediting) 事件，该事件会在编辑完成时触发。
:::
...
文本输入框的文本发生变化时调用的回调。变化后的文本会作为单个字符串参数传递给回调处理函数。
...
按下按键时调用的回调。对于相应的按键，此回调接收的对象中 `keyValue` 将为 `'Enter'` 或 `'Backspace'`，对于其他按键则为输入的字符，包括空格 `' '`。该回调会在 `onChange` 回调之前触发。注意：在 Android 上，仅处理软键盘的输入，不处理硬件键盘的输入。
...
内容滚动时调用。也可能包含来自 `ScrollEvent` 的其他属性，但在 Android 上出于性能原因不会提供 `contentSize`。
...
将回车键设置为指定标签。使用它代替 `returnKeyType`。
...
确定回车键的外观。在 Android 上也可以使用 `returnKeyLabel`。
...
如果为 `true`，则允许 TextInput 将触摸事件传递给父组件。这使得诸如 SwipeableListView 之类的组件可以从 iOS 上的 TextInput 处进行滑动，就像 Android 默认支持的行为一样。如果为 `false`，TextInput 始终会请求处理输入（禁用时除外）。默认值为 `true`。
...
设置 `TextInput` 的行数。与设置为 `true` 的 multiline 一起使用，才能填充这些行。
...
文本输入框选择内容的起始位置和结束位置。将 start 和 end 设置为相同的值即可定位光标。
...
设置选择手柄的颜色。与 `selectionColor` 不同，它允许独立于选择内容的颜色自定义选择手柄颜色。
...
如果为 `false`，则在字段获得焦点时阻止显示软键盘。默认值为 `true`。
...
如果为 `false`，则 iOS 系统不会在粘贴操作后插入额外空格，也不会在剪切或删除操作后删除一个或两个空格。
...
向键盘和系统提供用户输入内容的预期语义含义。

:::note
[`autoComplete`](#autocomplete) 提供相同的功能，并且适用于所有平台。你可以使用 [`Platform.select`](/docs/next/platform#select) 来实现不同平台上的不同表现。
...
`newPassword` 可用于表示用户可能希望保存到钥匙串中的新密码输入，`oneTimeCode` 可用于表示字段可以通过 SMS 中收到的验证码进行自动填充。
...
在 iOS 上使用 `textContentType` 的 `newPassword` 时，可以将密码的最低要求告知操作系统，以便系统生成符合要求的密码。要创建适用于 `PasswordRules` 的有效字符串，请参阅 [Apple Docs](https://developer.apple.com/password-rules/)。

:::tip
如果没有出现密码生成对话框，请确认：

- 已启用 AutoFill：**设置** → **密码与账户** → 将**自动填充密码**切换为“开”
- 使用了 iCloud Keychain：**设置** → **Apple ID** → **iCloud** → **钥匙串** → 将 **iCloud 钥匙串**切换为“开”
  :::
...
请注意，并非所有 Text 样式都受支持，以下是不受支持属性的不完整列表：
...
在 Android API Level 23 及更高版本上设置文本换行策略，可能的值包括 `simple`、`highQuality`、`balanced`。默认值为 `highQuality`。
...
要在文本输入框中显示的值。`TextInput` 是受控组件，这意味着如果提供了此 value prop，原生值将被强制匹配该值。对于大多数用法来说，这种方式效果很好，但在某些情况下可能会导致闪烁——常见原因之一是通过保持 value 不变来阻止编辑。除了设置相同的值之外，还可以设置 `editable={false}`，或者设置／更新 `maxLength`，以在避免闪烁的情况下阻止不需要的编辑。
