...
- `getModule` 方法根据 React Native 需要渲染的视图返回相应的 ViewManager。
- `getReactModuleInfoProvider` 提供在运行时注册模块时所需的全部信息，
