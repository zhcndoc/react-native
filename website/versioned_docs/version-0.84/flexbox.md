...
组件可以使用 Flexbox 算法来指定其子元素的布局。Flexbox 的设计目标是在不同屏幕尺寸上提供一致的布局。

通常你会结合使用 `flexDirection`、`alignItems` 和 `justifyContent` 来实现合适的布局。

:::caution
Flexbox 在 React Native 中的工作方式与它在 Web 端 CSS 中的工作方式相同，但有少数例外。
...
[`flex`](layout-props#flex) 将定义你的元素如何沿主轴 **“填充”** 可用空间。空间会根据每个元素的 flex 属性进行分配。

在下面的示例中，红色、橙色和绿色视图都是容器视图的子元素，该容器设置了 `flex: 1`。红色视图使用 `flex: 1`，橙色视图使用 `flex: 2`，绿色视图使用 `flex: 3`。**1+2+3 = 6**，这意味着红色视图将获得 `1/6` 的空间，橙色视图获得 `2/6` 的空间，绿色视图获得 `3/6` 的空间。
...
[`flexDirection`](layout-props#flexdirection) 控制节点子元素的布局方向。这也被称为主轴。交叉轴是与主轴垂直的轴，或者说是换行线布局所在的轴。

- `column` (**默认值**) 将子元素从上到下排列。如果启用了换行，则下一行会从容器顶部第一个项目的右侧开始。

- `row` 将子元素从左到右排列。如果启用了换行，则下一行会从容器左侧第一个项目的下方开始。

- `column-reverse` 将子元素从下到上排列。如果启用了换行，则下一行会从容器底部第一个项目的右侧开始。

- `row-reverse` 将子元素从右到左排列。如果启用了换行，则下一行会从容器右侧第一个项目的下方开始。
...
布局 [`direction`](layout-props#direction) 指定层级中子元素和文本的布局方向。布局方向也会影响 `start` 和 `end` 所指向的边。默认情况下，React Native 使用 LTR 布局方向进行布局。在这种模式下，`start` 指向左侧，`end` 指向右侧。

- `LTR` (**默认值**) 文本和子元素从左到右布局。应用在元素起始位置的 margin 和 padding 会作用在左侧。

- `RTL` 文本和子元素从右到左布局。应用在元素起始位置的 margin 和 padding 会作用在右侧。
...
[`justifyContent`](layout-props#justifycontent) 描述了如何在容器的主轴上对齐子元素。例如，你可以使用这个属性，在 `flexDirection` 设为 `row` 的容器中水平居中子元素，或在 `flexDirection` 设为 `column` 的容器中垂直居中子元素。

- `flex-start`（**默认值**）将容器中的子元素对齐到容器主轴的起始位置。

- `flex-end` 将容器中的子元素对齐到容器主轴的末尾位置。

- `center` 将容器中的子元素对齐到容器主轴的中心位置。

- `space-between` 在容器主轴上均匀分布子元素，将剩余空间分配到子元素之间。

- `space-around` 在容器主轴上均匀分布子元素，将剩余空间分配到子元素周围。与 `space-between` 相比，使用 `space-around` 会使空间分配到第一个子元素的开头以及最后一个子元素的末尾。

- `space-evenly` 沿主轴在对齐容器内均匀分配子元素。每一对相邻项目之间的间距、主轴起始边与第一个项目之间的间距，以及主轴末尾边与最后一个项目之间的间距，全部完全相同。
...
[`alignItems`](layout-props#alignitems) 描述了如何沿容器的交叉轴对齐子元素。它与 `justifyContent` 非常相似，但不是作用于主轴，而是作用于交叉轴。

- `stretch`（**默认值**）将容器中的子元素拉伸以匹配容器交叉轴的 `height`。

- `flex-start` 将容器中的子元素对齐到容器交叉轴的起始位置。

- `flex-end` 将容器中的子元素对齐到容器交叉轴的末尾位置。

- `center` 将容器中的子元素对齐到容器交叉轴的中心位置。

- `baseline` 沿共同的基线对齐容器中的子元素。单个子元素可以被设置为其父元素的参考基线。

:::info
要让 `stretch` 生效，子元素在次轴上不能有固定尺寸。在下面的示例中，只有移除子元素上的 `width: 50` 之后，设置 `alignItems: stretch` 才会生效。
:::
...
[`alignSelf`](layout-props#alignself) 与 `alignItems` 具有相同的选项和效果，但不同的是，`alignItems` 作用于容器内的子元素，而你可以将此属性应用于单个子元素，以改变其在父元素中的对齐方式。`alignSelf` 会覆盖父元素通过 `alignItems` 设置的任何选项。
...
[alignContent](layout-props#aligncontent) 定义了沿交叉轴的各行分布方式。只有当项目使用 `flexWrap` 换行到多行时，这个属性才会生效。
...
- `center` 将换行后的各行居中对齐于容器交叉轴。

- `space-between` 在容器交叉轴上均匀分布换行后的各行，在各行之间分配剩余空间。

- `space-around` 在容器交叉轴上均匀分布换行后的各行，在各行周围分配剩余空间。容器两端的空间大小是项目之间空间的一半。

- `space-evenly` 在容器交叉轴上均匀分布换行后的各行，在各行周围分配剩余空间。每个间距大小都相同。
...
[`flexWrap`](layout-props#flexwrap) 属性设置在容器上，用于控制当子元素沿主轴超出容器尺寸时会发生什么。默认情况下，子元素会被强制放在同一行中（这可能会使元素缩小）。如果允许换行，则在需要时，项目会沿主轴换成多行。

当发生换行时，可以使用 `alignContent` 来指定这些行在容器中的排列方式。更多信息请参见[这里](https://www.yogalayout.dev/docs/styling/flex-wrap)。
...
- [`flexBasis`](layout-props#flexbasis) 是一种与轴无关的方式，用于提供项目沿主轴的默认尺寸。为子元素设置 `flexBasis`，类似于当其父元素是 `flexDirection: row` 的容器时设置该子元素的 `width`，或者当其父元素是 `flexDirection: column` 的容器时设置该子元素的 `height`。项目的 `flexBasis` 是该项目的默认尺寸，也就是在执行任何 `flexGrow` 和 `flexShrink` 计算之前的尺寸。

- [`flexGrow`](layout-props#flexgrow) 描述了容器内沿主轴应如何在子元素之间分配空间。在对其子元素完成布局后，容器会根据子元素指定的 `flexGrow` 值分配任何剩余空间。

  `flexGrow` 接受任何大于等于 0 的浮点值，默认值为 0。容器会按照子元素的 `flexGrow` 值加权分配剩余空间。

- [`flexShrink`](layout-props#flexshrink) 描述了在子元素总尺寸沿主轴超出容器主轴尺寸时，如何沿主轴缩小子元素。`flexShrink` 与 `flexGrow` 非常相似，如果将任何溢出尺寸视为负的剩余空间，也可以用相同的方式理解这两个属性。这两个属性也能很好地协同工作，使子元素按需放大和缩小。

  `flexShrink` 接受任何大于等于 0 的浮点值，默认值为 0（在 web 上，默认值为 1）。容器会按照子元素的 `flexShrink` 值加权缩小子元素。

你可以在[这里](https://www.yogalayout.dev/docs/styling/flex-basis-grow-shrink)了解更多信息。
...
- [`rowGap`](layout-props#rowgap) 设置元素各行之间的间隙（gutter）大小。

- [`columnGap`](layout-props#columngap) 设置元素各列之间的间隙（gutter）大小。

- [`gap`](layout-props#gap) 设置行与列之间的间隙（gutter）大小。它是 `rowGap` 和 `columnGap` 的简写。
...
- `auto`（**默认值**）React Native 会根据元素的内容计算其宽度/高度，无论内容是其他子元素、文本还是图像。
...
元素的 `position` 类型定义了它相对于自身、父元素，或其 [包含块](./flexbox.md#the-containing-block) 的定位方式。

- `relative`（**默认值**）默认情况下，元素采用相对定位。这意味着元素会先按照布局的正常流进行定位，然后再根据 `top`、`right`、`bottom` 和 `left` 的值，相对于该位置进行偏移。这个偏移不会影响任何兄弟元素或父元素的位置。

- `absolute` 当采用绝对定位时，元素不参与正常的布局流，而是独立于其兄弟元素进行布局。其位置根据 `top`、`right`、`bottom` 和 `left` 的值来决定。这些值会将元素相对于其包含块进行定位。

- `static` 当采用静态定位时，元素会按照布局的正常流进行定位，并且会忽略 `top`、`right`、`bottom` 和 `left` 的值。此 `position` 还会导致该元素不会为绝对定位的后代形成包含块，除非存在某个其他优先级更高的样式属性（例如 `transform`）。这使得 `absolute` 元素可以相对于不是其父元素的对象进行定位。请注意，**`static` 仅在新架构中可用**。
...
元素的包含块是一个祖先元素，它控制该元素的位置和大小。
React Native 中包含块的工作方式与 [Web 上的工作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/Containing_block) 非常相似，只是由于缺少一些 Web 特性而做了一些简化。

绝对定位元素的 `top`、`right`、`bottom` 和 `left` 值将相对于其包含块。

应用于绝对定位元素的百分比长度（例如：`width: '50%'` 或 `padding: '10%'`）将相对于其包含块的大小进行计算。例如，如果包含块宽度为 100 点，那么绝对定位元素上的 `width: 50%` 将使其宽度为 50 点。
...
查看交互式 [yoga playground](https://www.yogalayout.dev/playground)，你可以借此更好地理解 flexbox。

我们已经介绍了基础知识，但布局可能还需要许多其他样式。控制布局的完整属性列表文档在 [这里](./layout-props.md)。

另外，你还可以查看来自 [Wix 工程师](https://medium.com/wix-engineering/the-full-react-native-layout-cheat-sheet-a4147802405c) 的一些示例。
