...
:::info
关于这些属性的更多详细示例可以在 [Flexbox 布局](flexbox) 页面中找到。
:::

### 示例

下面的示例展示了不同属性如何影响或塑造 React Native 布局。你可以尝试在更改属性 `flexWrap` 的值时，向 UI 中添加或移除方块。
...
- 对于带有 measure 函数的节点，宽高比的工作方式与 measure 函数测量 flex basis 类似
- 对于具有 flex grow/shrink 的节点，如果未设置，则宽高比控制其在交叉轴上的大小
- 宽高比会考虑最小/最大尺寸
...
它的作用类似于 CSS 中的 `bottom`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。

关于 `bottom` 如何影响布局的更多细节，请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/bottom)。
...
`boxSizing` 定义元素的各种尺寸属性（`width`、`height`、`minWidth`、`minHeight` 等）如何计算。如果 `boxSizing` 为 `border-box`，这些尺寸适用于元素的边框盒；如果为 `content-box`，则适用于元素的内容盒。默认值是 `border-box`。如果你想了解此属性的更多工作方式，`[web documentation](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing)` 是一个很好的信息来源。
...
`direction` 指定用户界面的方向流。默认值为 `inherit`，但根节点除外，根节点的值将根据当前区域设置而定。
...
在 React Native 中，`flex` 的工作方式与 CSS 中不同。`flex` 是数字而不是字符串，并且它按照 [Yoga](https://github.com/facebook/yoga) 布局引擎的规则工作。

当 `flex` 为正数时，它会使组件具有弹性，并按其 flex 值的比例进行尺寸分配。因此，`flex` 设置为 `2` 的组件会占用 `flex` 设置为 `1` 的组件两倍的空间。`flex: <positive number>` 等同于 `flexGrow: <positive number>, flexShrink: 1, flexBasis: 0`。

当 `flex` 为 `0` 时，组件会根据 `width` 和 `height` 进行尺寸计算，并且不具有弹性。

当 `flex` 为 `-1` 时，组件通常会根据 `width` 和 `height` 进行尺寸计算。但是，如果空间不足，组件会缩小到其 `minWidth` 和 `minHeight`。
...
`flexBasis` 是一种与轴无关的方式，用于提供项目在主轴上的默认尺寸。为子元素设置 `flexBasis` 类似于：当其父元素是 `flexDirection: row` 的容器时设置该子元素的 `width`，或当其父元素是 `flexDirection: column` 的容器时设置该子元素的 `height`。项目的 `flexBasis` 是该项目的默认尺寸，即在执行任何 `flexGrow` 和 `flexShrink` 计算之前的尺寸。
...
`flexDirection` 控制容器中子元素的排列方向。`row` 表示从左到右，`column` 表示从上到下，另外两个你应该也能猜到。它的作用类似于 CSS 中的 `flex-direction`，只是默认值是 `column`。
...
`flexGrow` 描述容器内的剩余空间应如何沿主轴分配给其子元素。布局完成后，容器会根据子元素指定的 flex grow 值分配任何剩余空间。

`flexGrow` 接受任何大于等于 0 的浮点数，默认值为 0。容器会根据子元素的 `flexGrow` 值按权重分配剩余空间。
...
[`flexShrink`](layout-props#flexshrink) 描述的是在主轴上子元素总尺寸超过容器主轴尺寸时，如何缩小子元素。`flexShrink` 与 `flexGrow` 非常相似，如果将任何溢出的尺寸视为负的剩余空间，也可以用同样的方式来理解。这两个属性还可以很好地协同工作，使子元素能够按需伸缩。

`flexShrink` 接受任何大于等于 0 的浮点数，默认值为 0。容器会根据子元素的 `flexShrink` 值按权重缩小其子元素。
...
请注意，它不再与 `alignItems: stretch`（默认值）配合使用，因此你可能希望改用 `alignItems: flex-start`，例如（破坏性变更详情：https://github.com/facebook/react-native/releases/tag/v0.28.0）。
...
它的作用类似于 CSS 中的 `height`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。
...
它的作用类似于 CSS 中的 `left`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。

关于 `left` 如何影响布局的更多细节，请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/left)。
...
它的作用类似于 CSS 中的 `max-height`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。
...
它的作用类似于 CSS 中的 `max-width`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。
...
它的作用类似于 CSS 中的 `min-height`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。
...
它的作用类似于 CSS 中的 `min-width`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。
...
设置 `paddingHorizontal` 的效果与同时设置 `paddingLeft` 和 `paddingRight` 相同。
...
设置 `paddingVertical` 的效果与同时设置 `paddingTop` 和 `paddingBottom` 相同。
...
React Native 中的 `position` 类似于 [普通 CSS](https://developer.mozilla.org/en-US/docs/Web/CSS/position)，但默认情况下所有元素都设置为 `relative`。

`relative` 会按照布局的正常流来定位元素。内边距（`top`、`bottom`、`left`、`right`）会相对于此布局进行偏移。

`absolute` 会将元素从布局的正常流中移除。内边距会相对于其 [包含块](./flexbox.md#the-containing-block) 进行偏移。

`static` 会按照布局的正常流来定位元素。内边距不会产生任何影响。
`static` 元素不会为绝对定位的后代形成包含块。

更多信息请参见 [Flexbox 布局文档](./flexbox.md#position)。另外，[Yoga 文档](https://www.yogalayout.dev/docs/styling/position) 中也有关于 `position` 在 React Native 与 CSS 之间差异的更多细节。
...
它的作用类似于 CSS 中的 `right`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。

关于 `right` 如何影响布局的更多细节，请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/right)。
...
它的作用类似于 CSS 中的 `top`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。

关于 `top` 如何影响布局的更多细节，请参见 [MDN CSS Reference](https://developer.mozilla.org/en-US/docs/Web/CSS/top)。
...
它的作用类似于 CSS 中的 `width`，但在 React Native 中必须使用点或百分比。Ems 和其他单位不受支持。
...
`zIndex` 控制哪些组件显示在其他组件之上。通常你不需要使用 `zIndex`。组件会按照它们在文档树中的顺序进行渲染，因此后面的组件会绘制在前面的组件之上。如果你有动画或自定义模态界面，并且不希望出现这种行为，`zIndex` 可能会很有用。

它的作用类似于 CSS 的 `z-index` 属性——具有更大 `zIndex` 的组件会显示在上方。可以把 z 方向想象成从手机指向你的眼睛。

在 iOS 上，`zIndex` 可能需要 `View` 彼此为兄弟节点才能按预期工作。
