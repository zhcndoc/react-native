...
查看 [`TextInput` 文档](textinput.md) 以了解更多配置选项。
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.description}>
          这个演示展示了如何使用可用的 TextInput 自定义配置，让表单更易于使用。试着完成表单，并注意不同字段具有特定的优化，以及回车键会从聚焦下一个输入项变为提交表单。
        </Text>
      </View>
      <TextInput
        style={styles.input}
...
软件键盘会占据屏幕将近一半。如果你有可能被键盘遮挡的交互元素，请使用 [`KeyboardAvoidingView` 组件](keyboardavoidingview.md) 确保它们仍可访问。
...
const App = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.description}>
          这个演示展示了如何避免重要的 UI 元素被软件键盘遮挡。聚焦下面的电子邮件输入框，并注意“注册”按钮和文本已经调整了位置，以确保它们不会被键盘隐藏。
        </Text>
      </View>
      <KeyboardAvoidingView behavior="padding" style={styles.form}>
        ...
        <View>
          ...
          <Text style={styles.legal}>此处是一些重要的法律免责声明</Text>
        </View>
      </KeyboardAvoidingView>
    </View>
  );
...
在手机上按按钮时，很难做到非常精确。请确保所有可交互元素的尺寸至少为 44x44。实现这一点的一种方法是为元素留出足够空间，`padding`、`minWidth` 和 `minHeight` 这些样式值会很有用。或者，你也可以使用 [`hitSlop` prop](touchablewithoutfeedback.md#hitslop) 在不影响布局的情况下增加可交互区域。下面是一个演示：
...
Android API 21+ 使用 Material Design 的 ripple 效果，在用户触摸屏幕上的可交互区域时提供反馈。React Native 通过 [`TouchableNativeFeedback` 组件](touchablenativefeedback.md) 暴露了这一能力。使用这种触摸效果而不是 opacity 或 highlight，通常会让你的应用在该平台上更贴合系统风格。话虽如此，使用它时需要小心，因为它在 iOS 上以及 Android API < 21 上都无法工作，因此你需要在 iOS 上回退使用其他 Touchable 组件之一。你可以使用类似 [react-native-platform-touchable](https://github.com/react-community/react-native-platform-touchable) 的库来为你处理平台差异。
    ...
    <TouchableNativeFeedback
      ...
      <View style={styles.button}>
        <Text style={styles.text}>
          这是不带边框的 ripple，这对图标更有用，例如在标签栏中
        </Text>
      </View>
    </TouchableNativeFeedback>
  </View>
...
const Buttons = () => (
  <View style={styles.buttonContainer}>
    <TouchableOpacity
      ...
      onPress={noop}
      hitSlop={defaultHitSlop}>
      <Text style={styles.text}>这是 opacity 效果</Text>
    </TouchableOpacity>
    <TouchableHighlight
      style={styles.button}
      onPress={noop}
      ...
      <Text style={styles.text}>这是 highlight 效果</Text>
    </TouchableHighlight>
  </View>
);
