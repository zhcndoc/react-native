...
:::note[关于安全性的注意事项]
如果您不希望以明文存储密码，并且您使用的是 macOS，也可以 [将凭据存储在 Keychain Access 应用中](https://pilloxa.gitlab.io/posts/safer-passwords-in-gradle/)。这样，您就可以跳过 `~/.gradle/gradle.properties` 中最后两行。
:::
...
:::caution[重要]
如果您已启用 Proguard，请务必彻底测试您的应用。Proguard 往往需要针对您使用的每个原生库进行特定配置。请参阅 `app/proguard-rules.pro`。
:::
