...
在 `android/src/main/java/com/webview` 文件夹中创建一个 `ReactWebView.java` 或 `ReactWebView.kt` 文件，内容如下：
...
该类定义了三个 Android 构造函数，但将实际初始化逻辑委托给了私有的 `configureComponent` 函数。这个函数负责初始化组件特定的属性：在这个例子中，设置了 `WebView` 的布局，并定义了自定义 `WebView` 行为的 `WebClient`。代码中，`ReactWebView` 在页面加载完成时通过实现 `WebClient` 的 `onPageFinished` 方法发出事件。
