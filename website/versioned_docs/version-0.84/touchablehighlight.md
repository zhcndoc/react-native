...
这是一个包装器，用于让视图正确响应触摸。按下时，被包裹视图的透明度会降低，从而让底层颜色透出来，使视图变暗或带有色调。

底层颜色来自于将子元素包裹在一个新的 View 中，这可能会影响布局，并且如果使用不当，有时会导致不想要的视觉瑕疵，例如当被包裹视图的 backgroundColor 没有显式设置为不透明颜色时。

TouchableHighlight 必须有一个子元素（不能是 0 个或多于 1 个）。如果你希望有多个子组件，请将它们包裹在一个 View 中。
...
决定触摸激活时被包裹视图的透明度应是多少。该值应介于 0 和 1 之间。默认为 0.85。需要设置 `underlayColor`。
