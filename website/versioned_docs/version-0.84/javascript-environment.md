...
在使用 React Native 时，你的 JavaScript 代码最多会在三种环境中运行：

- 在大多数情况下，React Native 会使用 [Hermes](hermes)，这是一个为 React Native 优化的开源 JavaScript 引擎。
- 如果禁用 Hermes，React Native 会使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)，也就是为 Safari 提供支持的 JavaScript 引擎。请注意，在 iOS 上，由于 iOS 应用中缺少可写可执行内存，JavaScriptCore 不会使用 JIT。
- 在使用 Chrome 调试时，所有 JavaScript 代码都会在 Chrome 自身中运行，并通过 WebSockets 与原生代码通信。Chrome 使用 [V8](https://v8.dev/) 作为其 JavaScript 引擎。

虽然这些环境非常相似，但你仍可能遇到一些不一致之处。最好避免依赖任何运行时的特定细节。

## JavaScript 语法转换器

语法转换器通过允许你使用新的 JavaScript 语法，而不必等待所有解释器都支持，从而让编写代码变得更愉快。
...
React Native 已启用转换的完整列表可在 [@react-native/babel-preset](https://github.com/facebook/react-native/tree/main/packages/react-native-babel-preset) 中找到。
...
<tbody>
  ...
  <TableRow name="粘性正则表达式" code="const a = /o+/y;" url="https://babeljs.io/docs/en/babel-plugin-transform-sticky-regex" />
  ...
  <TableRow name="可选的 catch 绑定" code="try {throw 0; } catch { doSomethingWhichDoesNotCareAboutTheValueThrown();}" url="https://babeljs.io/docs/en/babel-plugin-proposal-optional-catch-binding" />
  ...
  <TableRow name="从默认导出" code="export v from 'mod';" url="https://babeljs.io/docs/en/babel-plugin-proposal-export-default-from" />
