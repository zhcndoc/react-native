...
强制应用使用 RTL 布局，而不管设备语言设置如何。这主要用于在开发期间测试 RTL 布局。

不要在生产应用中强制使用 RTL，因为它需要完全重启应用后才会生效，这会带来较差的用户体验。
...
- 该设置会在应用重启后保留
- 仅用于开发和测试。在生产环境中，你应当完全禁止 RTL，或对其进行适当处理（参见 `isRTL`）
