...
> **已弃用。** 请使用 [`animationType`](modal.md#animationtype) 属性代替。
...
Modal 的 `backdropColor`（或 modal 容器的背景颜色）。如果未提供且 transparent 为 `false`，则默认为 `white`。如果 `transparent` 为 `true`，则忽略此属性。
