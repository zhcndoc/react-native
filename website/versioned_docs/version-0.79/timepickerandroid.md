...
返回一个 Promise，如果用户选择了时间，它将解析为一个包含 `action`、`hour` (0-23)、`minute` (0-59) 的对象。如果用户关闭了对话框，Promise 仍会被解决，其中 action 为 `TimePickerAndroid.dismissedAction`，所有其他键为 undefined。在读取值之前，**始终**检查 `action`。
