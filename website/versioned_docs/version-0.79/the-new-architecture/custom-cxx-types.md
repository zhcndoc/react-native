...
第一个类型是 `Address` 的定义。第二个类型包含将所有基础设施从 JS 桥接到 C++ 以及反之亦然所需的内容。我们需要添加的唯一额外步骤是定义扩展 `NativeSampleModuleAddressBridging` 类型的 `Bridging` 结构。
