...
在本指南中，你将探索如何使用 Swift 编写你的原生模块。

:::note
React Native 的核心主要是用 C++ 编写的，尽管 Apple 开发了 [互操作层](https://www.swift.org/documentation/cxx-interop/)，但 Swift 和 C++ 之间的互操作性并不好。

因此，由于语言之间的不兼容性，本指南中你要编写的模块不会是纯 Swift 实现。你必须编写一些 Objective-C++ 胶水代码，但本指南的目标是最小化所需的 Objective-C++ 代码量。如果你正在将现有的原生模块从旧架构迁移到新架构，这种方法应该允许你重用大部分代码。
:::

本指南从 [原生模块](/docs/next/turbo-native-modules-introduction) 指南的 iOS 实现开始。
在深入本指南之前，请确保熟悉该指南，最好实现该指南中的示例。

## 适配器模式

目标是使用 Swift 模块实现所有业务逻辑，并在 Objective-C++ 中拥有一个薄的胶水层，能够连接应用与 Swift 实现。

你可以利用 [适配器](https://en.wikipedia.org/wiki/Adapter_pattern) 设计模式来实现这一点，将 Swift 模块与 Objective-C++ 层连接起来。

Objective-C++ 对象由 React Native 创建，它保持对 Swift 模块的引用，处理其生命周期。Objective-C++ 对象将所有方法调用转发给 Swift。
...
1. 在 Xcode 项目中创建一个新空文件，命名为 `NativeLocalStorage.swift`
...
请注意，你必须将所有需要从 Objective-C 调用的方法声明为 `public` 并带有 `@objc` 注解。
还要记得让你的类继承自 `NSObject`，否则无法从 Objective-C 使用它。
...
记得导入 `"SampleApp-Swift.h"` 头文件。这是 Xcode 自动生成的头文件，包含你的 Swift 文件的公共 API，格式可供 Objective-C 使用。头文件中的 `SampleApp` 部分实际上是你的应用名称，所以如果你创建的应用名称**不同于** `SampleApp`，你必须更改它。
...
这种方法在接口中引入了一些代码重复，但它允许你重用代码库中可能已经拥有的 Swift 代码，只需很少的额外工作。
...
1. 在 Xcode 中，创建一个新文件并命名为 `"SampleApp-Bridging-Header.h"`
2. 更新 `"SampleApp-Bridging-Header.h"` 的内容如下：
...
现在你可以遵循 [原生模块指南](/docs/turbo-native-modules-introduction#build-and-run-your-code-on-a-simulator) 的最后一步，你应该能看到你的应用运行着用 Swift 编写的原生模块。
