...
你也失去了为整个子树设置默认字体的能力。同时，`fontFamily` 只接受单个字体名称，这与 CSS 中的 `font-family` 不同。在整个应用程序中使用一致字体和大小的推荐方法是创建一个包含它们的组件 `MyAppText`，并在整个应用程序中使用此组件。你也可以使用此组件为其他类型的文本创建更具体的组件，如 `MyAppHeaderText`。
...
指示屏幕阅读器在与元素交互时应使用哪种语言的值。它应遵循 [BCP 47 规范](https://www.rfc-editor.org/info/bcp47)。
...
在 iOS 上，这些角色映射到相应的无障碍特性。图像按钮的功能与将特性设置为 'image' 和 'button' 相同。有关更多信息，请参阅 [无障碍指南](accessibility.md#accessibilitytraits-ios)。
...
有关更多信息，请参阅 [无障碍指南](accessibility#accessible-ios-android)。
...
用于在计算文本布局（包括换行）后用省略号截断文本，以便总行数不超过此数字。将此属性设置为 `0` 将导致取消设置此值，这意味着不会应用行限制。
...
响应者已被从 `View` 移除。可能在调用 `onResponderTerminationRequest` 后被其他视图移除，或者可能被操作系统无需询问而移除（例如，在 iOS 上与控制中心/通知中心一起发生）
...
如果父 `View` 想要阻止子 `View` 在触摸开始时成为响应者，它应该具有此处理程序并返回 `true`。
...
当滚动视图被禁用时，这定义了你的触摸可以移动离开按钮多远，然后才停用按钮。一旦停用，尝试将其移回，你会看到按钮再次被激活！当滚动视图被禁用时，来回移动几次。确保你传入一个常量以减少内存分配。
...
`role` 向辅助技术用户传达组件的目的。优先于 [`accessibilityRole`](text#accessibilityrole) 属性。
...
当为 `true` 时，文本被按下时不会发生视觉变化。默认情况下，按下时灰色椭圆会高亮文本。
