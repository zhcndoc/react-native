...
每当我们要发布新版本 的 React Native 时，我们都会为您构建一个版本的 Hermes。这将确保您使用的 Hermes 版本与您正在使用的 React Native 版本完全兼容。
...
:::caution
如果您使用非标准方式加载 JS bundle，有可能 `HermesInternal` 变量可用，但您并没有使用高度优化的预编译字节码。
...
React Native 还支持使用 JavaScriptCore 作为 [JavaScript 引擎](javascript-environment)。请按照以下说明选择退出 Hermes。
