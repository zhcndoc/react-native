...
点击你的主项目文件（代表 `.xcodeproj` 的那个文件），选择 `Build Phases`，然后将你正在导入的库内部 `Products` 文件夹中的静态库拖拽到 `Link Binary With Libraries`
...
如果你确实需要从原生调用它，那么我们需要知道库的头文件。为此，你必须进入项目文件，选择 `Build Settings` 并搜索 `Header Search Paths`。在那里你应该包含你的库的路径。（此文档过去建议使用 `recursive`，但不再推荐这样做，因为它可能导致细微的构建失败，尤其是使用 CocoaPods 时。）
