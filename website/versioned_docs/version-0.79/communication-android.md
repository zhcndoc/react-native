...
虽然我们有一种跨语言回调的风格（[在此处描述](legacy/native-modules-android#callbacks)），但这些回调并不总是我们需要的。主要问题是它们不打算作为属性传递。相反，这种机制允许我们从 JS 触发原生操作，并在 JS 中处理该操作的结果。
...
React Native 使你能够执行跨语言函数调用。你可以从 JS 执行自定义原生代码，反之亦然。不幸的是，取决于我们工作的侧面，我们通过不同的方式实现相同的目标。对于原生——我们使用事件机制来调度 JS 中处理函数程序的执行，而对于 React Native，我们直接调用原生模块导出的方法。
...
原生模块是 JS 中可用的 Java/Kotlin 类。通常每个 JS 桥接创建一个模块实例。它们可以向 React Native 导出任意函数和常量。它们在 [这篇文章](legacy/native-modules-android) 中有详细说明。
