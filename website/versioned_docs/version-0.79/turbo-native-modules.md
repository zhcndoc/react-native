...
本指南将向您展示如何编写 [Web Storage API](https://html.spec.whatwg.org/multipage/webstorage.html#dom-localstorage-dev) 的实现：`localStorage`。该 API 与可能在您的项目上编写应用程序代码的 React 开发者息息相关。
...
当所有内容都为 Codegen 配置好后，我们需要准备原生代码以挂钩到生成的代码中。
...
使用 `NativeLocalStorage`，这是一个修改后的 `App.tsx`，包含一些我们想要持久化的文本、一个输入字段和一些用于更新此值的按钮。
...
准备好一切后，我们将开始编写原生平台代码。我们分 2 部分进行：
