...
InteractionManager 允许在任何交互/动画完成后调度长时间运行的任务。特别是，这使得 JavaScript 动画能够流畅运行。

应用程序可以使用以下方法调度在交互后运行的任务：
...
`runAfterInteractions` 接受一个普通回调函数，或者一个带有 `gen` 方法的 `PromiseTask` 对象，该方法返回一个 `Promise`。如果提供了 `PromiseTask`，则在开始下一个可能更早同步排队的任务之前，它将完全解析（包括通过 `runAfterInteractions` 调度更多任务的异步依赖项）。

默认情况下，排队的任务在一个 `setImmediate` 批次中的循环中一起执行。如果调用 `setDeadline` 并传入一个正数，那么任务只会执行到截止时间（就 js 事件循环运行时间而言）接近为止，此时执行将通过 setTimeout 让出，允许触摸等事件启动交互并阻止排队任务执行，从而使应用程序响应更灵敏。
...
const useFadeIn = (duration = 5000) => {
  ...
  useEffect(() => {
    ...
    // 行为，你可以设置 isInteraction 为 false 来禁用它。
