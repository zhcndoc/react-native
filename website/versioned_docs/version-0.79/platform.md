...
| Brand <div className="label android">Android</div>        | string  | 否       | 产品/硬件关联的消费者可见品牌。                                                                                                                    |
...
| uiMode <div className="label android">Android</div>       | string  | 否       | 可能的值包括：`'car'`、`'desk'`、`'normal'`、`'tv'`、`'watch'` 和 `'unknown'`。了解更多关于 [Android ModeType](https://developer.android.com/reference/android/app/UiModeManager.html)。 |
...
select 方法返回最适合您当前运行平台的值。也就是说，如果您在手机运行，`android` 和 `ios` 键将优先。如果未指定这些，将使用 `native` 键，然后是 `default` 键。
