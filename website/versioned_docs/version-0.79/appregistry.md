...
要在视图应该被销毁时“停止”应用，请使用传递给 `runApplication` 的 tag 调用 `AppRegistry.unmountApplicationComponentAtRootTag`。这两者应该始终成对使用。
...
| taskCancelProvider<br/><div className="label basic required two-lines">必需</div> | [TaskCancelProvider](appregistry#taskcancelprovider) | 一个不返回任何值且不接受参数的函数；当请求取消时，由 taskProvider 执行的函数应该结束并尽快返回。                                                                    |
