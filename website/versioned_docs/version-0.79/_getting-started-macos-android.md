...
如果你已经在系统上安装了 JDK，我们推荐使用 JDK 17。使用更高版本的 JDK 可能会遇到问题。
...
接下来，选择 "SDK Tools" 选项卡，并同样勾选这里的 "Show Package Details" 复选框。查找并展开 "Android SDK Build-Tools" 条目，然后确保选中 `35.0.0` 和 `Android SDK Command-line Tools (latest)`。
...
如果你拥有物理 Android 设备，可以通过 USB 电缆将其连接到电脑，并按照 [此处](running-on-device.md) 的说明进行操作，用它代替 AVD 进行开发。
