...
对于大小应始终固定为特定数值而不基于屏幕大小计算的组件，通常使用这种方式设置尺寸。

:::caution
数值与物理测量单位之间没有通用的映射关系。这意味着具有固定尺寸的组件在不同设备和屏幕尺寸上可能具有不同的物理大小。然而，对于大多数用例来说，这种差异是不明显的。
:::

## Flex 尺寸

在组件的样式中使用 `flex` 可以让组件根据可用空间动态扩展和收缩。通常你会使用 `flex: 1`，这告诉组件填充所有可用空间，并与具有相同父组件的其他组件均匀共享。给定的 `flex` 值越大，组件与其兄弟组件相比所占的空间比例就越高。
