...
关闭最上层的 iOS 操作表，如果没有显示操作表，则会显示警告。
...
> **注意：** 如果 `url` 指向本地文件，或是 base64 编码的 uri，则它将直接加载并分享所指的文件。通过这种方式，你可以分享图片、视频、PDF 文件等。如果 `url` 指向远程文件或地址，它必须符合 [RFC 2396](https://www.ietf.org/rfc/rfc2396.txt) 中描述的 URL 格式。例如，没有适当协议（HTTP/HTTPS）的 Web URL 将不会被分享。
