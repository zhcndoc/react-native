...
[Expo](https://docs.expo.dev/) 是一个面向 React Native 的工具和服务框架，专注于帮助您构建、发布和迭代您的应用，使用在 Web 开发中流行的预览部署工作流，并自动化您的开发工作流。Expo 还使得无需触碰 Xcode 或 Android Studio 即可构建 React Native 应用成为可能，如果您想使用这些工具，它也不会造成阻碍。

[Ignite](https://github.com/infinitered/ignite) 是一个带有多个 React Native 样板项目的启动套件 CLI。最新的 Ignite Maverick 使用 MobX-State-Tree 进行状态管理、React Navigation 以及其他常用库。它具有用于屏幕、模型等的生成器，并开箱即用支持 Expo。Ignite 还附带了一个专为自定义设计调整的组件库、主题支持以及测试功能。如果您正在寻找预配置的技术栈，Ignite 可能非常适合您。

## 示例应用

尝试 [展示页](https://reactnative.dev/showcase) 中的应用，看看 React Native 能做什么！想要更动手实践的内容吗？查看这个 [GitHub 上的示例应用集](https://github.com/ReactNativeNews/React-Native-Apps)。您可以查看它们的源代码——尝试在模拟器或设备上运行其中一个。

## 查找、制作和分享您自己的原生组件和 TurboModules

React Native 拥有一个由数千名像您一样的开发者组成的社区，他们制作内容、工具、教程——以及原生组件！
