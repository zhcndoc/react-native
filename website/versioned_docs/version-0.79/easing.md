...
`Easing` 模块实现了常见的缓动函数。该模块被 [`Animated.timing()`](animated.md#timing) 用于在动画中传达物理上可信的运动。
...
- [`back`](easing.md#back) 提供一个基本动画，对象在向前移动之前稍微向后
...
[`poly`](easing.md#poly) 函数可用于实现四次、五次和其他更高次幂函数。
...
默认弹性值为 1，这会稍微过冲一次。0 弹性值完全不过冲，而 N > 1 的弹性值将过冲大约 N 次。
...
与 `Animated.parallel()` 一起使用，创建一个基本效果，对象在动画开始时稍微向后动画。
...
一个用于可视化三次贝塞尔曲线的有用工具可以在 https://cubic-bezier.com/ 找到
...
使任何缓动函数对称。缓动函数将在持续时间的一半向前运行，然后在剩余持续时间向后运行。
