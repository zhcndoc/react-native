...
在 iOS 上，返回一个 Promise，该 Promise 将以一个包含 `action` 和 `activityType` 的对象被调用。如果用户关闭了对话框，Promise 仍会被解决，action 为 `Share.dismissedAction`，所有其他键为 undefined。注意，某些分享选项可能不会在 iOS 模拟器上出现或工作。
...
| 名称                                                         | 类型   | 描述                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------ | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| content <div className="label basic required">必需</div> | object | `message` - 要分享的消息<br/>`url` - 要分享的 URL <div className="label ios">iOS</div><br/>`title` - 消息的标题 <div className="label android">Android</div><hr/>`url` 和 `message` 至少需要提供其中一个。                                                                                                                                                          |
