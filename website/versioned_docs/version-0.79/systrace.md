...
beginEvent/endEvent 用于在同一调用堆栈帧内开始并结束性能分析。
...
beginAsyncEvent/endAsyncEvent 用于开始并结束一个性能分析，其结束可能发生在另一个线程或当前堆栈帧之外。例如，应将返回的 cookie 变量作为输入传递给 endAsyncEvent 调用来结束性能分析。
