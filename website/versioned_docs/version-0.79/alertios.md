...
`AlertIOS` 提供创建带有消息的 iOS 警报对话框或创建用户输入提示的功能。
...
| callbackOrButtons | ?(() => void),[ButtonsArray](alertios#buttonsarray) | 否       | 此可选参数应为单参数函数或按钮数组。如果传递函数，当用户点击 'OK' 时将调用它。如果传递按钮配置数组，每个按钮应包含一个 `text` 键，以及可选的 `onPress` 和 `style` 键。`style` 应为 'default'、'cancel' 或 'destructive' 之一。 |
...
| callbackOrButtons | ?((text: string) => void),[ButtonsArray](alertios#buttonsarray) | 否       | 此可选参数应为单参数函数或按钮数组。如果传递函数，当用户点击 'OK' 时将使用提示框的值调用它。如果传递按钮配置数组，每个按钮应包含一个 `text` 键，以及可选的 `onPress` 和 `style` 键（参见示例）。`style` 应为 'default'、'cancel' 或 'destructive' 之一。 |
| type              | [AlertType](alertios#alerttype)                                 | 否       | 此配置文本输入。'plain-text'、'secure-text' 或 'login-password' 之一。                                                                                                                                                                                                                                                                                                                |
