...
BackHandler API 检测用于返回导航的硬件按钮按下，允许你为系统的返回操作注册事件监听器，并允许你控制应用程序如何响应。它仅适用于 Android。

事件订阅按相反顺序调用（即最后注册的订阅最先被调用）。

- **如果某个订阅返回 true，** 则较早注册的订阅将不会被调用。
- **如果没有订阅返回 true 或未注册任何订阅，** 它以编程方式调用默认返回按钮功能以退出应用。

> **模态框用户警告：** 如果你的应用显示了一个打开的 `Modal`，`BackHandler` 将不会发布任何事件（[参见 `Modal` 文档](modal#onrequestclose)）。
...
如果你使用 React Navigation 在不同屏幕间导航，可以参考他们的指南 [自定义 Android 返回按钮行为](https://reactnavigation.org/docs/custom-android-back-button-handling/)
