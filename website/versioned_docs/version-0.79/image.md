...
定义图片替代文本描述的字符串，当用户与图片交互时将由屏幕阅读器读取。使用此项将自动标记该元素为可无障碍访问。
...
类似于 `source`，此属性代表用于渲染图片加载指示器的资源。加载指示器会显示直到图片准备好显示，通常在图片下载完成后。
...
当图片部分加载完成时调用。什么是“部分加载”的定义取决于加载器，但这旨在用于渐进式 JPEG 加载。
...
指示获取资源时使用哪个引用者的字符串。设置图片请求中 `Referrer-Policy` 头的值。工作方式类似于 HTML 中的 `referrerpolicy` 属性。
...
- `none`：不执行采样，图片以其完整分辨率显示。这应仅在罕见情况下使用，因为它被认为是不安全的，当尝试渲染消耗过多内存的图片时，Android 将抛出运行时异常。
...
- `center`：沿两个维度将图片居中于视图中。如果图片大于视图，统一缩小它以便包含在视图中。
...
当 `resizeMethod` 设置为 `resize` 时，目标尺寸乘以此值。`scale` 方法用于执行剩余的调整大小。`1.0` 的默认值意味着位图大小设计为适合目标尺寸。大于 `1.0` 的乘数将设置调整大小选项大于目标尺寸，并且生成的位图将从硬件大小缩小。默认为 `1.0`。

此属性在目标尺寸相当小且源图片显著较大的情况下最有用。`resize` 调整大小方法执行下采样，源和目标图片尺寸之间会丢失大量图片质量，通常导致图片模糊。通过使用乘数，解码后的图片略大于目标大小但小于源图片（如果源图片足够大）。这允许混叠伪影通过缩放操作在乘数图片上产生伪质量。
...
此属性还可以包含多个远程 URL，与它们的宽度和高度一起指定，可能还有 scale/其他 URI 参数。原生侧将根据图片容器的测量大小选择最佳 `uri` 显示。可以添加 `cache` 属性来控制网络请求如何与本地缓存交互。（更多信息见 [图片的缓存控制](images#cache-control)）。

当前支持的格式是 `png`, `jpg`, `jpeg`, `bmp`, `gif`, `webp`, `psd`（仅 iOS）。此外，iOS 支持几种 RAW 图片格式。请参阅 Apple 的文档以获取当前支持的相机型号列表（对于 iOS 12，见 https://support.apple.com/en-ca/HT208967）。

请注意，`webp` 格式仅在 iOS 上与 JavaScript 代码捆绑时**才**支持。
...
表示逗号分隔的可能候选图片源列表的字符串。每个图片源包含图片的 URL 和像素密度描述符。如果未指定描述符，则默认为 `1x` 描述符。
...
- `force-cache`：现有缓存数据将用于满足请求，无论其年龄或过期日期如何。如果缓存中没有对应请求的现有数据，则数据从原始来源加载。
- `only-if-cached`：现有缓存数据将用于满足请求，无论其年龄或过期日期如何。如果缓存中没有对应 URL 加载请求的现有数据，则不会尝试从原始来源加载数据，并且加载被视为失败。
...
| headers                                    | object                                     | 一个对象，表示随远程图像请求一起发送的 HTTP 头部。                                                                                           |
...
- `number` - 不透明类型，由类似 `require('./image.jpg')` 的东西返回。
