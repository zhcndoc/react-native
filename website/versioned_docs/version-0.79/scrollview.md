...
该组件包装了平台原生的 ScrollView，同时提供了与触摸锁定“响应者”（responder）系统的集成。

请记住，ScrollView 必须具有有限的高度才能工作，因为它们将无限高度的子组件包含在一个有限高度的容器中（通过滚动交互）。为了限定 ScrollView 的高度，可以直接设置视图的高度（不推荐）或确保所有父视图都有有限的高度。忘记将 `{flex: 1}` 沿视图栈向下传递可能会导致此处出现错误，元素检查器可以快速调试这些问题。

尚不支持其他包含的响应者阻止此滚动视图成为响应者。

`<ScrollView>` 与 [`<FlatList>`](flatlist.md) - 该用哪一个？

`ScrollView` 会一次性渲染所有 React 子组件，但这会带来性能劣势。

想象一下你有一个很长的列表需要显示，可能有几屏的内容。一次性创建所有 JS 组件和原生视图，其中许多甚至可能不会显示，会导致渲染缓慢和内存使用增加。

这就是 `FlatList` 发挥作用的地方。`FlatList` 按需渲染项目，即在它们即将出现时渲染，并移除滚出屏幕很远的项目以节省内存和处理时间。

如果你想在项目之间渲染分隔符、多列、无限滚动加载或它支持的其他许多功能，`FlatList` 也很方便。
...
一个用于渲染粘性头部的 React 组件，应与 `stickyHeaderIndices` 一起使用。如果你的粘性头部使用自定义变换，例如当你希望列表具有动画且可隐藏的头部时，可能需要设置此组件。如果未提供组件，将使用默认的 [`ScrollViewStickyHeader`](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Components/ScrollView/ScrollViewStickyHeader.js) 组件。
...
控制 iOS 是否应自动调整滚动指示器的 inset。参见 Apple 的 [关于该属性的文档](https://developer.apple.com/documentation/uikit/uiscrollview/3198043-automaticallyadjustsscrollindica)。
...
当为 true 时，如果内容沿滚动方向大于滚动视图，则滚动视图在到达内容尽头时会反弹。当 `false` 时，即使 `alwaysBounce*` 属性为 `true`，它也会禁用所有反弹。
...
当 `true` 时，手势可以驱动缩放超过最小/最大值，并且缩放将在手势结束时动画过渡到最小/最大值，否则缩放将不会超过限制。
...
当为 true 时，ScrollView 将尝试在拖动时锁定为仅垂直或水平滚动。
...
当为 true 时，ScrollView 上的默认 JS pan responder 被禁用，ScrollView 内部触摸的完全控制权留给其子组件。如果启用了 `snapToInterval`，这特别有用，因为它不遵循典型的触摸模式。在没有 `snapToInterval` 的常规 ScrollView 用例中不要使用此属性，因为它可能导致滚动时发生意外的触摸。
...
有时滚动视图占据的空间比其内容填充的空间大。在这种情况下，此属性将用颜色填充滚动视图的其余部分，以避免设置背景并创建不必要的过度绘制。这是一个高级优化，在一般情况下不需要。
...
当 `true` 时，滚动视图的子项水平排列成行，而不是垂直排列成列。
...
如果粘性头部应该固定在 ScrollView 的底部而不是顶部。这通常与倒置的 ScrollViews 一起使用。
...
- `'never'` 当键盘弹出时，点击聚焦文本输入框之外的区域会 dismiss 键盘。发生这种情况时，子项不会接收点击。
- `'always'`，键盘不会自动 dismiss，滚动视图不会捕获点击，但滚动视图的子项可以捕获点击。
- `'handled'`，当点击被滚动视图的子项（或被祖先捕获）处理时，键盘不会自动 dismiss。
...
设置后，滚动视图将调整滚动位置，以便当前可见且在 `minIndexForVisible` 或之后的第一个子项不会改变位置。这对于双向加载内容的列表很有用，例如聊天线程，其中新消息的进入否则可能导致滚动位置跳跃。值 0 很常见，但其他值（如 1）可用于跳过加载旋转器或其他不应保持位置的内容。

可选的 `autoscrollToTopThreshold` 可用于在进行调整后，如果用户在调整前位于顶部阈值内，则使内容自动滚动到顶部。这也适用于类似聊天的应用程序，你希望看到新消息滚动到位，但如果用户已经向上滚动了一段距离，滚动一大段内容会造成干扰。

注意事项 1：在此启用的情况下重新排序滚动视图中的元素可能会导致跳跃和卡顿。它可以修复，但目前没有计划这样做。现在，不要重新排序使用此功能的任何 ScrollViews 或 Lists 的内容。
...
当为 true 时，滚动视图在滚动时停在滚动视图大小的倍数处。这可用于水平分页。
...
实验性：当 `true` 时，离屏子视图（其 `overflow` 值为 `hidden`）在离屏时从其原生备份父视图中移除。这可以提高长列表的滚动性能。
...
滚动视图指示器距离滚动视图边缘的 inset 量。这通常应设置为与 `contentInset` 相同的值。
...
用于记录此滚动视图滚动性能的标签。将强制开启动量事件（参见 sendMomentumEvents）。这本身不起任何作用，你需要实现一个自定义原生 FpsListener 才能使其有用。
...
当 `true` 时，滚动视图可以通过编程方式滚动超出其内容大小。
...
与 `snapToOffsets` 结合使用。默认情况下，列表末尾算作一个吸附偏移量。将 `snapToEnd` 设置为 false 可禁用此行为，并允许列表在其末尾和最后一个 `snapToOffsets` 偏移量之间自由滚动。
...
设置后，导致滚动视图停在 `snapToInterval` 值的倍数处。这可用于遍历长度小于滚动视图的子项。通常与 `snapToAlignment` 和 `decelerationRate="fast"` 结合使用。覆盖配置较少的 `pagingEnabled` 属性。
...
设置后，导致滚动视图停在定义的偏移量处。这可用于遍历长度小于滚动视图的各种大小的子项。通常与 `decelerationRate="fast"` 结合使用。覆盖配置较少的 `pagingEnabled` 和 `snapToInterval` 属性。
...
与 `snapToOffsets` 结合使用。默认情况下，列表开头算作一个吸附偏移量。将 `snapToStart` 设置为 `false` 可禁用此行为，并允许列表在其开头和第一个 `snapToOffsets` 偏移量之间自由滚动。
...
子项索引数组，确定哪些子项在滚动时固定在屏幕顶部。例如，传递 `stickyHeaderIndices={[0]}` 将使第一个子项固定在滚动视图的顶部。你也可以使用像 [x,y,z] 这样的方式使多个项在顶部时粘性。此属性不支持与 `horizontal={true}` 结合使用。
