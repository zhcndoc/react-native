...
使用此 API 的一种常见方法是在函数组件中更新状态钩子之前调用它，以及在类组件中调用 `setState` 之前调用它。
...
`config` 参数是一个对象，包含以下键。[`create`](layoutanimation.md#create) 返回一个有效的 `config` 对象，[`Presets`](layoutanimation.md#presets) 对象也都可以作为 `config` 传递。
...
要在 [`create`](layoutanimation.md#create) 方法中使用的动画类型枚举，或用于 [`configureNext`](layoutanimation.md#configurenext) 的 `create`/`update`/`delete` 配置中。（示例用法：`LayoutAnimation.Types.easeIn`）
...
要在 [`create`](layoutanimation.md#create) 方法中使用的布局属性枚举，或用于 [`configureNext`](layoutanimation.md#configurenext) 的 `create`/`update`/`delete` 配置中。（示例用法：`LayoutAnimation.Properties.opacity`）
