...
首次启动时，DevTools 将打开欢迎面板，同时打开一个控制台抽屉，你可以在其中查看日志并与 JavaScript 运行时交互。从窗口顶部，你可以导航到其他面板，包括集成的 React 组件检查器和性能分析器。

![React Native DevTools 打开到 "Welcome" 面板](/docs/assets/debugging-rndt-welcome.jpg)

React Native DevTools 由 React Native 内置的专用调试架构提供支持，并使用 [Chrome DevTools](https://developer.chrome.com/docs/devtools) 前端的定制构建。这使我们能够提供熟悉的、与浏览器对齐的调试功能，这些功能深度集成并为端到端的可靠性而构建。
...
React Native DevTools 取代了之前的 Flipper、Experimental Debugger 和 Hermes debugger (Chrome) 前端。如果你使用的是较旧版本的 React Native，请前往 [你的版本](/versions) 的文档。

对于使用 JavaScriptCore 而不是 Hermes 的应用，Direct JSC Debugging 仍然可用（参见 [其他调试方法](./other-debugging-methods)）。
...
- <a href="https://shift.infinite.red/why-you-dont-need-flipper-in-your-react-native-app-and-how-to-get-by-without-it-3af461955109" target="_blank">为什么你的 React Native 应用不需要 Flipper……以及如何在没有它的情况下继续工作&nbsp;↗</a>
...
当发生不可恢复的错误时，例如 JavaScript 语法错误，LogBox 将打开并显示错误位置。在这种状态下，LogBox 无法关闭，因为你的代码无法执行。一旦语法错误被修复，LogBox 将自动关闭——要么通过快速刷新，要么在手动重新加载后。
...
- **警告** 将显示一个不含详情的通知横幅，提示你打开 React Native DevTools。
...
可以使用 `LogBox.ignoreAllLogs()` 禁用 LogBox 通知。这在诸如产品演示等情况下可能很有用。
...
可以通过 `LogBox.ignoreLogs()` 逐个日志地禁用通知。这对于嘈杂的警告或无法修复的警告（例如在第三方依赖中）可能很有用。
...
![iOS 和 Android 上的性能监控覆盖层](/docs/assets/debugging-performance-monitor.jpg)

:::info
性能监控器在应用内运行，仅供参考。我们建议调查 Android Studio 和 Xcode 下的原生工具以获得准确的性能测量。
:::
