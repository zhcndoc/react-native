...
Headless JS 是一种当你的应用处于后台时在 JavaScript 中运行任务的方式。例如，它可以用于同步最新数据、处理推送通知或播放音乐。
...
你可以在任务中做任何事情，例如网络请求、计时器等，只要它不涉及 UI。一旦你的任务完成（即 promise 被 resolved），React Native 将进入“暂停”模式（除非有其他任务正在运行，或者有一个前台应用）。
...
- 默认情况下，如果你尝试在应用处于前台时运行任务，你的应用将会崩溃。这是为了防止开发者因在任务中执行大量工作而拖慢 UI 从而引发问题。你可以传递第四个 `boolean` 参数来控制此行为。
...
广播接收器然后在 onReceive 函数中处理被广播的 intent。这是检查你的应用是否处于前台的好地方。如果应用不在前台，我们可以准备要启动的 intent，不使用信息或使用 `putExtra` 捆绑额外信息（请记住 bundle 只能处理 parcelable 值）。最后服务被启动并且获取了 wakelock。
