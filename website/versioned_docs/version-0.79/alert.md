...
在 iOS 上，你可以指定任意数量的按钮。每个按钮可以可选地指定一个样式或被强调，可用选项由 [AlertButtonStyle](#alertbuttonstyle-ios) 枚举和 [AlertButton](alert#alertbutton) 上的 `isPreferred` 字段表示。
...
| callbackOrButtons                                          | function<hr/>[AlertButton](alert#alertButton)[] | 如果传递的是函数，当用户点击 'OK' 时，它将带有提示框的值被调用<br/>`(text: string) => void`。<hr/>如果传递的是数组，按钮将根据数组内容进行配置。 |
| type                                                       | [AlertType](alert#alerttype-ios)                | 配置文本输入。                                                                                                                                                                       |
