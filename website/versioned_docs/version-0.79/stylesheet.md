...
> **警告：实验性功能。** 破坏性变更可能会频繁发生，且不会可靠地宣布。整个功能可能会被删除，谁知道呢？使用风险自负。
...
设置一个函数用于预处理样式属性值。这在内部用于处理颜色和变换值。除非你真的知道自己在做什么并且已经穷尽了其他选项，否则不应使用此功能。
...
有时你可能想要 `absoluteFill` 但需要一些调整 - `absoluteFillObject` 可用于在 `StyleSheet` 中创建自定义条目，例如：
