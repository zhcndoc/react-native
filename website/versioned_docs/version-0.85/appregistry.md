...
<div className="banner-native-code-required">
  <h3>需要原生代码的项目</h3>
  <p>如果你使用的是托管的 Expo 工作流，则只有一个入口组件注册到 <code>AppRegistry</code>，并且它是自动处理的（或通过 <a href="https://docs.expo.dev/versions/latest/sdk/register-root-component/">registerRootComponent</a>）。你不需要使用此 API。</p>
</div>
...
当视图应该被销毁时，要“停止”一个应用，请使用传递给 `runApplication` 的 tag 调用 `AppRegistry.unmountApplicationComponentAtRootTag`。这两者应该总是成对使用。
...
注册一个可以被取消的无头任务。无头任务是一段在没有 UI 的情况下运行的代码。

**参数：**

| 名称                                                                                  | 类型                                                 | 描述                                                                                                                                                                                                                         |
| ------------------------------------------------------------------------------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| taskKey<br/><div className="label basic required two-lines">必需</div>            | string                                               | 当调用 startHeadlessTask 时使用的此任务实例的原生 id。                                                                                                                                               |
...
这是一种当你的应用处于后台时在 JavaScript 中运行任务的方式。例如，它可以用于同步最新数据、处理推送通知或播放音乐。

**参数：**

| 名称                                                                        | 类型                                     | 描述                                                                                                                                                                                                                         |
| --------------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| taskKey <div className="label basic required two-lines">必需</div>      | string                                   | 当调用 startHeadlessTask 时使用的此任务实例的原生 id。                                                                                                                                               |
