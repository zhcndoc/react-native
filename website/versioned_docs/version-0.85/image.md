...
此示例展示了获取并显示本地存储的图片，以及来自网络甚至来自 `'data:'` uri 方案提供的数据的图片。
...
```groovy
dependencies {
  // 如果你的应用支持冰激凌三明治之前的 Android 版本（API 级别 14）
...
:::note
上面列出的版本可能未及时更新。请检查主仓库中的 [`packages/react-native/gradle/libs.versions.toml`](https://github.com/facebook/react-native/blob/main/packages/react-native/gradle/libs.versions.toml) 以查看特定标记版本中使用的 fresco 版本。
:::
...
一个字符串，定义图片的替代文本描述，当用户与图片交互时将由屏幕阅读器读取。使用此项将自动标记该元素为可无障碍访问。
...
当图片调整大小时，由 `capInsets` 指定的大小的角将保持固定大小，但图片的中心内容和边框将被拉伸。这对于创建可调整大小的圆角按钮、阴影和其他可调整大小的资源很有用。更多信息请参阅 [苹果官方文档](https://developer.apple.com/library/ios/documentation/UIKit/Reference/UIImage_Class/index.html#//apple_ref/occ/instm/UIImage/resizableImageWithCapInsets)。
...
在图片部分加载完成时调用。“部分加载”的定义取决于加载器，但这旨在用于渐进式 JPEG 加载。
...
一个字符串，指示获取资源时使用哪个引用者。设置图片请求中 `Referrer-Policy` 头的值。工作方式类似于 HTML 中的 `referrerpolicy` 属性。
...
- `none`：不执行采样，图片以其完整分辨率显示。这应仅在罕见情况下使用，因为它被认为是不安全的，因为当尝试渲染消耗过多内存的图片时，Android 将抛出运行时异常。

关于 `resize` 和 `scale` 的更多详情可以在 https://frescolib.org/docs/resizing 找到。
...
确定当框架不匹配原始图片尺寸时如何调整图片大小。默认为 `cover`。

- `cover`：均匀缩放图片（保持图片的纵横比），以便
  - 图片的两个维度（宽度和高度）将等于或大于视图的相应维度（减去内边距）
  - 缩放图片的至少一个维度将等于视图的相应维度（减去内边距）

- `contain`：均匀缩放图片（保持图片的纵横比），以便图片的两个维度（宽度和高度）将等于或小于视图的相应维度（减去内边距）。

- `stretch`：独立缩放宽度和高度，这可能会改变源的纵横比。

- `repeat`：重复图片以覆盖视图的框架。图片将保持其大小和纵横比，除非它大于视图，在这种情况下它将均匀缩小以便包含在视图中。

- `center`：沿两个维度将图片居中在视图中。如果图片大于视图，将其均匀缩小以便包含在视图中。
...
当 `resizeMethod` 设置为 `resize` 时，目标尺寸将乘以该值。`scale` 方法用于执行剩余的调整大小。默认值 `1.0` 表示位图大小设计为适合目标尺寸。大于 `1.0` 的乘数将设置比目标尺寸更大的调整大小选项，并且生成的位图将从硬件大小缩小。默认为 `1.0`。

此属性在目标尺寸相当小且源图片显著较大的情况下最有用。`resize` 调整大小方法执行下采样，源和目标图片尺寸之间会丢失大量图片质量，通常导致图片模糊。通过使用乘数，解码后的图片略大于目标尺寸但小于源图片（如果源图片足够大）。这允许混叠伪影通过缩放操作在乘数图片上产生伪造的质量。
...
此属性还可以包含多个远程 URL，与它们的宽度和高度一起指定，并可能带有 scale/其他 URI 参数。原生端将根据图片容器的测量大小选择最佳的 `uri` 进行显示。可以添加 `cache` 属性来控制网络请求如何与本地缓存交互。（更多信息请参阅 [图片的缓存控制](images#cache-control)）。

当前支持的格式有 `png`、`jpg`、`jpeg`、`bmp`、`gif`、`webp`、`psd`（仅 iOS）。此外，iOS 支持多种 RAW 图片格式。请参阅苹果文档以获取当前支持的相机型号列表（对于 iOS 12， see https://support.apple.com/en-ca/HT208967）。

请注意，`webp` 格式在 iOS 上 **仅** 在与 JavaScript 代码捆绑时支持。
...
一个唯一标识符，用于此元素以便在 UI 自动化测试脚本中使用。
...
为了获取图像尺寸，图像可能需要先被加载或下载，之后它将被缓存。这意味着原则上你可以使用此方法来预加载图像，但它未针对该目的进行优化，并且将来可能会以实现不会完全加载/下载图像数据的方式实施。正确的、受支持的预加载图像方式将作为单独的 API 提供。
...
预取远程图像以供稍后使用，方法是将其下载到磁盘缓存。返回一个解析为布尔值的 Promise。
...
- `reload`: URL 的数据将从原始来源加载。不应使用现有的缓存数据来满足 URL 加载请求。
- `force-cache`: 现有的缓存数据将用于满足请求，无论其年龄或过期日期如何。如果缓存中没有对应请求的现有数据，则从原始来源加载数据。
- `only-if-cached`: 现有的缓存数据将用于满足请求，无论其年龄或过期日期如何。如果缓存中没有对应 URL 加载请求的现有数据，则不会尝试从原始来源加载数据，并且加载被视为失败。
...
| width                                      | number                                     | 如果在构建时已知，可以指定，在这种情况下，该值将用于设置默认 `<Image/>` 组件维度。                                                     |
| height                                     | number                                     | 如果在构建时已知，可以指定，在这种情况下，该值将用于设置默认 `<Image/>` 组件维度。                                                     |
...
| headers                                    | object                                     | 表示随远程图像请求一起发送的 HTTP 头信息的对象。                                                                                           |
