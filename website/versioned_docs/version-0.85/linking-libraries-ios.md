...
:::info
此步骤中 `--save` 或 `--save-dev` 标志非常重要。React Native 将根据你 `package.json` 文件中的 `dependencies` 和 `devDependencies` 来链接你的库。
:::
...
如果你确实需要从原生侧调用它，那么我们需要知道库的头文件。为此，你必须进入项目文件，选择 `Build Settings` 并搜索 `Header Search Paths`。在那里你应该包含你的库的路径。（此文档曾经推荐使用 `recursive`，但不再推荐这样做，因为它可能导致细微的构建失败，尤其是使用 CocoaPods 时。）
