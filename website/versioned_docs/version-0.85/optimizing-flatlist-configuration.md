...
- **空白区域：** 当 `VirtualizedList` 无法足够快地渲染你的项目时，你可能会进入列表中没有渲染组件的部分，显示为空白空间。
...
**缺点：** 请注意，此实现可能存在错误，例如内容丢失（主要在 iOS 上观察到），尤其是当你使用复杂的变换和/或绝对定位时。还要注意，这并不能节省大量内存，因为视图没有被释放，只是被分离了。
...
**优点：** 将此属性与 `maxToRenderPerBatch` 结合使用，使你可以例如在较不频繁的批中渲染更多项目，或在较频繁的批中渲染较少项目。

**缺点：** 较不频繁的批可能导致空白区域，较频繁的批可能导致响应能力问题。
...
此处传递的数字是一个测量单位，其中 1 等同于你的视口高度。默认值为 21（上方 10 个视口，下方 10 个视口，中间一个）。
...
你的组件越复杂，渲染速度越慢。尽量避免在列表项中包含大量逻辑和嵌套。如果你在应用中大量复用此列表项组件，请专门为大型列表创建一个组件，并使其逻辑和嵌套尽可能少。

### 使用轻量组件

你的组件越重，渲染越慢。避免使用重型图片（列表项使用裁剪版本或缩略图，尽可能小）。与设计团队沟通，在列表中尽可能少地使用效果、交互和信息。在项的详情中展示它们。

### 使用 `memo()`

`React.memo()` 创建一个记忆化组件，仅当传递给组件的 props 更改时才会重新渲染。我们可以使用此函数来优化 FlatList 中的组件。
...
你可以使用社区包（例如来自 [Dream11](https://github.com/ds-horizon) 的 [@d11/react-native-fast-image](https://github.com/ds-horizon/react-native-fast-image)）来获得性能更高的图片。列表中的每个图片都是一个 `new Image()` 实例。它达到 `loaded` 钩子的速度越快，你的 JavaScript 线程就越快释放。

### 使用 `getItemLayout`

如果所有列表项组件具有相同的高度（对于水平列表则为宽度），提供 [getItemLayout](flatlist#getitemlayout) 属性可以消除 `FlatList` 管理异步布局计算的需要。这是一种非常理想的优化技术。

如果你的组件具有动态大小且你确实需要性能，请考虑询问你的设计团队是否可以重新设计以获得更好的性能。

### 使用 `keyExtractor` 或 `key`

你可以为 `FlatList` 组件设置 [`keyExtractor`](flatlist#keyextractor)。此属性用于缓存并作为 React `key` 来跟踪项目重新排序。
