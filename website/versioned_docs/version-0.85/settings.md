...
`watchId` 是最初配置订阅时由 `watchKeys()` 返回的数字。
...
订阅通知，以便当 `NSUserDefaults` 中 `keys` 参数指定的任何键的值发生更改时收到通知。返回一个 `watchId` 数字，可与 `clearWatch()` 一起使用以取消订阅。

:::note
`watchKeys()` 设计上忽略内部的 `set()` 调用，仅在 React Native 代码外部执行的更改时触发回调。
:::
