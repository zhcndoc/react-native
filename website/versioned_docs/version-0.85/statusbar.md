...
对于不适合使用组件的场景，也提供了作为组件静态函数暴露的命令式 API。不过，不建议对同一个属性同时使用静态 API 和组件，因为静态 API 设置的任何值都会在下一次渲染时被组件设置的值覆盖。
...
如果状态栏属性变化之间的过渡应当进行动画处理。支持 `backgroundColor`、`barStyle` 和 `hidden` 属性。
...
:::warning
由于 Android 15 引入的边到边强制执行，在 API 级别 35 中设置状态栏背景颜色已被弃用，设置后将不起作用。你可以在这里阅读更多关于我们的 [edge-to-edge 推荐](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::

| Type            | Required | Default                                                                |
| --------------- | -------- | ---------------------------------------------------------------------- |
| [color](colors) | 否 | 默认系统 StatusBar 背景颜色，或者在未定义时为 `'black'` |
...
在 Android 上，这只会对 API 23 及以上版本生效。
...
状态栏是否为半透明。当 `translucent` 设为 `true` 时，应用会在状态栏下方绘制内容。在使用半透明状态栏颜色时这很有用。

:::warning
由于 Android 15 引入的边到边强制执行，在 API 级别 35 中将状态栏设为半透明已被弃用，设置后将不起作用。你可以在这里阅读更多关于我们的 [edge-to-edge 推荐](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
...
获取并移除堆栈中的最后一个 StatusBar 条目。

**参数：**

| Name                                                       | Type | Description                           |
| ---------------------------------------------------------- | ---- | ------------------------------------- |
| entry <div className="label basic required">Required</div> | any  | 来自 `pushStackEntry` 返回的条目。 |
...
将一个 StatusBar 条目压入堆栈。完成后应将返回值传递给 `popStackEntry`。
...
| Name                                                       | Type | Description                                                                  |
| ---------------------------------------------------------- | ---- | ---------------------------------------------------------------------------- |
| entry <div className="label basic required">Required</div> | any  | 要替换的、来自 `pushStackEntry` 返回的条目。                             |
| props <div className="label basic required">Required</div> | any  | 包含要在替换后的堆栈条目中使用的 StatusBar 属性的对象。 |
...
:::warning[Deprecated]
iOS 13 及更高版本不支持状态栏网络活动指示器。此功能将在未来版本中移除。
:::
