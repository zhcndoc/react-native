...
React Native 不仅仅适用于 Android 和 iOS 设备——我们的合作伙伴和社区维护着将 React Native 带到其他平台的项目，例如：
...
目前，从头创建 React Native 平台的过程还没有很好的文档记录——即将到来的重构（[Fabric](/blog/2018/06/14/state-of-react-native-2018)）的目标之一就是让维护平台变得更容易。
...
`"providesModuleNodeModules"` 是一个模块数组，这些模块将被添加到 Haste 模块搜索路径中，而 `"platforms"` 是一个平台后缀数组，将被添加为有效平台。
