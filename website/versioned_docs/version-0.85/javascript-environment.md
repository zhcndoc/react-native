...
- 如果 Hermes 被禁用，React Native 将使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)，这是驱动 Safari 的 JavaScript 引擎。请注意，在 iOS 上，由于 iOS 应用中不存在可写的可执行内存，JavaScriptCore 不使用 JIT。
...
<tbody>
  ...
  <TableRow name="粘滞正则" code="const a = /o+/y;" url="https://babeljs.io/docs/en/babel-plugin-transform-sticky-regex" />
  ...
  <TableRow name="导出默认从" code="export v from 'mod';" url="https://babeljs.io/docs/en/babel-plugin-proposal-export-default-from" />
