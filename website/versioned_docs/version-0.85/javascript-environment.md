...
- 如果禁用了 Hermes，React Native 将使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)，这是一款驱动 Safari 的 JavaScript 引擎。请注意，在 iOS 上，由于 iOS 应用中不存在可写可执行内存，JavaScriptCore 不会使用 JIT。
- 使用 Chrome 调试时，所有 JavaScript 代码都在 Chrome 本身中运行，并通过 WebSockets 与原生代码通信。Chrome 使用 [V8](https://v8.dev/) 作为其 JavaScript 引擎。

虽然这些环境非常相似，但你可能仍会遇到一些不一致之处。最好避免依赖任何运行时的特定细节。

## JavaScript 语法转换器

语法转换器通过允许你使用新的 JavaScript 语法，而无需等待所有解释器都支持它，从而让编写代码更加愉快。

React Native 自带 [Babel JavaScript 编译器](https://babeljs.io)。有关其支持的转换的更多细节，请查看 [Babel 文档](https://babeljs.io/docs/plugins/#transform-plugins)。

React Native 已启用的完整转换列表可在 [@react-native/babel-preset](https://github.com/facebook/react-native/tree/main/packages/react-native-babel-preset) 中找到。
