...
React Native 为社区提供了能力，使其能够在新功能的设计和实现几乎完成时即可采用个别新功能，甚至在这些功能包含在稳定版本之前。这种方法被称为**发布级别**。
...
React Native 也不使用 `@canary` 或 `@experimental` NPM 标签，因为发布级别适用于 React Native 的稳定版和 nightly 版本。
:::

此外，由于 react-native 消费 React 版本的方式，将发布级别设置为 `EXPERIMENTAL` 或 `CANARY` **不会** 导致使用 `react@nightly` 或 `react@canary`（[你可以在此阅读更多相关信息](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Renderer/README.md#react--react-native-versions)）。
