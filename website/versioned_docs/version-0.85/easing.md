...
默认弹性值为 1，会稍微过冲一次。0 弹性值完全不会过冲，而 N > 1 的弹性值将过冲大约 N 次。
...
与 `Animated.parallel()` 一起使用以创建基本效果，对象在动画开始时稍微向后动画。
...
可以在 https://cubic-bezier.com/ 找到一个有用的可视化工具来查看三次贝塞尔曲线。
...
使任何缓动函数对称。缓动函数将在一半持续时间向前运行，然后在剩余持续时间向后运行。
