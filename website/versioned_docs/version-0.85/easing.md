...
- [`back`](easing.md#back) 提供一种基础动画：对象在向前移动前先轻微后退
- [`bounce`](easing.md#bounce) 提供弹跳动画
- [`ease`](easing.md#ease) 提供基础的惯性动画
- [`elastic`](easing.md#elastic) 提供基础的弹簧交互
...
默认弹性系数为 1，会略微超出一次。弹性系数为 0 时完全不会超出，而弹性系数为 N > 1 时大约会超出 N 次。
...
与 `Animated.parallel()` 一起使用，可创建一种基础效果：动画开始时对象会略微向后回弹。
...
用于可视化三次贝塞尔曲线的一个实用工具可见于 https://cubic-bezier.com/
...
使任意缓动函数对称。缓动函数会在持续时间的前半段正向运行，然后在剩余时间中反向运行。
