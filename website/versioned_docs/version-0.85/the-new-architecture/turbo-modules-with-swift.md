...
在深入本指南之前，请确保熟悉该指南，最好实现指南中的示例。

## 适配器模式

目标是用 Swift 模块实现所有业务逻辑，并在 Objective-C++ 中保留一个薄的胶水层，以便将应用与 Swift 实现连接起来。
...
1. 在 Xcode 项目中创建一个新的空文件，命名为 `NativeLocalStorage.swift`
...
注意，你必须将所有需要从 Objective-C 调用的方法声明为 `public` 并加上 `@objc` 注解。
...
这种方法在接口中引入了一些代码重复，但它允许你复用代码库中可能已经拥有的 Swift 代码，只需付出很少的额外努力。
