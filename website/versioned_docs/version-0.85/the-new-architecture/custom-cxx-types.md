...
支持新自定义类型的第一步是定义桥接头，该头文件负责将类型 **从** JS 表示转换为 C++ 表示，以及 **从** C++ 表示转换为 JS 表示。
...
- 针对自定义类型的 `Bridging` 结构的显式特化。在本例中，模板指定了 `int64_t` 类型。
...
2. 打开 `specs/NativeSampleModule.cpp` 文件并实现新函数：
...
第一个类型是 `Address` 的定义。第二个类型包含将所有基础结构从 JS 桥接到 C++ 以及反之亦然所需的内容。我们需要添加的唯一额外步骤是定义扩展 `NativeSampleModuleAddressBridging` 类型的 `Bridging` 结构。
...
- 名称的第一部分始终是模块类型。本例中为 `NativeSampleModule`。
- 名称的第二部分始终是规范中定义的 JS 类型名称。本例中为 `Address`。
  :::

### 3. 实现原生代码

现在，我们需要在 C++ 中实现 `validateAddress` 函数。首先，我们需要将函数声明添加到 `.h` 文件中，然后可以在 `.cpp` 文件中实现它。
...
:::note
如果您想了解更多关于 `JSI` 及其工作原理的信息，请参阅 App.JS 2024 的 [这次精彩的演讲](https://youtu.be/oLmGInjKU2U?feature=shared)
:::
