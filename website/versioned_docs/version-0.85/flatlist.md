...
要渲染多列，请使用 [`numColumns`](flatlist.md#numcolumns) 属性。使用此方法而不是 `flexWrap` 布局可以避免与 item 高度逻辑冲突。

下面是一个更复杂的、可选择示例。

- 通过传递 `extraData={selectedId}` 给 `FlatList`，我们确保当状态改变时 `FlatList` 本身会重新渲染。如果不设置此属性，`FlatList` 将不知道它需要重新渲染任何项，因为它是一个 `PureComponent` 并且属性比较不会显示任何变化。
...
这是 [`<VirtualizedList>`](virtualizedlist.md) 的便捷包装器，因此继承了它的属性（以及 [`<ScrollView>`](scrollview.md) 的属性），除非这里明确列出，同时具有以下注意事项：
...
- 为了限制内存并实现平滑滚动，内容在屏幕外异步渲染。这意味着有可能滚动速度快于填充率，暂时看到空白内容。这是一个可以根据每个应用的需求进行调整的权衡，我们正在幕后改进它。
- 默认情况下，列表查找每个项上的 `key` 属性并将其用作 React 键。或者，你可以提供自定义 `keyExtractor` 属性。
...
要渲染的项的数组（或类数组列表）。其他数据类型可以通过直接针对 [`VirtualizedList`](virtualizedlist.md) 使用。
...
添加 `getItemLayout` 可以大大提高几百项列表的性能。如果你指定了 `ItemSeparatorComponent`，请记住在偏移计算中包含分隔符长度（高度或宽度）。
...
用于提取给定项在指定索引处的唯一键。键用于缓存以及作为 react 键来跟踪项重新排序。默认提取器检查 `item.key`，然后检查 `item.id`，然后像 React 一样回退到使用索引。
...
多列只能与 `horizontal={false}` 一起渲染，并将像 `flexWrap` 布局一样呈锯齿状排列。所有项的高度应该相同 - 不支持砌体布局。
...
有关 flow 类型和进一步文档，请参阅 [`ViewabilityHelper.js`](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Lists/ViewabilityHelper.js)。
...
在触发可见性回调之前，项必须物理可见的最短时间（毫秒）。数字高意味着不停止地滚动内容不会将内容标记为可见。

#### viewAreaCoveragePercentThreshold

部分遮挡的项必须覆盖视口的百分比才能算作“可见”，0-100。完全可见的项始终被视为可见。值为 0 意味着视口中的单个像素使项可见，值为 100 意味着项必须完全可见或覆盖整个视口才算可见。
...
`ViewabilityConfig`/`onViewableItemsChanged` 对的列表。当特定的 `onViewableItemsChanged` 对应的 `ViewabilityConfig` 条件满足时，它将被调用。有关 flow 类型和进一步文档，请参阅 `ViewabilityHelper.js`。
...
滚动到指定索引的项，使其位于可见区域内，其中 `viewPosition` 为 0 时将其置于顶部，为 1 时置于底部，为 0.5 时居中。
...
- 'viewPosition' (number) - 值 `0` 将索引指定的项置于顶部，`1` 置于底部，`0.5` 置于中间居中。
