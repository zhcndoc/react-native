...
:::warning[已弃用]
避免长时间运行的工作，请改用 [`requestIdleCallback`](global-requestIdleCallback)。
:::
...
// 运行动画...（`runAfterInteractions` 任务会排队）
// 稍后，在动画完成时：
InteractionManager.clearInteractionHandle(handle);
// 如果所有句柄都已清除，则会运行排队的任务
```

`runAfterInteractions` 接受普通回调函数，或一个带有 `gen` 方法并返回 `Promise` 的 `PromiseTask` 对象。如果提供了 `PromiseTask`，那么在开始执行下一个可能先前已同步排队的任务之前，它会被完全解析（包括那些也通过 `runAfterInteractions` 继续排队更多任务的异步依赖）。

默认情况下，排队的任务会在一个 `setImmediate` 批次中循环一起执行。如果调用 `setDeadline` 并传入一个正数，那么任务只会执行到截止时间（以 js 事件循环运行时间衡量）临近为止，此时执行会通过 `setTimeout` 让出控制权，从而允许触摸等事件开始交互并阻止排队任务继续执行，使应用更具响应性。
...
正数会在 `eventLoopRunningTime` 达到截止值后使用 `setTimeout` 来安排任何任务，否则所有任务都会在一个 `setImmediate` 批次中执行（默认）。
