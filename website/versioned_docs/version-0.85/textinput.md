...
一个用于通过键盘向应用中输入文本的基础组件。Props 提供了多种功能的可配置性，例如自动纠正、自动大写、占位符文本以及不同的键盘类型，比如数字键盘。
...
请注意，某些 props 仅在 `multiline={true/false}` 时可用。此外，只有单侧生效的边框样式（例如 `borderBottomColor`、`borderLeftWidth` 等）在 `multiline=true` 时不会应用。要实现相同效果，可以将 `TextInput` 包裹在 `View` 中：
...
`TextInput` 默认在其视图底部带有一条边框。该边框的内边距由系统提供的背景图像设置，且无法更改。避免此问题的方案要么是不显式设置高度，此时系统会负责在正确位置显示边框；要么通过将 `underlineColorAndroid` 设置为透明来不显示边框。

请注意，在 Android 上对输入框执行文本选择会将应用的 activity `windowSoftInputMode` 参数更改为 `adjustResize`。这可能会在键盘激活时导致 `position: 'absolute'` 的组件出现问题。要避免这种行为，可以在 AndroidManifest.xml 中显式指定 `windowSoftInputMode`（ https://developer.android.com/guide/topics/manifest/activity-element.html ），或者使用原生代码以编程方式控制该参数。
...
告知 `TextInput` 自动将某些字符大写。该属性不受某些键盘类型（如 `name-phone-pad`）支持。
...
为系统提供自动完成提示，使其可以提供自动填充。在 Android 上，系统始终会尝试通过启发式方法识别内容类型来提供自动填充。要禁用自动完成，请将 `autoComplete` 设置为 `off`。
...
如果为 `true`，提交时文本字段将失去焦点。单行字段的默认值为 `true`，多行字段的默认值为 `false`。请注意，对于多行字段，将 `blurOnSubmit` 设为 `true` 意味着按回车会使字段失去焦点并触发 `onSubmitEditing` 事件，而不是在字段中插入换行符。
...
何时在文本视图右侧显示清除按钮。该属性仅支持单行 `TextInput` 组件。默认值为 `never`。
...
决定在文本输入中将哪些类型的数据转换为可点击 URL。仅在 `multiline={true}` 且 `editable={false}` 时有效。默认不检测任何数据类型。

你可以提供一种类型或一个包含多种类型的数组。

`dataDetectorTypes` 的可选值有：
...
传入该属性时，会设置组件中光标（或“caret”）的颜色。与 `selectionColor` 的行为不同，光标颜色会独立于文本选择框的颜色进行设置。
...
当为 `false` 时，如果文本输入框周围可用空间较小（例如手机横屏时），操作系统可能会选择让用户在全屏文本输入模式中编辑文本。当为 `true` 时，此功能将被禁用，用户将始终直接在文本输入框内编辑文本。默认值为 `false`。
...
确定回车键上应显示的文本。优先级高于 `returnKeyType` prop。
...
告诉操作系统，在 Android API Level 26+ 上，应用中的各个字段是否应被包含在用于自动填充的视图结构中。可选值为 `auto`、`no`、`noExcludeDescendants`、`yes` 和 `yesExcludeDescendants`。默认值为 `auto`。
...
如果定义了该属性，则提供的图片资源会渲染在左侧。图片资源必须位于 `/android/app/src/main/res/drawable` 中，并像下面这样引用：
...
其作用类似于 HTML 中的 `inputmode` 属性，用于确定打开哪种键盘，例如 `numeric`，其优先级高于 `keyboardType`。
...
如果为 `true`，文本输入可以多行显示。默认值为 `false`。

:::note
需要注意的是，这会使 iOS 上的文本顶部对齐，而在 Android 上居中。若要在两个平台上获得相同表现，请将 `textAlignVertical` 设为 `top`。
:::
...
设置 `TextInput` 的最大行数。与将 multiline 设为 `true` 一起使用，才能填满这些行。
...
当按键被按下时调用的回调。对于相应按键，将使用 `keyValue` 为 `'Enter'` 或 `'Backspace'` 的对象进行调用；其他情况下则为输入的字符，包括空格时的 `' '`。在 `onChange` 回调之前触发。注意：在 Android 上仅处理软键盘输入，不处理硬件键盘输入。
...
在内容滚动时触发。在 Android 上，虽然也可能包含 `ScrollEvent` 的其他属性，但出于性能原因不会提供 `contentSize`。
...
确定回车键的显示样式。在 Android 上也可以使用 `returnKeyLabel`。
...
如果为 `true`，则允许 `TextInput` 将触摸事件传递给父组件。这使得诸如 `SwipeableListView` 之类的组件在 iOS 上可以从 `TextInput` 处开始滑动，就像 Android 上的默认行为一样。如果为 `false`，`TextInput` 始终会请求处理输入（除非被禁用）。默认值为 `true`。
...
设置 `TextInput` 的行数。与将 multiline 设为 `true` 一起使用，才能填满这些行。
...
如果为 `true`，文本输入会将输入内容遮蔽，以保护密码等敏感文本的安全。默认值为 `false`。不适用于 `multiline={true}`。
...
文本输入选择范围的起始和结束位置。将 start 和 end 设置为相同值即可定位光标。
...
设置选择手柄的颜色。与 `selectionColor` 不同，它允许单独自定义选择手柄颜色，而不影响选择区域的颜色。
...
为键盘和系统提供关于用户输入内容预期语义含义的信息。

:::note
[`autoComplete`](#autocomplete) 提供相同功能，并且适用于所有平台。你可以使用 [`Platform.select`](/docs/next/platform#select) 来实现不同的平台行为。
...
`newPassword` 可用于指示用户可能希望保存到钥匙串中的新密码输入，而 `oneTimeCode` 可用于指示某个字段可被通过短信接收的验证码自动填充。
...
在 iOS 上将 `textContentType` 用作 `newPassword` 时，我们可以让操作系统了解密码的最低要求，从而生成一个满足条件的密码。要创建有效的 `PasswordRules` 字符串，请参阅 [Apple Docs](https://developer.apple.com/password-rules/)。

:::tip
如果密码生成对话框没有出现，请确保：

- 已启用 AutoFill：**Settings** → **Passwords & Accounts** → 将 **AutoFill Passwords** 切换为“On”；
- 正在使用 iCloud 钥匙串：**Settings** → **Apple ID** → **iCloud** → **Keychain** → 将 **iCloud Keychain** 切换为“On”。
  :::
...
在 Android API Level 23+ 上设置文本断行策略，可选值为 `simple`、`highQuality`、`balanced`。默认值为 `highQuality`。
...
显示在文本输入中的值。`TextInput` 是一个受控组件，这意味着如果提供了该值，原生值将被强制与此 `value` prop 保持一致。对大多数用途来说，这非常有效，但在某些情况下可能会导致闪烁——一种常见原因是通过保持值不变来阻止编辑。除了设置相同的值之外，还可以设置 `editable={false}`，或者设置/更新 `maxLength`，以在不闪烁的情况下防止不必要的编辑。
...
如果输入当前处于焦点状态则返回 `true`；否则返回 `false`。
