...
用于通过键盘向应用输入文本的基础组件。Props 提供了多项功能的配置选项，例如自动纠正、自动大写、占位文本和不同的键盘类型（如数字键盘）。
...
原生元素提供的两个方法是 `.focus()` 和 `.blur()`，可通过编程方式聚焦或取消聚焦 TextInput。
...
`TextInput` 默认会在其视图底部显示边框。此边框的内边距由系统提供的背景图像设置，且无法更改。避免此问题的方法有两种：不显式设置高度，此时系统会负责在正确的位置显示边框；或者将 `underlineColorAndroid` 设置为 transparent，以不显示边框。

请注意，在 Android 上对输入内容执行文本选择时，可能会将应用的 activity `windowSoftInputMode` 参数更改为 `adjustResize`。当键盘处于活动状态时，这可能会导致 `position: 'absolute'` 的组件出现问题。若要避免此行为，可以在 AndroidManifest.xml（ https://developer.android.com/guide/topics/manifest/activity-element.html ）中指定 `windowSoftInputMode`，或使用原生代码以编程方式控制此参数。
...
指定系统的自动完成提示，以便系统提供自动填充。在 Android 上，系统始终会尝试使用启发式方法识别内容类型并提供自动填充。若要禁用自动完成，请将 `autoComplete` 设置为 `off`。
...
如果为 `true`，文本字段将在提交时取消聚焦。单行字段的默认值为 true，多行字段的默认值为 false。请注意，对于多行字段，将 `blurOnSubmit` 设置为 `true` 意味着按下回车键时会取消字段聚焦并触发 `onSubmitEditing` 事件，而不是在字段中插入换行符。
...
指定清除按钮何时出现在文本视图的右侧。此属性仅支持单行 TextInput 组件。默认值为 `never`。
...
确定将文本输入中的哪些数据转换为可点击的 URL。仅当 `multiline={true}` 且 `editable={false}` 时有效。默认情况下不会检测任何数据类型。
...
提供一个初始值，该值会在用户开始输入时发生变化。适用于不希望处理事件监听并更新 value prop 来使受控状态保持同步的场景。
...
提供此属性时，会设置组件中光标（或“caret”）的颜色。与 `selectionColor` 的行为不同，光标颜色会独立于文本选择框的颜色进行设置。
...
当值为 `false` 时，如果文本输入周围可用空间较小（例如手机处于横屏方向），操作系统可能会让用户在全屏文本输入模式中编辑文本。当值为 `true` 时，此功能会被禁用，用户始终可以直接在文本输入框中编辑文本。默认值为 `false`。
...
告知操作系统，在 Android API Level 26+ 上，应用中的各个字段是否应包含在用于自动填充的视图结构中。可能的值为 `auto`、`no`、`noExcludeDescendants`、`yes` 和 `yesExcludeDescendants`。默认值为 `auto`。
...
将自定义 [InputAccessoryView](inputaccessoryview.md) 与此文本输入框关联的可选标识符。当此文本输入框聚焦时，InputAccessoryView 会渲染在键盘上方。
...
其工作方式类似于 HTML 中的 `inputmode` 属性，用于确定要打开的键盘，例如 `numeric`，并且其优先级高于 `keyboardType`。
...
请在[此处](https://davidl.fr/blog/keyboard-react-native-ios-android#all-react-native-keyboard-type-examples-i-os-on-the-left-android-on-the-right)查看所有类型的屏幕截图。
...
:::note
需要注意的是，在 iOS 上这会使文本顶部对齐，而在 Android 上会使文本居中对齐。将其与 `textAlignVertical` 设置为 `top` 一起使用，可使两个平台具有相同的行为。
:::
...
文本输入框的文本发生变化时调用的回调。更改后的文本会作为单个字符串参数传递给回调处理函数。
...
按下按键时调用的回调。对于相应的按键，此回调接收的对象中 `keyValue` 为 `'Enter'` 或 `'Backspace'`；对于其他按键，则为输入的字符，包括空格 `' '`。此回调会在 `onChange` 回调之前触发。注意：在 Android 上，仅处理软键盘的输入，不处理硬件键盘的输入。
...
如果为 `true`，则允许 TextInput 将触摸事件传递给父组件。这使得 SwipeableListView 之类的组件可以在 iOS 上从 TextInput 开始滑动，就像 Android 上默认的情况一样。如果为 `false`，TextInput 始终会请求处理输入（禁用时除外）。默认值为 `true`。
...
文本输入框选择内容的起始位置和结束位置。将 start 和 end 设置为相同的值，可以定位光标。
...
当值为 `false` 时，会阻止字段聚焦时显示软键盘。默认值为 `true`。
...
- `'submit'` 只发送提交事件，不取消输入框聚焦
- `'blurAndSubmit`' 同时取消输入框聚焦并发送提交事件
...
在 iOS 上将 `textContentType` 设置为 `newPassword` 时，可以告知 OS 密码的最低要求，以便 OS 生成符合要求的密码。要创建适用于 `PasswordRules` 的有效字符串，请参阅 [Apple Docs](https://developer.apple.com/password-rules/)。

:::tip
如果密码生成对话框没有出现，请确认：

- AutoFill 已启用：**Settings** → **Passwords & Accounts** → 将 **AutoFill Passwords** 切换为“On”
...
文本输入框要显示的值。`TextInput` 是受控组件，这意味着如果提供了此 value prop，原生值将被强制与该值保持一致。对于大多数用法，这种方式效果很好，但在某些情况下可能会导致闪烁——一个常见原因是通过保持 value 不变来阻止编辑。除了设置相同的值之外，还可以设置 `editable={false}`，或设置／更新 `maxLength`，以便在避免闪烁的情况下阻止不需要的编辑。
