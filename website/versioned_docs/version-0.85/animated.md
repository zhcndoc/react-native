...
创建动画的核心工作流程是创建一个 `Animated.Value`，将其挂钩到动画组件的一个或多个样式属性，然后通过使用 `Animated.timing()` 驱动动画更新。

:::note
不要直接修改动画值。您可以使用 [`useRef` Hook](https://react.dev/reference/react/useRef) 返回一个可变 ref 对象。此 ref 对象的 `current` 属性初始化为给定参数，并在整个组件生命周期中持久存在。
:::
...
参阅 [动画](animations#animated-api) 指南以查看 `Animated` 运行的更多示例。

## 概述

您可以配合 `Animated` 使用两种值类型：
...
`Animated` 提供三种动画类型。每种动画类型提供特定的动画曲线，控制您的值如何从初始值动画化到最终值：

- [`Animated.decay()`](animated#decay) 以初始速度开始并逐渐减速至停止。
- [`Animated.spring()`](animated#spring) 提供基本的弹簧物理模型。
- [`Animated.timing()`](animated#timing) 使用 [缓动函数](easing) 随时间动画化值。

在大多数情况下，您将使用 `timing()`。默认情况下，它使用对称的 easeInOut 曲线，传达物体逐渐加速至全速并通过逐渐减速至停止的过程。

### 使用动画

通过调用动画上的 `start()` 来启动动画。`start()` 接受一个完成回调，该回调将在动画完成时调用。如果动画正常完成，完成回调将使用 `{finished: true}` 调用。如果动画在完成之前被调用 `stop()` 而完成（例如，因为它被手势或另一个动画中断），那么它将接收 `{finished: false}`。
...
您可以通过加、减、乘、除或取模组合两个动画值以制作新的动画值：
...
`interpolate()` 函数允许输入范围映射到不同的输出范围。默认情况下，它将外推给定范围之外的曲线，但您也可以让它钳制输出值。默认情况下它使用线性插值，但也支持缓动函数。
...
- `damping`: 定义弹簧运动应如何因摩擦力而阻尼。默认 10。
...
创建一个新 Animated 值，它是提供的 Animated 值的（非负）取模结果
...
用于窥探通过 props 传递的动画事件的高级命令式 API。它允许向现有的 `AnimatedEvent` 添加新的 javascript 监听器。如果 `animatedEvent` 是 javascript 监听器，它将合并 2 个监听器为一个，如果 `animatedEvent` 是 null/undefined，它将直接分配 javascript 监听器。尽可能直接使用值。
...
用于将动画值附加到视图上的事件的命令式 API。如果可能，优先使用带有 `useNativeDriver: true` 的 `Animated.event`。
