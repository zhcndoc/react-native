...
`Systrace` 是一个标准的基于标记的 Android 性能分析工具（在安装 Android platform-tools 包时会安装）。被分析的代码块被起始/结束标记包围，随后以彩色图表格式可视化。Android SDK 和 React Native 框架都提供了可可视化的标准标记。
...
beginEvent/endEvent 用于在同一调用栈帧内开始并结束一个性能分析剖面。
...
beginAsyncEvent/endAsyncEvent 用于开始并结束一个性能分析剖面，其中结束可能发生在另一个线程或当前栈帧之外，例如，返回的 cookie 变量应作为输入传递给 endAsyncEvent 调用以结束该性能分析。
