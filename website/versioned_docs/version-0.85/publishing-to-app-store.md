...
每次针对物理设备构建时都会构建静态包，即使在 Debug 模式下也是如此。如果你想节省时间，可以通过在 Xcode Build Phase `Bundle React Native code and images` 的 shell 脚本中添加以下内容来关闭 Debug 模式下的包生成：
...
- 启动你的终端，导航进入应用的 iOS 文件夹并输入 `open .`。
