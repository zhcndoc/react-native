...
React Native 应用渲染一个代表 UI 的原生视图树，类似于 React DOM 在 Web 上的做法（DOM 树）。React Native 通过 [ref](https://react.dev/learn/manipulating-the-dom-with-refs) 提供对该树的命令式访问，所有原生组件都会返回 ref（包括由内置组件（如 [`View`](/docs/next/view)）渲染的组件）。
...
:::info
**与 Web 不同，这些节点不允许突变**（例如：[`node.appendChild`](https://developer.mozilla.org/en-US/docs/Web/API/Node/appendChild)），因为树的内容完全由 React 渲染器管理。
:::
