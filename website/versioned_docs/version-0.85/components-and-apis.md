...
React Native 提供了许多内置的 [核心组件](intro-react-native-components) 供你在应用中使用。你可以在左侧边栏（如果你在窄屏设备上，则在上方菜单）中找到它们。如果你不确定从哪里开始，请查看以下类别：
...
你并不局限于 React Native 捆绑的组件和 API。React Native 拥有一个数千名开发者的社区。如果你在寻找实现特定功能的库，请参考 [这份关于寻找库的指南](libraries#finding-libraries)。
...
<div className="component-grid component-grid-border">
  <div className="component">
    <a href="./button">
      <h3>Button</h3>
      <p>一个用于处理触摸的基础按钮组件，应在任何平台上都能良好渲染。</p>
    </a>
  </div>
  <div className="component">
    <a href="./switch">
...
与更通用的 [`ScrollView`](./scrollview) 不同，以下列表视图组件仅渲染当前显示在屏幕上的元素。这使得它们成为显示长数据列表的性能之选。
