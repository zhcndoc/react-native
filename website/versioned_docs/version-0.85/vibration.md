...
**在 Android 上，** 振动时长默认为 400 毫秒，可以通过传递一个数字作为 `pattern` 参数的值来指定任意振动时长。**在 iOS 上，** 振动时长固定为大约 400 毫秒。

`vibrate()` 方法可以接受一个 `pattern` 参数，该参数是一个代表毫秒时间的数字数组。你可以将 `repeat` 设置为 true 以循环运行振动模式，直到调用 `cancel()`。
