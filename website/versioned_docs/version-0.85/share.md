...
在 iOS 上，返回一个 Promise，该 Promise 将通过一个包含 `action` 和 `activityType` 的对象来调用。如果用户取消了对话框，Promise 仍会被解析，其中 action 为 `Share.dismissedAction`，所有其他键为 undefined。请注意，某些分享选项可能不会出现在 iOS 模拟器上或无法正常工作。

在 Android 上，返回一个 Promise，该 Promise 将始终被解析，其中 action 为 `Share.sharedAction`。

**属性：**

| 名称                                                         | 类型   | 描述                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------ | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| content <div className="label basic required">必需</div> | object | `message` - 要分享的消息<br/>`url` - 要分享的 URL <div className="label ios">iOS</div><br/>`title` - 消息的标题 <div className="label android">Android</div><hr/>`url` 和 `message` 至少需要提供一个。                                                                                                                                                          |
