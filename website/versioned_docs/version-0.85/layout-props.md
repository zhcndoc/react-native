...
`alignContent` 控制行如何在交叉方向对齐，覆盖父级的 `alignContent`。
...
`alignItems` 在交叉方向对齐子元素。例如，如果子元素垂直排列，`alignItems` 控制它们如何在水平方向对齐。它的工作方式类似于 CSS 中的 `align-items`（默认：stretch）。
...
`alignSelf` 控制子元素如何在交叉方向对齐，覆盖父级的 `alignItems`。它的工作方式类似于 CSS 中的 `align-self`（默认：auto）。
...
Aspect ratio 控制节点未定义维度的尺寸。

- 在设置了宽度/高度的节点上，aspect ratio 控制未设置维度的尺寸
- 在设置了 flex basis 的节点上，如果未设置，aspect ratio 控制节点在交叉轴上的尺寸
- 在具有测量函数的节点上，aspect ratio 的工作方式如同测量函数测量 flex basis
- 在具有 flex grow/shrink 的节点上，如果未设置，aspect ratio 控制节点在交叉轴上的尺寸
- Aspect ratio 会考虑最小/最大尺寸
...
`bottom` 是偏移此组件底部边缘的逻辑像素数量。

它的工作方式类似于 CSS 中的 `bottom`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
`boxSizing` 定义如何计算元素的各种尺寸属性（`width`、`height`、`minWidth`、`minHeight` 等）。如果 `boxSizing` 为 `border-box`，这些尺寸应用于元素的边框盒。如果为 `content-box`，它们应用于元素的内容盒。默认值为 `border-box`。如果您想了解更多关于此属性如何工作的信息，[Web 文档](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) 是一个很好的信息来源。
...
`direction` 指定用户界面的方向流。默认为 `inherit`，根节点除外，其值将基于当前区域设置。
...
它的工作方式类似于 CSS 中的 `display`，但仅支持值 'flex'、'none' 和 'contents'。默认为 `flex`。
...
当 `flex` 为正数时，它使组件具有弹性，并且其尺寸将与其 flex 值成比例。因此，`flex` 设置为 `2` 的组件将占据 `flex` 设置为 1 的组件两倍的空间。`flex: <正数>` 等同于 `flexGrow: <正数>, flexShrink: 1, flexBasis: 0`。

当 `flex` 为 `0` 时，组件根据 `width` 和 `height` 确定尺寸，且无弹性。

当 `flex` 为 `-1` 时，组件通常根据 `width` 和 `height` 确定尺寸。但是，如果没有足够的空间，组件将收缩至其 `minWidth` 和 `minHeight`。
...
`flexBasis` 是一种与轴无关的方式，用于提供项目沿主轴的默认尺寸。设置子元素的 `flexBasis` 类似于设置该子元素的 `width`（如果其父容器为 `flexDirection: row`）或设置子元素的 `height`（如果其父容器为 `flexDirection: column`）。项目的 `flexBasis` 是该项目的默认尺寸，即在任何 `flexGrow` 和 `flexShrink` 计算之前的尺寸。
...
`flexDirection` 控制容器子元素的走向。`row` 从左到右，`column` 从上到下，您可以猜到另外两个的作用。它的工作方式类似于 CSS 中的 `flex-direction`，但默认为 `column`。
...
`flexGrow` 描述容器内的任何空间应如何沿主轴在其子元素之间分配。在布局子元素后，容器将根据其子元素指定的 flex grow 值分配任何剩余空间。

`flexGrow` 接受任何 >= 0 的浮点值，0 为默认值。容器将根据子元素的 `flexGrow` 值加权分配任何剩余空间。
...
[`flexShrink`](layout-props#flexshrink) 描述在子元素总尺寸溢出容器主轴尺寸的情况下，如何沿主轴收缩子元素。`flexShrink` 与 `flexGrow` 非常相似，如果任何溢出尺寸被视为负剩余空间，则可以以相同方式思考。这两个属性还可以很好地配合工作，允许子元素按需增长和收缩。

`flexShrink` 接受任何 >= 0 的浮点值，0 为默认值。容器将根据子元素的 `flexShrink` 值加权收缩其子元素。
...
`flexWrap` 控制子元素在到达弹性容器末尾后是否可以换行。它的工作方式类似于 CSS 中的 `flex-wrap`（默认：nowrap）。

注意它不再与 `alignItems: stretch`（默认）一起工作，因此您可能想使用 `alignItems: flex-start` 例如（破坏性变更详情：https://github.com/facebook/react-native/releases/tag/v0.28.0）。
...
它的工作方式类似于 CSS 中的 `height`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
设置 `inset` 的效果等同于设置 `top`、`bottom`、`right` 和 `left` 属性。
...
`isolation` 允许您形成 [堆叠上下文](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_positioned_layout/Stacking_context)。
...
`justifyContent` 在主轴方向对齐子元素。例如，如果子元素垂直排列，`justifyContent` 控制它们如何垂直对齐。它的工作方式类似于 CSS 中的 `justify-content`（默认：flex-start）。
...
`left` 是偏移此组件左边缘的逻辑像素数量。

它的工作方式类似于 CSS 中的 `left`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
设置 `margin` 的效果等同于设置 `marginTop`、`marginLeft`、`marginBottom` 和 `marginRight`。
...
`marginBottom` 的工作方式类似于 CSS 中的 `margin-bottom`。参见 [MDN CSS 参考](https://developer.mozilla.org/en-US/docs/Web/CSS/margin-bottom) 了解更多详情。
...
设置 `marginHorizontal` 的效果等同于设置 `marginLeft` 和 `marginRight`。
...
`marginLeft` 的工作方式类似于 CSS 中的 `margin-left`。参见 [MDN CSS 参考](https://developer.mozilla.org/en-US/docs/Web/CSS/margin-left) 了解更多详情。
...
设置 `marginVertical` 的效果等同于设置 `marginTop` 和 `marginBottom`。
...
它的工作方式类似于 CSS 中的 `max-height`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
它的工作方式类似于 CSS 中的 `max-width`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
它的工作方式类似于 CSS 中的 `min-height`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
它的工作方式类似于 CSS 中的 `min-width`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
`overflow` 控制子元素如何测量和显示。`overflow: hidden` 导致视图被裁剪，而 `overflow: scroll` 导致视图独立于其父元素的主轴进行测量。它的工作方式类似于 CSS 中的 `overflow`（默认：visible）。
...
设置 `padding` 的效果等同于设置 `paddingTop`、`paddingBottom`、`paddingLeft` 和 `paddingRight`。
...
设置 `paddingHorizontal` 类似于设置 `paddingLeft` 和 `paddingRight`。
...
设置 `paddingVertical` 类似于设置 `paddingTop` 和 `paddingBottom`。
...
React Native 中的 `position` 类似于 [常规 CSS](https://developer.mozilla.org/en-US/docs/Web/CSS/position)，但默认都设置为 `relative`。

`relative` 将根据布局的正常流定位元素。Insets（`top`、`bottom`、`left`、`right`）将相对于此布局偏移。

`absolute` 将元素移出布局的正常流。Insets 将相对于其 [包含块](./flexbox.md#the-containing-block) 偏移。

`static` 将根据布局的正常流定位元素。Insets 将无效。
`static` 元素不为绝对后代形成包含块。

更多信息，请参阅 [Flexbox 布局文档](./flexbox.md#position)。另外，[Yoga 文档](https://www.yogalayout.dev/docs/styling/position) 有关於 `position` 在 React Native 和 CSS 之间差异的更多详情。
...
`right` 是偏移此组件右边缘的逻辑像素数量。

它的工作方式类似于 CSS 中的 `right`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
`top` 是偏移此组件顶部边缘的逻辑像素数量。

它的工作方式类似于 CSS 中的 `top`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
它的工作方式类似于 CSS 中的 `width`，但在 React Native 中您必须使用点数或百分比。不支持 Em 及其他单位。
...
`zIndex` 控制哪些组件显示在其他组件之上。通常，您不使用 `zIndex`。组件根据其在文档树中的顺序渲染，因此后面的组件绘制在前面的组件之上。如果您有不希望此行为的动画或自定义模态界面，`zIndex` 可能很有用。

它的工作方式类似于 CSS `z-index` 属性 - 具有较大 `zIndex` 的组件将渲染在顶部。想象 z 方向就像从手机指向你的眼球。
