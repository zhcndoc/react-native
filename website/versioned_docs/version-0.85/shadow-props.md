...
- `boxShadow`：View 样式属性，以及与 [同名的 Web 样式属性](https://developer.mozilla.org/en-US/docs/Web/CSS/box-shadow) 规范一致的实现。
...
总体而言，`boxShadow` 和 `dropShadow` 的能力都比 `shadow` 属性更强。不过，`shadow` 属性映射到原生平台级 API，因此如果你只需要简单直接的阴影，推荐使用这些属性。请注意，只有 `shadowColor` 同时适用于 Android 和 iOS，其他所有 `shadow` 属性都仅适用于 iOS。
