...
当下一个布局发生时，自动将视图动画过渡到新位置。

在函数组件中，常见的使用方式是在更新状态 hook 之前调用它；在类组件中，则是在调用 `setState` 之前调用。

请注意，为了让这在 **Android** 上生效，你需要通过 `UIManager` 设置以下标志：
...
| onAnimationDidEnd  | function | 否   | 动画结束时调用。             |
...
- `springDamping`（数值，可选，仅在 `type: Type.spring` 时使用）
...
辅助方法，用于创建一个对象（包含 `create`、`update` 和 `delete` 字段），并将其传入 [`configureNext`](layoutanimation.md#configurenext)。`type` 参数是一个[动画类型](layoutanimation.md#types)，`creationProp` 参数是一个[布局属性](layoutanimation.md#properties)。
