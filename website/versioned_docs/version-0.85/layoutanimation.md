...
使用此 API 的一种常见方法是在函数组件中更新 state hook 之前调用它，以及在类组件中调用 `setState` 之前调用它。
...
- `springDamping`（number，可选，仅与 `type: Type.spring` 一起使用）
...
一组预定义的动画配置以传递给 [`configureNext`](layoutanimation.md#configurenext)。
