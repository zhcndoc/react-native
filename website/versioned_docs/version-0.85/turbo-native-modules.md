...
你的 React Native 应用代码可能需要与原生平台 API 交互，而这些 API 并未由 React Native 或现有库提供。你可以使用 **Turbo Native Module** 自行编写集成代码。本指南将向你展示如何编写一个。

基本步骤如下：

1. 使用最流行的 JavaScript 类型注解语言之一：Flow 或 TypeScript，**定义一个带类型的 JavaScript 规范**；
2. **配置你的依赖管理系统运行 Codegen**，它会将该规范转换为原生语言接口；
3. **使用你的规范编写应用代码**；以及
4. **使用生成的接口编写原生平台代码**，将你的原生代码接入 React Native 运行时环境。

让我们通过构建一个 Turbo Native Module 示例来逐步完成这些步骤。本指南的其余部分假设你已经通过运行以下命令创建了应用：
...
本指南将向你展示如何实现 [Web Storage API](https://html.spec.whatwg.org/multipage/webstorage.html#dom-localstorage-dev)：`localStorage`。对于正在你的项目中编写应用代码的 React 开发者来说，这个 API 很容易理解。

为了让它在移动端可用，我们需要使用 Android 和 iOS API：
...
React Native 提供了一个名为 [Codegen](/docs/the-new-architecture/what-is-codegen) 的工具，它会接收使用 TypeScript 或 Flow 编写的规范，并为 Android 和 iOS 生成平台特定代码。该规范声明了将在你的原生代码与 React Native JavaScript 运行时之间来回传递的方法和数据类型。Turbo Native Module 既包括你的规范、你编写的原生代码，也包括根据你的规范生成的 Codegen 接口。
...
:::info
你可以在 [附录](/docs/appendix) 文档中查看规范中可使用的所有类型，以及生成的原生类型。
:::
...
该规范会被 React Native Codegen 工具用于为我们生成平台特定接口和样板代码。为此，Codegen 需要知道去哪里查找我们的规范，以及如何处理它。请在你的 `package.json` 中添加：
...
配置好 Codegen 之后，我们需要准备原生代码，以便接入生成的代码。
...
使用 `NativeLocalStorage`，下面是一个修改后的 `App.tsx`，它包含我们想要持久化的一些文本、一个输入框以及一些用于更新该值的按钮。
...
一切准备就绪后，我们将开始编写原生平台代码。我们分为两部分来做：
