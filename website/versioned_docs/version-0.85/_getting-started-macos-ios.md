...
虽然你可以使用任何编辑器来开发应用，但你需要安装 Xcode 以便设置构建 iOS React Native 应用所需的必要工具。
...
安装 Xcode 最简单的方法是通过 [Mac App Store](https://itunes.apple.com/us/app/xcode/id497799835?mt=12)。安装 Xcode 还会安装 iOS 模拟器以及构建 iOS 应用所需的所有必要工具。
...
要安装模拟器，打开 **Xcode > Settings... (或 Preferences...)** 并选择 **Platforms (或 Components)** 选项卡。选择与你希望使用的 iOS 版本相对应的模拟器。

如果你使用的是 Xcode 14.0 或更高版本来安装模拟器，打开 **Xcode > Settings > Platforms** 选项卡，然后点击 "+" 图标并选择 **iOS…** 选项。

#### CocoaPods

[CocoaPods](https://cocoapods.org/) 是 iOS 可用的依赖管理系统之一。CocoaPods 是一个 Ruby [gem](https://en.wikipedia.org/wiki/RubyGems)。你可以使用最新版 macOS 自带的 Ruby 版本来安装 CocoaPods。
...
你可能还希望确保 Xcode 项目的所有 "shell script build phase" 都使用 `/bin/zsh` 作为其 shell。
:::
