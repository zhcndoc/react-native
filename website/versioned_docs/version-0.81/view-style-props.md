...
为元素添加阴影效果，能够控制阴影的位置、颜色、大小和模糊度。根据阴影是否为 _内嵌 (inset)_，此阴影要么出现在元素边框盒的外部，要么出现在内部。这是 [同名的 web 样式属性](https://developer.mozilla.org/en-US/docs/Web/CSS/box-shadow) 的规范兼容实现。阅读 [BoxShadowValue](./boxshadowvalue) 文档以了解所有可用参数的更多信息。
...
设置视图的 elevation，使用 Android 底层的 [elevation API](https://developer.android.com/training/material/shadows-clipping.html#Elevation)。这会给项目添加投影并影响重叠视图的 z-order。仅支持 Android 5.0+，早期版本无效。
...
为 `View` 添加图形滤镜。此滤镜由任意数量的 _滤镜函数_ 组成，每个函数代表对 `View` 图形组合的一些原子更改。有效滤镜函数的完整列表定义如下。`filter` 将应用于 `View` 的后代以及 `View` 本身。`filter` 隐含 `overflow: hidden`，因此后代将被裁剪以适应 `View` 的边界。
...
- `dropShadow`: 在 `View` 的 alpha 遮罩周围添加阴影（只有 `View` 中非零 alpha 像素才会投射阴影）。接受一个可选颜色代表阴影颜色，以及 2 或 3 个长度。如果指定了 2 个长度，它们被解释为 `offsetX` 和 `offsetY`，将分别在 X 和 Y 维度上平移阴影。如果给出第 3 个长度，它被解释为用于阴影的高斯模糊的标准差——因此较大的值会使阴影更模糊。阅读 [DropShadowValue](./dropshadowvalue.md) 中的参数更多信息。
...
- `sepia`: 将 `View` 转换为 [深褐色](<https://en.wikipedia.org/wiki/Sepia_(color)>)。接受非负数字或百分比，其中 `1` 或 `100%` 代表完全深褐色。
...
`filter` 可以接受由上述滤镜函数组成的对象数组，或模拟 [Web 语法](https://developer.mozilla.org/en-US/docs/Web/CSS/filter#syntax) 的字符串。
...
对于更细粒度的控制，请参阅 [isolation](layout-props#isolation)。
...
设置元素轮廓的颜色。参见 [web 文档](https://developer.mozilla.org/en-US/docs/Web/CSS/outline-color) 了解更多详情。
...
设置轮廓与元素边界之间的空间量。不影响布局。参见 [Web 文档](https://developer.mozilla.org/en-US/docs/Web/CSS/outline-offset) 了解更多详情。
...
设置元素轮廓的样式。参见 [Web 文档](https://developer.mozilla.org/en-US/docs/Web/CSS/outline-style) 了解更多详情。
...
绘制在元素周围、边框外部的轮廓宽度。不影响布局。参见 [Web 文档](https://developer.mozilla.org/en-US/docs/Web/CSS/outline-width) 了解更多详情。
