...
接下来我们需要创建 `NativeLocalStoragePackage`。它通过将其包装为 Base Native Package，提供一个对象以便在 React Native 运行时中注册我们的模块：
...
:::info
稍后你将了解如何将你的原生模块作为 [npm 包](the-new-architecture/create-module-library.mdx#publish-the-library-on-npm) 分发，我们的构建工具会为你自动链接。
:::
