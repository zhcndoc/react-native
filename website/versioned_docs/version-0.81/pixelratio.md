...
如果你在高像素密度设备上，你应该获取更高分辨率的图像。一个经验法则是将你显示的图像尺寸乘以像素比。
...
在 iOS 上，你可以为元素指定任意精度的位置和尺寸，例如 29.674825。但是，最终物理显示屏只有固定数量的像素，例如 iPhone SE（第 1 代）为 640×1136，或 iPhone 11 为 828×1792。iOS 试图通过将一个原始像素扩展到多个像素来欺骗眼睛，从而尽可能忠实地反映用户值。这种技术的缺点是它会使生成的元素看起来模糊。
...
返回字体大小的缩放因子。这是用于计算绝对字体大小的比率，因此任何严重依赖于此的元素都应使用此方法进行计算。
...
将布局尺寸 (dp) 舍入到最接近的对应整数像素数的布局尺寸。例如，在 PixelRatio 为 3 的设备上，`PixelRatio.roundToNearestPixel(8.4) = 8.33`，这正好对应 (8.33 \* 3) = 25 像素。
