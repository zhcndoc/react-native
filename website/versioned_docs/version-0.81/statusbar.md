...
对于不使用组件更理想的情况，还有一个作为组件上的静态函数暴露的命令式 API。然而，不建议对同一个 prop 同时使用静态 API 和组件，因为静态 API 设置的任何值将在下一次渲染中被组件设置的值覆盖。
...
| props <div className="label basic required">必填</div> | any  | 包含要在替换栈条目中使用的 StatusBar props 的对象。 |
