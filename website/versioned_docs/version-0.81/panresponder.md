...
`PanResponder` 将多个触摸整合为一个手势。它能让单点触摸手势在额外触摸出现时依然保持稳定，也可用于识别基本的多点触摸手势。

默认情况下，`PanResponder` 会持有一个 `InteractionManager` 句柄，用于阻止长时间运行的 JS 事件打断当前进行中的手势。
...
- `stateID` - gestureState 的 ID，只要屏幕上至少还有一个触摸点就会一直保持
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        // 手势已经开始。显示视觉反馈，让用户知道
        // 发生了什么！
        // 现在 gestureState.d{x,y} 将被设为 0
      },
      onPanResponderMove: (evt, gestureState) => {
        // 最近一次移动的距离是 gestureState.move{X,Y}
...
| ----------------------------------------------------------- | ------ | ----------- |
| config <div className="label basic required">Required</div> | object | 参考下文 |

`config` 对象为所有 responder 回调提供了增强版本：它不仅提供 [`PressEvent`](pressevent)，还通过将每个典型的 `onResponder*` 回调中的 `Responder` 替换为 `PanResponder`，同时提供 `PanResponder` 的手势状态。例如，`config` 对象如下所示：
...
请注意 `onStartShould*` 回调。它们只会反映会 bubble/capture 到 Node 的 start/end 事件的更新后 `gestureState`。一旦该节点成为 responder，你就可以依赖每个 start/end 事件都由手势处理，并且 `gestureState` 会相应更新。若你不是 responder，`numberActiveTouches` 可能并不完全准确。
