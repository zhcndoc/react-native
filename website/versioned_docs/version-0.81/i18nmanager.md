...
如果你绝对定位元素以与其他 flexbox 元素对齐，它们在 RTL 语言中可能无法对齐。使用 `isRTL` 可以调整对齐或动画。
...
- 否则，在以下情况下 `isRTL` 将为 `true`：
  - **iOS：**
    - 设备上的用户首选语言是 RTL 语言
    - 应用定义的本地化包含用户选择的语言（如在 Xcode 项目文件 (`knownRegions = (...)`) 中定义）
  - **Android：**
    - 设备上的用户首选语言是 RTL 语言
...
一个布尔值，指示在 RTL 模式下是否应自动交换左右样式属性。启用后，在 RTL 布局中左变为右，右变为左。
...
强制应用使用 RTL 布局，无论设备语言设置如何。这主要用于开发期间测试 RTL 布局。
...
- 设置在应用重启后持久保存
- 仅用于开发和测试。在生产环境中，你应该完全禁止 RTL 或适当处理它（参见 `isRTL`）
...
在 RTL 模式下交换左右样式属性。启用后，在 RTL 布局中左变为右，右变为左。不影响 `isRTL` 的值。
