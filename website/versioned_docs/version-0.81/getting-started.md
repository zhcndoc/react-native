...
您也可以不使用框架直接使用 React Native，但我们发现大多数开发者通过使用像 [Expo](https://expo.dev) 这样的 React Native 框架受益良多。Expo 提供了基于文件的路由、高质量的通用库以及编写插件修改原生代码而无需管理原生文件等功能。

<details>
<summary>我可以不使用框架直接使用 React Native 吗？</summary>

可以。您可以不使用框架直接使用 React Native。**但是，如果您正在使用 React Native 构建新应用，我们建议使用框架。**
...
如果没有框架，您要么必须编写自己的解决方案来实现核心功能，要么必须拼凑一组现有的库来创建框架的骨架。这需要付出真正的努力，无论是在启动应用时，还是在后来维护它时。
