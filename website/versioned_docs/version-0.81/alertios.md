...
| callbackOrButtons | ?(() => void),[ButtonsArray](alertios#buttonsarray) | 否       | 此可选参数应为单参数函数或按钮数组。如果传递函数，当用户点击 'OK' 时将被调用。如果传递按钮配置数组，每个按钮应包含 `text` 键，以及可选的 `onPress` 和 `style` 键。`style` 应为 'default'、'cancel' 或 'destructive' 之一。 |
...
| callbackOrButtons | ?((text: string) => void),[ButtonsArray](alertios#buttonsarray) | 否       | 此可选参数应为单参数函数或按钮数组。如果传递函数，当用户点击 'OK' 时将以提示框的值调用该函数。如果传递按钮配置数组，每个按钮应包含 `text` 键，以及可选的 `onPress` 和 `style` 键（见示例）。`style` 应为 'default'、'cancel' 或 'destructive' 之一。 |
