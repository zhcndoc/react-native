...
除了 `https`，你可能还熟悉 `mailto` 方案。当你打开一个带有 mailto 方案的链接时，操作系统将打开已安装的邮件应用程序。同样，也有用于拨打电话和发送短信的方案。请在下方阅读更多关于 [内置 URL](#built-in-url-schemes) 方案的信息。

就像使用 mailto 方案一样，也可以使用自定义 URL 方案链接到其他应用程序。例如，当你收到来自 Slack 的 **Magic Link** 电子邮件时，**Launch Slack** 按钮是一个锚标签，其 href 看起来像这样：`slack://secret/magic-login/other-secret`。与 Slack 一样，你可以告诉操作系统你想要处理一个自定义方案。当 Slack 应用打开时，它会收到用于打开它的 URL。这通常被称为深链接。阅读更多关于如何 [获取深链接](#get-the-deep-link) 到你的应用的信息。
...
> **注意：** 在 iOS 上，你需要按照 [此处](linking-libraries-ios#step-3) 步骤 3 中的描述将 `LinkingIOS` 文件夹添加到你的头文件搜索路径中。如果你还希望在应用执行期间监听传入的应用链接，你需要将以下行添加到你的 `*AppDelegate.m` 中：
...
#### 2. 如果应用尚未打开，它将被打开并且 URL 将作为 initialURL 传入
...
const OpenURLButton = ({url, children}) => {
  const handlePress = useCallback(async () => {
    // 检查是否支持具有自定义 URL 方案的链接。
    const supported = await Linking.canOpenURL(url);

    if (supported) {
      // 用某个应用打开链接，如果 URL 方案是 "http"，网页链接应该被打开
      // 由手机上的某个浏览器
...
通过监听 `url` 事件类型并提供处理程序，来添加一个用于处理 Linking 变化的处理器。
...
> 此方法在 iOS 9+ 上有限制。来自 [苹果官方文档](https://developer.apple.com/documentation/uikit/uiapplication/1622952-canopenurl)：
>
...
你可以使用其他 URL，例如位置（例如 Android 上的 "geo:37.484847,-122.148386" 或 iOS 上的 "https://maps.apple.com/?ll=37.484847,-122.148386"）、联系人，或任何可以用已安装应用打开的其他 URL。

该方法返回一个 `Promise` 对象。如果用户确认打开对话框或 url 自动打开，promise 将 resolve。如果用户取消打开对话框或没有为该 url 注册的应用，promise 将 reject。
