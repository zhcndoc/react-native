...
滚动到指定 `sectionIndex` 和 `itemIndex`（在部分内）的项，并将其定位在可见区域，使得 `viewPosition` 0 将其置于顶部（可能被粘性头部覆盖），1 置于底部，0.5 居中。

> 注意：如果不指定 `getItemLayout` 或 `onScrollToIndexFailed` 属性，则无法滚动到渲染窗口之外的位置。
...
| 名称                                                      | 类型               | 描述                                                                                                                                                         |
| --------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| data <div className="label basic required">必需</div> | array              | 用于渲染此部分中项的数据。对象数组，类似于 [`FlatList` 的 data 属性](flatlist#required-data)。                                         |
| key                                                       | string             | 可选的 key，用于跟踪部分的重新排序。如果您不打算重新排序部分，默认将使用数组索引。                              |
| renderItem                                                | function           | 可选地为此部分定义任意项渲染器，覆盖列表默认的 [`renderItem`](sectionlist#renderitem)。                          |
| ItemSeparatorComponent                                    | component, element | 可选地为此部分定义任意项分隔符，覆盖列表默认的 [`ItemSeparatorComponent`](sectionlist#itemseparatorcomponent)。 |
| keyExtractor                                              | function           | 可选地为此部分定义任意 key 提取器，覆盖列表默认的 [`keyExtractor`](sectionlist#keyextractor)。                                   |
