...
你的 React Native 应用代码可能需要与 React Native 本身或现有库未提供的原生平台 API 进行交互。你可以使用一个 **Turbo 原生模块** 自行编写集成代码。本指南将向你展示如何编写它。

基本步骤如下：

1. **定义一个带类型的 JavaScript 规范**，使用最流行的 JavaScript 类型注解语言之一：Flow 或 TypeScript；
2. **配置你的依赖管理系统以运行 Codegen**，它会将该规范转换为原生语言接口；
3. **使用你的规范编写应用代码**；以及
4. **使用生成的接口编写你的原生平台代码**，以编写原生代码并将其接入 React Native 运行时环境。

让我们通过构建一个示例 Turbo 原生模块来逐步完成这些步骤。本指南的其余部分假设你已经通过运行以下命令创建了应用：
...
本指南将展示如何实现 [Web Storage API](https://html.spec.whatwg.org/multipage/webstorage.html#dom-localstorage-dev)：`localStorage`。这个 API 对于可能正在为你的项目编写应用代码的 React 开发者来说很容易理解。

为了在移动端实现这一点，我们需要使用 Android 和 iOS API：
...
React Native 提供了一个名为 [Codegen](/the-new-architecture/what-is-codegen.mdx) 的工具，它会读取用 TypeScript 或 Flow 编写的规范，并为 Android 和 iOS 生成平台特定的代码。该规范声明了原生代码与 React Native JavaScript 运行时之间来回传递的方法和数据类型。Turbo 原生模块既包括你的规范、你编写的原生代码，也包括根据你的规范生成的 Codegen 接口。
...
:::info
你可以在[附录](/appendix.md)文档中查看规范中可使用的所有类型，以及生成的原生类型。
:::

:::info
如果你想更改模块名称和相关 specs 文件的名称，请务必始终使用 'Native' 作为前缀（例如 `NativeStorage` 或 `NativeUsersDefault`）。
:::

下面是 `localStorage` 规范的实现：
...
该规范会被 React Native Codegen 工具用于为我们生成平台特定的接口和样板代码。为此，Codegen 需要知道去哪里找到我们的规范，以及如何处理它。将你的 `package.json` 更新为包含以下内容：
...
在为 Codegen 完成所有配置后，我们需要准备原生代码以接入生成的代码。
...
使用 `NativeLocalStorage`，下面是一个修改后的 `App.tsx`，其中包含我们想要持久化的一些文本、一个输入框以及一些用于更新该值的按钮。
...
在一切准备就绪后，我们将开始编写原生平台代码。我们分两部分来做：
