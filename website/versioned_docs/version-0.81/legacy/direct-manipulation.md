...
直接操作不会是你经常使用的工具。你通常只在创建连续动画时使用它，以避免渲染组件层次结构和协调许多视图的开销。
`setNativeProps` 是命令式的，并将状态存储在本地层（DOM、UIView 等），而不是在你的 React 组件内，这使得你的代码更难理解。
...
这允许我们编写以下代码，并知道子组件的透明度会响应点击而更新，而子组件无需知晓该事实或对其实现进行任何更改：
...
假设 `setNativeProps` 不可用。我们在这种约束下实现它的一种方法是将透明度值存储在 state 中，然后在每次触发 `onPress` 时更新该值：
...
与原始示例相比，这在计算上更密集 —— 每次透明度变化时，React 都需要重新渲染组件层次结构，即使视图及其子组件的其他属性并未改变。通常这种开销不是问题，但在执行连续动画和响应手势时，明智地优化组件可以提高动画的保真度。
...
复合组件没有原生视图支持，因此你不能在其上调用 `setNativeProps`。考虑这个例子：
...
如果你运行这个，你会立即看到此错误：`Touchable child must either be native or forward setNativeProps to a native component`。发生这种情况是因为 `MyButton` 没有直接由需要设置透明度的原生视图支持。你可以这样想：如果你使用 `createReactClass` 定义组件，你不会期望能够在其上设置 style prop 并使其生效 —— 你需要将 style prop 传递给子组件，除非你包装的是原生组件。同样，我们将把 `setNativeProps` 转发给由原生支持的子组件。

#### 将 setNativeProps 转发给子组件

由于 `setNativeProps` 方法存在于任何 `View` 组件的 ref 上，因此只需将自定义组件上的 ref 转发到它渲染的 `<View />` 组件之一就足够了。这意味着在自定义组件上调用 `setNativeProps` 的效果与在被包装的 `View` 组件本身上调用 `setNativeProps` 的效果相同。
...
你可能注意到我们使用 `{...props}` 将所有 props 传递给了子视图。原因是 `TouchableOpacity` 实际上是一个复合组件，因此除了依赖其子组件上的 `setNativeProps` 外，它还要求子组件执行触摸处理。为此，它传递了 [各种 props](view.md#onmoveshouldsetresponder) 回调到 `TouchableOpacity` 组件。相比之下，`TouchableHighlight` 由原生视图支持，只要求我们实现 `setNativeProps`。

## 使用 setNativeProps 编辑 TextInput 值

`setNativeProps` 的另一个非常常见的用例是编辑 TextInput 的值。当 `bufferDelay` 较低且用户输入很快时，TextInput 的 `controlled` prop 有时会丢失字符。一些开发者倾向于完全跳过此 prop，而是在必要时使用 `setNativeProps` 直接操作 TextInput 值。例如，以下代码演示了点击按钮时编辑输入：
...
你可以使用 [`clear`](../textinput#clear) 方法清除 `TextInput`，它使用相同的方法清除当前输入文本。
...
此处描述的方法可用于 React Native 提供的大多数默认组件。但请注意，它们 _不_ 适用于没有直接由原生视图支持的复合组件。这通常包括你在自己的应用中定义的大多数组件。
...
:::note
此方法也可以使用 `relativeToNativeNode` 处理程序（而不是引用）调用，但这种变体在新架构中已过时。
:::
