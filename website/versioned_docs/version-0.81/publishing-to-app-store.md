...
每次针对物理设备构建时都会生成静态包，即使在 Debug 模式下。如果你想节省时间，可以通过在 Xcode Build Phase `Bundle React Native code and images` 的 shell 脚本中添加以下内容来关闭 Debug 模式下的包生成：
...
你现在可以通过按 <kbd>Cmd ⌘</kbd> + <kbd>B</kbd> 或从菜单栏选择 **Product** → **Build** 来构建你的发布版本应用。一旦构建为发布版本，你将能够将应用分发给测试人员并提交应用到 App Store。
...
Apple Store 要求你提供最新设备的截图。此类设备的参考信息可在 [这里](https://developer.apple.com/help/app-store-connect/reference/screenshot-specifications/) 找到。请注意，如果提供了其他尺寸的截图，则不需要提供某些显示尺寸的截图。
