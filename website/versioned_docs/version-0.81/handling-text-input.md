...
你还可以对文本输入做更多事情。例如，你可以在用户输入时验证文本。有关更详细的示例，请参阅 [关于受控组件的 React 文档](https://react.dev/reference/react-dom/components/input#controlling-an-input-with-a-state-variable)，或 [TextInput 的参考文档](textinput.md)。

文本输入是用户与应用程序交互的方式之一。接下来，让我们看看另一种输入类型并 [学习如何处理触摸](handling-touches.md)。
