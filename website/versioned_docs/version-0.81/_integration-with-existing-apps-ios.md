...
在阅读本指南时，我们建议你将 [React Native Community Template](https://github.com/react-native-community/template/) 作为参考。该模板包含一个**最小化的 iOS 应用**，可以帮助你理解如何将 React Native 集成到现有的 iOS 应用中。

## 前提条件

请先阅读 [配置开发环境](set-up-your-environment) 以及 [不使用框架使用 React Native](getting-started-without-a-framework) 的指南，为构建 iOS 上的 React Native 应用配置开发环境。
本指南还默认你已经熟悉 iOS 开发基础，例如创建 `UIViewController` 以及编辑 `Podfile` 文件。

### 1. 设置目录结构

为确保顺利体验，请为你的集成 React Native 项目创建一个新文件夹，然后将你现有的 iOS 项目**移动到 `/ios` 子文件夹**中。
...
这会将社区模板中的 <RNTemplateRepoLink href="template/package.json">`package.json` 文件</RNTemplateRepoLink>复制到你的项目中。
...
安装命令行工具。在 Xcode 菜单中选择 **设置...（或偏好设置...）**。进入 Locations 面板，并在 Command Line Tools 下拉菜单中选择最新版本来安装工具。
...
Xcode 16 生成项目的方式与之前版本略有不同，你需要最新的 CocoaPods 和 Xcodeproj gem 才能正常工作。
:::
...
请把社区模板作为 <RNTemplateRepoLink href="template/Gemfile">Gemfile</RNTemplateRepoLink> 和 <RNTemplateRepoLink href="template/ios/Podfile">Podfile</RNTemplateRepoLink> 的参考。
...
第一个命令会安装 Ruby 依赖，第二个命令会实际将 React Native 代码集成到你的应用中，这样你的 iOS 文件就可以导入 React Native 头文件。

## 5. 编写 TypeScript 代码

现在我们将真正修改原生 iOS 应用，以集成 React Native。

我们首先要编写的是新屏幕的 React Native 代码，这个屏幕将被集成到我们的应用中。

### 创建 `index.js` 文件

首先，在 React Native 项目的根目录下创建一个空的 `index.js` 文件。

`index.js` 是 React Native 应用的入口点，并且始终是必需的。它可以是一个较小的文件，用于 `import` 其他属于你的 React Native 组件或应用的文件；也可以包含所需的全部代码。
...
一旦该类初始化完成，你既可以通过提供 `UIWindow` 对象来启动 React Native 视图，也可以让工厂生成一个 `UIView`，然后将其加载到任意 `UIViewController` 中。

在下面的示例中，我们将创建一个可以将 React Native 视图作为其 `view` 加载的 ViewController。

#### 创建 `ReactViewController`

创建一个新的模板文件（<kbd>⌘</kbd>+<kbd>N</kbd>），并选择 Cocoa Touch Class 模板。
...
最后，我们可以展示我们的 React Native 视图。为此，我们需要一个新的 View Controller，用来承载我们可以加载 JS 内容的视图。
我们已经有了初始的 `ViewController`，并且可以让它展示 `ReactViewController`。实现方式有很多，取决于你的应用。此示例中，我们假设你有一个按钮用于以模态方式展示 React Native。
...
确保禁用 Sandbox scripting。为此，在 Xcode 中点击你的应用，然后进入 build settings。筛选 script，并将 `User Script Sandboxing` 设置为 `NO`。此步骤对于正确在 React Native 附带的 [Hermes 引擎](https://github.com/facebook/hermes/blob/main/README.md) 的 Debug 和 Release 版本之间切换是必要的。

![禁用沙盒](/docs/assets/disable-sandboxing.png)

最后，确保在你的 `Info.plist` 文件中添加 `UIViewControllerBasedStatusBarAppearance` 键，并将其值设置为 `NO`。

![禁用 UIViewControllerBasedStatusBarAppearance](/docs/assets/disable-UIViewControllerBasedStatusBarAppearance.png)

## 6. 测试你的集成

你已经完成了将 React Native 与应用程序集成所需的所有基础步骤。现在我们将启动 [Metro bundler](https://metrobundler.dev/)，把你的 TypeScript 应用代码构建为一个 bundle。Metro 的 HTTP 服务器会将你开发环境中 `localhost` 上的 bundle 分享到模拟器或设备上。这支持 [热重载](https://reactnative.dev/blog/2016/03/24/introducing-hot-reloading)。
...
配置文件准备好后，你就可以运行 bundler 了。在项目根目录中执行以下命令：
...
你也可以使用 Xcode 来创建发布构建！唯一额外的一步，是添加一个在应用构建时执行的脚本，用于把你的 JS 和图片打包进 iOS 应用。
...
在某些情况下，你可能希望从原生应用向 JavaScript 传递一些信息。例如，你可能想把当前登录用户的用户 ID 以及一个可用于从数据库中获取信息的 token 传给 React Native。

这可以通过 `RCTReactNativeFactory` 类的 `view(withModuleName:initialProperty)` 重载中的 `initialProperties` 参数来实现。下面的步骤将告诉你如何完成。
...
此时你可以像往常一样继续开发你的应用。请参考我们的 [调试](debugging) 和 [部署](running-on-device) 文档，了解更多关于使用 React Native 的内容。
