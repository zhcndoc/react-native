...
用 C++ 编写模块是 Android 和 iOS 之间共享平台无关代码的最佳方式。对于纯 C++ 模块，你只需编写一次逻辑，就可以立即在所有平台上复用，而无需编写平台特定代码。
...
纯 C++ Turbo Native Modules 本身就是 Turbo Native Modules。它们需要一个规范文件（也称为 spec 文件），这样 Codegen 才能为我们创建脚手架代码。规范文件也是我们在 JS 中访问 Turbo Native Module 的方式。

spec 文件需要使用带类型的 JS 方言编写。React Native 目前支持 Flow 或 TypeScript。
...
下一步是在 `package.json` 中配置 [Codegen](what-is-codegen.mdx)。将文件更新为包含以下内容：
...
编写 C++ Turbo Native Module 可以让你在 Android 和 iOS 之间共享代码。因此，我们将只编写一次代码，然后查看为了让 C++ 代码被正确接入，需要对各个平台做哪些修改。
...
- `NativeSampleModule.h` 文件是纯 C++ TurboModule 的头文件。`include` 语句确保我们包含了由 Codegen 生成的规范，其中包含我们需要实现的接口和基类。
- 该模块位于 `facebook::react` 命名空间中，以便访问该命名空间中的所有类型。
- `NativeSampleModule` 类是实际的 Turbo Native Module 类，它继承了 `NativeSampleModuleCxxSpec` 类；后者包含一些连接代码和样板代码，使这个类能够表现得像一个 Turbo Native Module。
- 最后，我们有构造函数，它接收一个指向 `CallInvoker` 的指针，以便在需要时与 JS 通信，还有我们必须实现的函数原型。

`NativeSampleModule.cpp` 文件是我们的 Turbo Native Module 的实际实现，它实现了构造函数和我们在规范中声明的方法。

## 4. 在平台中注册模块

接下来的步骤将让我们在平台中注册这个模块。这一步会将原生代码暴露给 JS，使 React Native 应用最终可以从 JS 层调用原生方法。
...
3. 在 Android 应用中创建一个 `OnLoad.cpp` 文件来注册新的 Turbo Native Module。

#### 1. 创建 `CMakeLists.txt` 文件

Android 使用 CMake 来构建。CMake 需要访问我们在共享文件夹中定义的文件，才能构建它们。

1. 创建一个新文件夹 `SampleApp/android/app/src/main/jni`。`jni` 文件夹是 Android 的 C++ 侧所在的位置。
...
# 此文件包含所有必要内容，可让你构建 React Native 应用
include(${REACT_ANDROID_DIR}/cmake-utils/ReactNative-application.cmake)

# 定义附加源代码的位置。我们需要从 jni、main、src、app、android 这些目录一路返回
target_sources(${CMAKE_PROJECT_NAME} PRIVATE ../../../../../shared/NativeSampleModule.cpp)

# 定义 CMake 可以找到附加头文件的位置。这里同样需要从 jni、main、src、app、android 这些目录一路返回
...
- 使用 `target_sources` 指令添加我们需要构建的模块 C++ 源代码。默认情况下，React Native 已经会为 `appmodules` 库填充默认源文件，这里我们添加自己的自定义源文件。你可以看到，我们需要从 `jni` 文件夹一路返回到存放 C++ Turbo Module 的 `shared` 文件夹。
- 指定 CMake 可以在哪里找到模块头文件。在这里也一样，我们需要从 `jni` 文件夹一路返回。
...
2. 在现有的 `android` 块内，将以下代码块添加到 Gradle 文件中：
...
这段代码告诉 Gradle 文件去哪里查找 CMake 文件。该路径是相对于 `build.gradle` 文件所在文件夹的，因此我们需要添加指向 `jni` 文件夹中 `CMakeLists.txt` 文件的路径。

#### 3. 注册新的 Turbo Native Module

最后一步是在运行时注册这个新的 C++ Turbo Native Module，这样当 JS 请求这个 C++ Turbo Native Module 时，应用就知道在哪里找到它并将其返回。
...
std::shared_ptr<TurboModule> cxxModuleProvider(
  ...
  // 这里你可以提供来自
  // 你的应用或外部库的 CXX Turbo Modules。可遵循如下方式（以名为 `NativeCxxModuleExample` 的模块为例）：
  //
...
+  }
  ...
  // 然后回退到 autolink 的 CXX module providers
...
这些步骤会从 React Native 下载原始的 `OnLoad.cpp` 文件，以便我们可以安全地覆盖它，从而在应用中加载 C++ Turbo Native Module。

下载完该文件后，我们可以通过以下方式对其进行修改：

- 包含指向我们模块的头文件
- 注册 Turbo Native Module，使得当 JS 请求它时，应用可以返回它
...
我们需要运行的第一步是每次准备 iOS 应用时都会执行的常规步骤。CocoaPods 是我们用来设置和安装 React Native 依赖的工具，在这个过程中，它也会为我们运行 Codegen。
...
2. 配置 `package.json`，将 JS 模块名称关联到 `ModuleProvider` 类。
...
3. 添加名称 `NativeSampleModuleProvider`（其余字段保持为 `Subclass of: NSObject` 和 `Language: Objective-C`）
4. 点击 Next 生成文件。
...
这段代码通过在调用 `getTurboModule:` 方法时创建纯 C++ 的 `NativeSampleModule`，实现了 `RCTModuleProvider` 协议。

##### 3.2 更新 package.json

最后一步是更新 `package.json`，告诉 React Native JS 规范中的 Native Module 与原生代码中具体实现之间的对应关系。
...
- `const revString = SampleTurboModule.reverseString(value);` 位于 `onPress` 回调中：这是在应用中使用 Turbo Native Module 的方式。

:::warning
为了这个示例并尽可能保持简短，我们直接在应用中导入了 spec 文件。
在这种情况下，更好的实践是创建一个单独的文件来封装这些 spec，并在应用中使用该文件。
这样可以让你为这些 spec 准备输入，并在 JS 中对它们拥有更多控制。
:::
