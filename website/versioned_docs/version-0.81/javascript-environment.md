...
- 在大多数情况下，React Native 会使用 [Hermes](hermes)，这是一个针对 React Native 进行优化的开源 JavaScript 引擎。
...
- 使用 Chrome 调试时，所有 JavaScript 代码都会在 Chrome 自身内部运行，并通过 WebSockets 与原生代码通信。Chrome 使用 [V8](https://v8.dev/) 作为其 JavaScript 引擎。

虽然这些环境非常相似，但你仍可能遇到一些不一致之处。最好避免依赖任何运行时的特定细节。

## JavaScript 语法转换器

语法转换器通过让你可以使用新的 JavaScript 语法，而不必等待所有解释器都支持它，从而让编写代码更加愉快。

React Native 内置了 [Babel JavaScript 编译器](https://babeljs.io)。有关其支持的转换的更多详情，请查看 [Babel 文档](https://babeljs.io/docs/plugins/#transform-plugins)。
