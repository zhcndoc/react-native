...
> **注意：** 如果 `url` 指向本地文件，或是 base64 编码的 uri，则它将指向的文件会被加载并直接分享。通过这种方式，你可以分享图片、视频、PDF 文件等。如果 `url` 指向远程文件或地址，它必须符合 [RFC 2396](https://www.ietf.org/rfc/rfc2396.txt) 中描述的 URL 格式。例如，没有适当协议 (HTTP/HTTPS) 的 web URL 将不会被分享。

`failureCallback` 函数接受一个参数，一个错误对象。该对象上定义的唯一属性是一个可选的 `string` 类型的 `stack` 属性。
