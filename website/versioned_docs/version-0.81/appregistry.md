...
| taskCancelProvider<br/><div className="label basic required two-lines">必需</div> | [TaskCancelProvider](appregistry#taskcancelprovider) | 一个不返回值的函数，不接受参数；当请求取消时，由 taskProvider 执行的函数应该结束并尽快返回。                                                                    |
