...
`config` 参数是一个包含以下键的对象。[`create`](layoutanimation.md#create) 会返回一个有效的 `config` 对象，[`Presets`](layoutanimation.md#presets) 中的对象也都可以作为 `config` 传入。
...
- `property`，要动画过渡的[布局属性](layoutanimation.md#properties)（可选，但建议在 `create` 和 `delete` 中使用）
...
用于创建一个对象（包含 `create`、`update` 和 `delete` 字段），将其传入 [`configureNext`](layoutanimation.md#configurenext)。`type` 参数是一个[动画类型](layoutanimation.md#types)，`creationProp` 参数是一个[布局属性](layoutanimation.md#properties)。
