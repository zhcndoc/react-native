...
- **深度导入弃用**：从 0.80 开始，我们将针对来自 `react-native` 包的深度导入引入弃用警告。
- **可选择启用的 Strict TypeScript API**：我们正在迁移到基于源码的 TypeScript 类型，以及一个新的、基于 TypeScript 的公共 API 基线。这些改进将带来更强、更具未来适应性的类型准确性，并且这将是一次性的破坏性变更。[通过](/blog/2025/06/12/moving-towards-a-stable-javascript-api#future--strict-typescript-api-by-default) 在你项目的 `tsconfig.json` 中的 `compilerOptions` 进行[选择启用]。
- 我们将随着时间与社区合作，确保这些变更对所有人都能正常工作，然后再在未来的 React Native 版本中默认启用 Strict TypeScript API。
...
<div style={{display: 'flex', alignItems: 'center', gap: 40}}>
  <div style={{flex: 1, gap: 8}}>
    ...
    <span>
      我们在 <strong>App.js 2025</strong> 分享了关于我们动机以及 Strict TypeScript API 背后工作的深入解析。
    </span>
    <a
      href="https://www.youtube.com/live/UTaJlqhTk2g?si=SDRmj80kss7hXuGG&t=6520"
      target="_blank">
      <strong>在 YouTube 上查看</strong>
    </a>
  </div>
  <img
