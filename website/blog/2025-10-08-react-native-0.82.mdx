...
如果你还没有将项目迁移到新架构，我们建议先将项目迁移到 React Native 0.81 或 Expo SDK 54。这是最后允许你使用遗留架构的版本。它们包含警告和性能改进，专门用于帮助迁移到新架构。

<br /> 然后在 0.81 中启用新架构，并确认
你的应用运行正常。
<br /> 一旦你在 0.81 中使用了新架构，就可以
安全更新到 React Native 0.82，该版本将禁止启用
遗留架构。
...
```jsx
// 预期在 Hermes V1 中输出 "250829098.0.1"
