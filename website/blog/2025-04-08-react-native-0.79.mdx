...
本次发布包含了 [Metro 0.82](https://github.com/facebook/metro/releases/tag/v0.82.0) 版本。该版本引入了延迟哈希技术，提升了首次执行 `yarn start` 的速度，通常提高 3 倍以上（大型项目和 monorepos 中提升更明显），为日常开发体验与 CI 构建带来加速。
